...
(function () {
    ...
    let currentYearFilter = null; // null, '2026', '2025', '2024', 'older'
    let currentGenreFilters = []; // active genre names
    ...
    function createFilterPanel() {
        ...
        content.querySelectorAll(".filter-btn[data-genre]").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                currentGenreFilters = [...content.querySelectorAll(".filter-btn[data-genre].active")].map(b => b.dataset.genre);
            });
        });
        ...
        content.querySelector("#apply-filters").addEventListener("click", () => {
            ...
            if (currentFilter !== 'all') {
                const labels = { movies: 'Movies', tv: 'TV Shows', nowplaying: 'Now Playing' };
                parts.push(labels[currentFilter] || currentFilter);
            }
            if (currentYearFilter) {
                parts.push(currentYearFilter === 'older' ? 'Pre-2024' : currentYearFilter);
            }
            if (currentGenreFilters.length > 0) {
                parts.push(currentGenreFilters.join(', '));
            }

            if (parts.length > 0) {
                showToast('Now playing: ' + parts.join(' \u00b7 '));
            } else {
    ...
    function updateFilterBadge() {
        ...
        let count = 0;
        const parts = [];
        if (currentYearFilter) { count++; parts.push(currentYearFilter); }
        if (currentGenreFilters.length > 0) { count += currentGenreFilters.length; parts.push(currentGenreFilters.join(', ')); }
        if (currentFilter !== 'all') { count++; parts.push(currentFilter === 'movies' ? 'Movies' : 'TV'); }
        // Remove old badge
        const oldBadge = toggle.querySelector('.filter-badge');
        if (oldBadge) oldBadge.remove();
        if (count > 0) {
            const badge = document.createElement('span');
            badge.className = 'filter-badge';
            badge.textContent = count;
            badge.style.cssText = 'position:absolute;top:-4px;right:-4px;background:#ef4444;color:#fff;font-size:10px;font-weight:bold;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;pointer-events:none;';
            toggle.style.position = 'relative';
            toggle.appendChild(badge);
            toggle.title = 'Filters: ' + parts.join(' \u00b7 ');
        } else {
            toggle.title = '';
        }
    }
    ...
    function setupNavigationHandlers() {
        ...
        const findAndSetupNavButtons = () => {
            ...
            allButtons.forEach(btn => {
                ...
                // Skip buttons inside enhancer overlays (Freestyle, Motivation, Top 50, Playlist, etc.)
                if (btn.closest('#ms2-freestyle-overlay, #ms2-motivation-overlay, #ms2-top-panel, #ms2-playlist-panel, #freestyleOverlay, .freestyle-overlay')) return;
    ...
    let settingsCollapsed = localStorage.getItem("movieshows-settings-collapsed") !== "false";
    ...
    async function loadMoviesData() {
        ...
        // Try local DB API first, then centralized DB (50webs), then static JSON
        var sources = [
            { url: 'api/get-movies.php', name: 'Database API (local)', needsTransform: true }
        ];
        // If not on findtorontoevents.ca, also try the centralized DB
        if (window.location.hostname !== 'findtorontoevents.ca') {
            sources.push({ url: 'https://findtorontoevents.ca/MOVIESHOWS2/api/get-movies.php', name: 'Database API (50webs)', needsTransform: true });
        }
        sources.push({ url: 'movies-database.json?v=' + new Date().getTime(), name: 'Static JSON', needsTransform: false });
        ...
        if (scrollAfter) {
            ...
            setTimeout(() => {
                ...
                if (iframe && iframe.dataset.src) {
                    ...
                    try { if (typeof updateMuteControl === 'function') updateMuteControl(); } catch(e) {}
    ...
    function handleScroll() {
        ...
        if (newIndex !== currentIndex) {
            ...
            forceStopAllVideos();
            // Debounce: cancel pending play, schedule new one
            if (_playPendingTimer) clearTimeout(_playPendingTimer);
            _playPendingTimer = setTimeout(() => {
        ...
        if (isQueuePanelOpen()) {
            ...
            scrollEndTimer = setTimeout(() => {
                ...
                // Don't call forceStopAll/forcePlay again — the main scroll handler already did it
                updateQueuePanel();
    ...
    function getFilteredMovies() {
        ...
        // Apply year filter
        if (currentYearFilter) {
            if (currentYearFilter === 'older') {
                filtered = filtered.filter(m => {
                    const y = parseInt(m.year);
                    return y && y < 2024;
                });
            } else {
                filtered = filtered.filter(m => String(m.year) === currentYearFilter);
            }
        }

        // Apply genre filters (match any selected genre)
        if (currentGenreFilters.length > 0) {
            filtered = filtered.filter(m => {
                if (!m.genres || !m.genres.length) return false;
                return currentGenreFilters.some(g => m.genres.includes(g));
            });
        }
    ...
    function init() {
        ...
        updateFilterBadge(); // Show badge if filters were persisted
        createLayoutControl();
                    ...
                    switch(action) {
                        ...
                        case 'search':
                            const searchBtn = Array.from(document.querySelectorAll('button')).find(b =>
                                !b.closest('#ms2-freestyle-overlay, #ms2-motivation-overlay, #ms2-top-panel, #ms2-playlist-panel, #freestyleOverlay, .freestyle-overlay') &&

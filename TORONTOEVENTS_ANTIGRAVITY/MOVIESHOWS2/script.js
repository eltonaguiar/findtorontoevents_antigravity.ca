...
async function loadContent() {
  ...
  // 4) Fallback: load from central database API (findtorontoevents.ca)
  try {
    const apiRes = await fetchJson("https://findtorontoevents.ca/MOVIESHOWS2/api/movies.php?action=list&limit=5000");
    if (apiRes && apiRes.success && Array.isArray(apiRes.movies)) {
      const normalized = normalizePayload(apiRes.movies);
      if (normalized.length) {
        console.log(`[loadContent] Loaded ${normalized.length} items from central API`);
        return normalized;
      }
    }
  } catch (_) {}

# Live Trading System - Architecture Design Document

> **HONESTY NOTICE (Feb 16, 2026):**
> This document describes an **aspirational architecture** that has NOT been built.
> None of the infrastructure below (Kubernetes, TimescaleDB, Redis Streams,
> Prometheus, Grafana) is deployed or running.
>
> **What actually runs:** A Python script (`live_scanner.py`) executed every 15
> minutes by GitHub Actions. It fetches Yahoo Finance data, applies 5 academic
> strategies + 5 scout signals, and writes JSON files to a static HTML dashboard.
> There is no WebSocket feed, no < 20ms latency, no order execution, and no
> Kubernetes cluster. Latency is measured in minutes (cron schedule), not
> milliseconds.
>
> This document is retained as a **design reference** for what a production
> system could look like if funded and built. Do not cite it as current state.

## Overview

This is a **design document** for a production-grade live trading system architecture. If implemented, it would support:

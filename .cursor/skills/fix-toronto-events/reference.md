...
Read these project files when you need full detail. Paths are relative to the workspace root (`e:\findtorontoevents_antigravity.ca`).
...
- **serve_local.py** – **Use this for local testing.** Serves `/next/_next/` with correct MIME (application/javascript for .js, text/css for .css) and handles proxy-style URLs. Do NOT use `python -m http.server` (wrong MIME / PHP source → SyntaxError → events never load).
...
- **Do not run:** `python -m http.server` – it does not set correct MIME for `/next/_next/` and would serve PHP source for proxy URLs → SyntaxError → React never loads → events never load.
...
| Issue | Cause | Fix |
|-------|--------|-----|
| **Local: events not loading, SyntaxError** | (1) `python -m http.server` → wrong MIME or PHP source. (2) Nav chunk syntax error (extra `]` after FAVCREATORS). | Use **`python tools/serve_local.py`**. If error persists, fix chunk: FAVCREATORS closing must be `]})]})]}),` not `]})]})]})],`. |
...
| **Events fetch** | Consuming/cloning can break app’s response. | Intercept `events.json` request → try `/next/events.json`, then `/events.json`, then `/data/events.json`; return the fetch promise so the app gets the response. |

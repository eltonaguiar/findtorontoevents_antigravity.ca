...
<<<<<<< Updated upstream
        ...
        # TRAINED on-disk bundles (score_active_picks_ab joblib.loads OLD/NEW itself) and
        # its only training dependency, strategy_router, is persisted to
        # models/strategy_router.json — it needs NO fresh training data. Run it here
...
>>>>>>> Stashed changes
        ...
        # joblib.loads OLD/NEW itself) and its only training dependency, strategy_router,
        # is persisted to models/strategy_router.json — so it needs NO fresh training
        # data. Run it here (with on-disk bundles + persisted router) so the A/B sidecar
...
=======
                print("[gatekeeper] #134: insufficient retrain data → A/B dual-write with on-disk bundles")
>>>>>>> Stashed changes
                score_active_picks_ab(_sr)
            except Exception as _e:
                print(f"[gatekeeper] #134 low-data A/B dual-write failed ({_e!r}); using heuristic fallback")
                heuristic_fallback()
        else:
            heuristic_fallback()
        return

 ...
 * Every 5 minutes: php /path/to/cron_scheduler.php  
 * Every 2 hours for stats: php /path/to/cron_scheduler.php stats
 */
...
if ($mode === 'auto') {
    ...
    $to_scrape = array();
    if ($nba_active && $is_game_time)
        $to_scrape[] = 'nba';
    if ($nhl_active && $is_game_time)
        $to_scrape[] = 'nhl';
    if ($nfl_active && ($hour >= 17 || $hour <= 23))
        $to_scrape[] = 'nfl'; // NFL mostly weekends
    if ($mlb_active && $is_game_time)
        $to_scrape[] = 'mlb';

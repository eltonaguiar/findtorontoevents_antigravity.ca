...
class NBAScraper {
    ...
    private function _ensure_tables() {
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_nba_odds (
            id INT AUTO_INCREMENT PRIMARY KEY, game_date DATETIME, away_team VARCHAR(100), home_team VARCHAR(100),
            spread VARCHAR(20), total VARCHAR(20), bookmaker VARCHAR(50), recorded_at DATETIME DEFAULT NOW(),
            INDEX idx_teams (away_team, home_team), INDEX idx_date (game_date)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
        
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_nba_team_stats (
            id INT AUTO_INCREMENT PRIMARY KEY, team_name VARCHAR(100), abbreviation VARCHAR(10), conference VARCHAR(20),
            wins INT, losses INT, win_pct DECIMAL(4,3), streak VARCHAR(20), updated_at DATETIME DEFAULT NOW(),
            UNIQUE KEY idx_team (team_name), INDEX idx_conf (conference)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
        
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_nba_injuries (
            id INT AUTO_INCREMENT PRIMARY KEY, player VARCHAR(100), team VARCHAR(100), status VARCHAR(100),
            source VARCHAR(50), reported_at DATETIME DEFAULT NOW(), INDEX idx_team (team), INDEX idx_player (player)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
        
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_nba_schedule (
    ...
    private function _store_odds($odds) {
        foreach ($odds as $source => $games) {
            ...
            foreach ($games as $game) {
                ...
                $query = "INSERT INTO lm_nba_odds (away_team, home_team, spread, total, bookmaker, recorded_at) 
    ...
    private function _store_stats($stats) {
        foreach ($stats as $team) {
            ...
            $query = "INSERT INTO lm_nba_team_stats (team_name, abbreviation, conference, wins, losses, win_pct, streak, updated_at)
                      VALUES ('$name', '$abbr', '$conf', $wins, $losses, $win_pct, '$streak', NOW())
                      ON DUPLICATE KEY UPDATE wins=VALUES(wins), losses=VALUES(losses), win_pct=VALUES(win_pct), streak=VALUES(streak), updated_at=NOW()";
            $this->conn->query($query);
        }
    }
    
    private function _store_injuries($injuries) {
        $this->conn->query("DELETE FROM lm_nba_injuries WHERE reported_at < DATE_SUB(NOW(), INTERVAL 7 DAY)");
        foreach ($injuries as $injury) {
            ...
            $query = "INSERT INTO lm_nba_injuries (player, team, status, source, reported_at)
    ...
    private function _store_schedule($games) {
        foreach ($games as $game) {
            ...
            $query = "INSERT INTO lm_nba_schedule (game_id, game_date, away_team, home_team, away_score, home_score, status, updated_at)
...
$scraper = new NBAScraper($conn);

if ($action === 'scrape') {
    $results = $scraper->scrape_all();
    echo json_encode(array('ok' => true, 'sport' => 'NBA', 'data' => $results, 'timestamp' => gmdate('Y-m-d H:i:s')));
} elseif ($action === 'odds') {

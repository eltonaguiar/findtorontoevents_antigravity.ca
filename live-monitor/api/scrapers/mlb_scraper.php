...
class MLBScraper {
    ...
    private function _scrape_espn_lines() {
        ...
        foreach ($matches[1] as $game_html) {
            $game = array('away_team' => '', 'home_team' => '', 'run_line' => '', 'total' => '', 'bookmaker' => 'ESPN');
            if (preg_match('/<span[^>]*class="[^"]*team-name[^"]*"[^>]*>(.*?)<\/span>/', $game_html, $tm)) {
                $game['away_team'] = trim(strip_tags($tm[1]));
            }
            if (preg_match('/Run Line[^>]*>([^<]+)/', $game_html, $rl)) {
                $game['run_line'] = trim($rl[1]);
            }
    ...
    private function _fetch_espn_standings() {
        ...
        foreach ($data['children'] as $league) {
            foreach ($league['children'] as $div) {
                if (!isset($div['standings']['entries'])) continue;
                $div_name = isset($div['name']) ? $div['name'] : '';
                $league_name = isset($league['name']) ? $league['name'] : '';
                foreach ($div['standings']['entries'] as $entry) {
                    ...
                    $teams[] = array(
                        ...
                        'gb' => isset($stats['gamesBack']) ? $stats['gamesBack'] : '',
                        'runs_for' => isset($stats['runsFor']) ? (int)$stats['runsFor'] : 0,
                        'runs_against' => isset($stats['runsAgainst']) ? (int)$stats['runsAgainst'] : 0,
    ...
    private function _ensure_tables() {
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_mlb_odds (
            id INT AUTO_INCREMENT PRIMARY KEY, game_date DATETIME, away_team VARCHAR(100), home_team VARCHAR(100),
            run_line VARCHAR(20), total VARCHAR(20), away_ml VARCHAR(10), home_ml VARCHAR(10), bookmaker VARCHAR(50),
            recorded_at DATETIME DEFAULT NOW(), INDEX idx_teams (away_team, home_team), INDEX idx_date (game_date)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
        
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_mlb_team_stats (
            id INT AUTO_INCREMENT PRIMARY KEY, team_name VARCHAR(100), abbreviation VARCHAR(10), league VARCHAR(20),
            division VARCHAR(50), wins INT, losses INT, win_pct DECIMAL(4,3), gb VARCHAR(10), runs_for INT, runs_against INT,
            streak VARCHAR(20), updated_at DATETIME DEFAULT NOW(), UNIQUE KEY idx_team (team_name), INDEX idx_div (division)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
        
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_mlb_injuries (
        ...
        $this->conn->query("CREATE TABLE IF NOT EXISTS lm_mlb_schedule (
            id INT AUTO_INCREMENT PRIMARY KEY, game_id VARCHAR(50), game_date DATETIME, away_team VARCHAR(100),
            home_team VARCHAR(100), away_score INT, home_score INT, probable_pitchers TEXT, status VARCHAR(50),
            updated_at DATETIME DEFAULT NOW(), UNIQUE KEY idx_game (game_id), INDEX idx_date (game_date)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8");
    }
    ...
    private function _store_odds($odds) {
        foreach ($odds as $source => $games) {
            ...
            foreach ($games as $game) {
                ...
                $rl = isset($game['run_line']) ? $this->conn->real_escape_string($game['run_line']) : '';
                ...
                $query = "INSERT INTO lm_mlb_odds (away_team, home_team, run_line, total, bookmaker, recorded_at) 
                          VALUES ('$away', '$home', '$rl', '$total', '$book', NOW())";
    ...
    private function _store_stats($stats) {
        foreach ($stats as $team) {
            ...
            $league = isset($team['league']) ? $this->conn->real_escape_string($team['league']) : '';
            ...
            $gb = isset($team['gb']) ? $this->conn->real_escape_string($team['gb']) : '';
            $rf = isset($team['runs_for']) ? (int)$team['runs_for'] : 0;
            $ra = isset($team['runs_against']) ? (int)$team['runs_against'] : 0;
            $streak = isset($team['streak']) ? $this->conn->real_escape_string($team['streak']) : '';
            $query = "INSERT INTO lm_mlb_team_stats (team_name, abbreviation, league, division, wins, losses, win_pct, gb, runs_for, runs_against, streak, updated_at)
                      VALUES ('$name', '$abbr', '$league', '$div', $wins, $losses, $win_pct, '$gb', $rf, $ra, '$streak', NOW())
                      ON DUPLICATE KEY UPDATE wins=VALUES(wins), losses=VALUES(losses), win_pct=VALUES(win_pct),
                      gb=VALUES(gb), runs_for=VALUES(runs_for), runs_against=VALUES(runs_against), streak=VALUES(streak), updated_at=NOW()";
            $this->conn->query($query);
        }
    }
    
    private function _store_injuries($injuries) {
        $this->conn->query("DELETE FROM lm_mlb_injuries WHERE reported_at < DATE_SUB(NOW(), INTERVAL 7 DAY)");
        foreach ($injuries as $injury) {
            ...
            $query = "INSERT INTO lm_mlb_injuries (player, team, position, status, source, reported_at)
    ...
    private function _store_schedule($games) {
        foreach ($games as $game) {
            ...
            $pitchers = isset($game['probable_pitchers']) ? $this->conn->real_escape_string(json_encode($game['probable_pitchers'])) : '';
            $status = isset($game['status']) ? $this->conn->real_escape_string($game['status']) : '';
            $query = "INSERT INTO lm_mlb_schedule (game_id, game_date, away_team, home_team, away_score, home_score, probable_pitchers, status, updated_at)
                      VALUES ('$game_id', '$date', '$away', '$home', $away_score, $home_score, '$pitchers', '$status', NOW())
                      ON DUPLICATE KEY UPDATE away_score=VALUES(away_score), home_score=VALUES(home_score), probable_pitchers=VALUES(probable_pitchers), status=VALUES(status), updated_at=NOW()";
            $this->conn->query($query);
        }
    }
}
...
$scraper = new MLBScraper($conn);

if ($action === 'scrape') {
    $results = $scraper->scrape_all();
    echo json_encode(array('ok' => true, 'sport' => 'MLB', 'data' => $results, 'timestamp' => gmdate('Y-m-d H:i:s')));
} elseif ($action === 'odds') {

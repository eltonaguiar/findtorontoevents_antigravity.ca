...
$PF_ADMIN_KEY = getenv('ADMIN_API_KEY') !== false ? getenv('ADMIN_API_KEY') : '';
...
$meme_conn = new mysqli('mysql.50webs.com', 'ejaguiar1_memecoin', getenv('MEMECOIN_DB_PASS') ?: '', 'ejaguiar1_memecoin');
...
function _pf_get_current_price($pair, $class, $meme_conn) {
    ...
    if ($class === 'FOREX') {
        ...
        if ($quote === '') { $quote = 'USD'; }
        global $CURRENCYLAYER_API_KEY;
        $url = 'http://api.currencylayer.com/live?access_key=' . $CURRENCYLAYER_API_KEY . '&currencies=' . $quote . '&source=' . $base;
        $ctx = stream_context_create(array('http' => array('timeout' => 5)));
        $raw = @file_get_contents($url, false, $ctx);
        if ($raw) {
            $data = json_decode($raw, true);

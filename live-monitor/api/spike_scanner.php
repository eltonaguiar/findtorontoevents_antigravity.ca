...
$SS_ADMIN_KEY = ($__ss_admin_key_env = getenv('ADMIN_API_KEY')) !== false ? $__ss_admin_key_env : '';
...
function _ss_require_key($expected) {
    ...
    // Deny if expected key is empty (env var not configured) or key mismatch
...
function _ss_scan_crypto_internal($conn) {
    ...
    // Primary: Crypto.com API (High performance, no rate limits for public tickers)
    $cdc_url = 'https://api.crypto.com/v2/public/get-ticker';
    $cdc = _ss_fetch_json($cdc_url);
    if ($cdc && isset($cdc['result']['data'])) {
        foreach ($cdc['result']['data'] as $item) {
            $inst = $item['i']; // e.g. BTC_USDT
            if (!str_ends_with($inst, '_USDT')) { continue; }
            $sym = str_replace('_USDT', '', $inst);
            $data[] = array(
                'symbol'        => $sym,
                'current_price' => isset($item['a']) ? $item['a'] : 0, // last trade
                'total_volume'  => isset($item['vv']) ? $item['vv'] : 0, // 24h vol
                'price_change_percentage_24h' => 0 // CDC doesn't give 24h % directly easily here
            );
        }
    }

...
$_admin_key_expected = getenv('ADMIN_API_KEY');
$admin  = ($_admin_key_expected !== false && $_admin_key_expected !== '' && $key === $_admin_key_expected);
...
function _gm_archive_meme($conn) {
    ...
    $mc = @new mysqli('mysql.50webs.com', 'ejaguiar1_memecoin', getenv('MEMECOIN_DB_PASS') ?: '', 'ejaguiar1_memecoin');
...
function _gm_check_sports_bankroll($conn) {
    ...
    // Win rate: wins / (wins + losses) only (exclude push/void/pending)
    $sr = $sc->query("SELECT
        SUM(CASE WHEN result IN ('won','win') THEN 1 ELSE 0 END) as wins,
        SUM(CASE WHEN result IN ('lost','loss') THEN 1 ELSE 0 END) as losses
        FROM lm_sports_bets WHERE result IS NOT NULL AND result != ''");
    $sports_wr = 0;
    $sports_total = 0;
    if ($sr && ($srow = $sr->fetch_assoc())) {
        $swins = intval($srow['wins']);
        $sloss = intval($srow['losses']);
        $sports_total = $swins + $sloss;
        $sports_wr = ($sports_total > 0) ? round(($swins / $sports_total) * 100, 1) : 0;
    }

    // Recent streak (last 10 directional)
    $streak_r = $sc->query("SELECT result FROM lm_sports_bets WHERE result IN ('won','win','lost','loss') ORDER BY COALESCE(settled_at, placed_at) DESC, id DESC LIMIT 10");
    $consec_losses = 0;
    if ($streak_r) {
        while ($srow = $streak_r->fetch_assoc()) {
            $rr = strtolower(trim($srow['result']));
            if ($rr === 'lost' || $rr === 'loss') {
                $consec_losses++;
            } else {
                break;

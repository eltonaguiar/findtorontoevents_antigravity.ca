...
(function() {
    ...
    // ── Fetch team stats for a sport (with failover) ──
    function fetchSportStats(sportKey) {
        var cfg = SPORT_CONFIG[sportKey];
        if (!cfg) return;

        // Try failover proxy first (if sports-failover.js is loaded)
        if (window.SportsFailover && typeof window.SportsFailover.fetch === 'function') {
            window.SportsFailover.fetch(sportKey, 'standings', function(result) {
                if (result.ok && result.data) {
                    // Normalize failover data into the same shape as the original PHP response
                    ...
                    if (Object.keys(teamMap).length > 0) {
                        window._sportTeamStats[sportKey] = teamMap;
                        enrichPickCards();
                        if (sportKey === 'NBA') renderNbaInsightsPanel({ teams: teamMap, updated_at: result.data.timestamp, source: result.source });
                        if (sportKey === 'NHL') renderNhlInsightsPanel({ teams: teamMap, updated_at: result.data.timestamp, source: result.source });
                        return; // Failover succeeded
                    }
                }
                // Failover didn't return data — fall through to original PHP endpoint
                fetchSportStatsDirect(sportKey);
            });
        } else {
            fetchSportStatsDirect(sportKey);
        }
    ...
    // Original direct PHP fetch (fallback when failover module unavailable)
    function fetchSportStatsDirect(sportKey) {
        var cfg = SPORT_CONFIG[sportKey];
        if (!cfg) return;
    ...
    function renderNbaInsightsPanel(data) {
        ...
        html += '<p class="text-dim" style="font-size:0.85rem;margin-bottom:0.5rem">Live team stats powering our NBA predictions. Updated via failover cascade.</p>';
        ...
        html += '</table></div>';
        if (data.updated_at) html += '<div style="font-size:0.72rem;color:var(--text-dim);margin-top:0.5rem;text-align:right">Updated: ' + data.updated_at + ' | Source: ' + (data.source || 'ESPN + failover') + '</div>';
    ...
    function renderNhlInsightsPanel(data) {
        ...
        html += '<p class="text-dim" style="font-size:0.85rem;margin-bottom:0.5rem">Live NHL standings powering our hockey predictions. Updated via failover cascade.</p>';

...
"""
FX Kill Switch Enforcer — Automatic detection and blocking of toxic FOREX strategies.

P0 Implementation per DAILY_IDEAS.MD (IDEA-003: FX-Kill-Switch-Enforcer).
...
Integration:
  - Called by feed_hygiene.is_valid_active_pick() for every FOREX pick
  - Updates strategy_blocklist._RETIRED_STRATEGIES dynamically
  - Logs all kills to audit_trail for forensic review

Rollback:
  - Set env FX_KILL_SWITCH_DISABLED=1 to bypass all rules
...
# Rollback flag
_FX_KILL_SWITCH_DISABLED = os.getenv("FX_KILL_SWITCH_DISABLED", "0") == "1"

# Known toxic FOREX strategies from swarm analysis
# These are immediately blocked without waiting for threshold breach
_KNOWN_TOXIC_FOREX_STRATEGIES = frozenset({
    # 2026-05-11 SUPREME EDGE: re-blocked. TEMP UNBLOCK 2026-05-08 was conditioned on
    # phantom_expired < 10% which has NOT been achieved (still 100% per /audit DB Health
    # red-tier panel even post-Wave-1 circuit-breaker rm). Restores test_fx_kill_switch
    # expectations + aligns with master plan FOREX BLOCKED state.
    "myfxbook_retail_contrarian",
    "forex_rsi2_mean_reversion",
    "ema_aggressive_prop",             # ~1326 trades, PF~0.40
            ...
            try:
                with open(path, 'r') as f:
                    ...
                    if 'strategies' in data:
                        ...
                        # Convert list of picks to strategy stats
...
def _compute_strategy_stats_from_picks(picks: List[Dict]) -> Dict[str, Dict]:
    ...
    Args:
        picks: List of pick dictionaries with 'strategy', 'pnl_pct', 'win', etc.
    
    Returns:
        Dict mapping strategy_name -> computed_stats
    """
    stats = {}
    ...
    for strategy, s in stats.items():
        ...
        # Compute decay (all-time vs rolling-30d)
...
def _compute_decay(trades: List[Dict]) -> float:
    ...
    # Decay = all-time WR - recent WR (positive = getting worse)
    return all_time_wr - recent_wr


def is_forex_strategy(strategy_name: str) -> bool:
    """
    Detect if a strategy is FOREX-focused based on naming conventions.
    Uses word-boundary detection to avoid false positives (usdt != usd).
    
    Args:
        strategy_name: Name of the strategy
    
    Returns:
        True if strategy appears to be FOREX-focused
    """
    # Exact matches (word boundaries only)
    exact_indicators = frozenset({'forex', 'fx', 'currency'})
    
    # Currency pair codes (word-boundary only, e.g. _usd, usd_)
    currency_codes = frozenset({
        'usd', 'eur', 'gbp', 'jpy', 'aud', 'nzd', 'cad', 'chf'
    })
    
    # 6-char currency pair combos (e.g. GBPJPY, EURUSD)
    six_char_pairs = frozenset({
        'eurusd', 'gbpusd', 'usdjpy', 'usdchf', 'audusd', 'usdcad', 'nzdusd',
        'eurgbp', 'gbpjpy', 'eurjpy', 'eurchf', 'gbpchf', 'audjpy', 'usdcnh',
    })
    
    name_lower = strategy_name.lower()
    parts = frozenset(name_lower.replace('-', '_').split('_'))
    
    # Check exact indicators at word boundaries
    if parts & exact_indicators:
        return True
    
    # Check currency codes at word boundaries
    if parts & currency_codes:
        return True
    
    # Check 6-char currency pair combos (GBPJPY, EURUSD, etc.)
    if any(pair in name_lower for pair in six_char_pairs):
        return True
...
def check_strategy_for_kill(
    ...
    """
    Check if a strategy should be killed based on FX kill rules.
    ...
    if not is_forex_strategy(strategy_name):
        return False, "Not a FOREX strategy"
    
    # Load stats if not provided
    if stats is None:
        all_stats = _load_strategy_performance()
        stats = all_stats.get(strategy_name, {})
    ...
    if not _rule_kill:
        return False, "Strategy passes all FX kill rules"
    ...
    # the Phase-2-D failure mode (CT=F killed on n=12, CL=F on n=6). Only
    # the min-n check from kill_gate is applied here; the binomial / Wilson
    # WR test would be a category error against PF- and decay-based rules.
    # Safe default-on. Override: M055_KILL_GATE_ENABLED=0.
    if os.environ.get("M055_KILL_GATE_ENABLED", "1") != "0":
        try:
            from audit_trail.kill_gate import MIN_N_BY_ASSET_CLASS, MIN_N_DEFAULT
            _min_n = MIN_N_BY_ASSET_CLASS.get("FOREX", MIN_N_DEFAULT)
...
def get_killed_strategies() -> Set[str]:
    ...
    """
    killed = set(_KNOWN_TOXIC_FOREX_STRATEGIES)
    
    if _FX_KILL_SWITCH_DISABLED:
        logger.warning("FX_KILL_SWITCH_DISABLED=1 - not checking dynamic kills")
        return killed
    ...
    for strategy_name, stats in all_stats.items():
        ...
        if should_kill and strategy_name not in killed:
            logger.warning(f"FX Kill Switch: {strategy_name} - {reason}")
...
def log_kill_to_audit(strategy_name: str, reason: str) -> None:
    ...
    audit_paths = [
        Path("audit_trail/data/fx_kill_audit.jsonl"),
        Path("alpha_engine/data/fx_kill_audit.jsonl"),
    ]
    ...
    for path in audit_paths:
        try:
            ...
            logger.info(f"Logged FX kill to {path}")
            return
        except Exception as e:
...
def refresh_kill_list() -> List[str]:
    ...
    if newly_killed:
        logger.warning(f"FX Kill Switch: {len(newly_killed)} strategies newly blocked: {newly_killed}")
    
    return newly_killed


# Module initialization - populate initial kill list
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    print("FX Kill Switch Enforcer v1.0.0")
    print("=" * 60)
    
    killed = refresh_kill_list()
    
    print(f"\nTotal strategies blocked: {len(killed)}")
    print(f"Known toxic (hard-coded): {len(_KNOWN_TOXIC_FOREX_STRATEGIES)}")
    print(f"Dynamically detected: {len(killed) - len(_KNOWN_TOXIC_FOREX_STRATEGIES)}")

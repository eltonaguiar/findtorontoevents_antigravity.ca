"""Alpha Engine -- Audit Trail Push (SQLite + MySQL dual-write)."""
import json, logging, os, sys

log = logging.getLogger("alpha.audit_push")
...
def normalize(p):
    return {
        ...
        "take_profit": p.get("take_profit", p.get("tp_price", 0)),
        "stop_loss": p.get("stop_loss", p.get("sl_price", 0)),
        "confidence": p.get("confidence", p.get("ml_score", 0.5)),
        "strategy": p.get("strategy", p.get("id", "alpha_engine")),
        "timestamp": p.get("timestamp", p.get("signal_time", "")),
    }
...
def main():
    ...
    picks = [p for p in load_picks() if p.get("status", "").upper() != "CLOSED"]
    if not picks:
        log.info("No Alpha Engine picks to push")
        return
    ...
    record_event("ALPHA_AUDIT_PUSH", run_id=run_id,
    ...
    log.info("AlphaEngine: %d/%d picks pushed to audit trail", ok, len(picks))

...
def _generate_luxalgo_short_v2() -> list[dict[str, Any]]:
    ...
    # P0-1 fallback: probation symbols when live scanner empty (intrabar luxalgo SHORT n=38)
    if not picks:
        _fallback_syms = ("NEARUSDT", "SOLUSDT", "AVAXUSDT", "LINKUSDT", "ADAUSDT")
        for sym in _fallback_syms[:3]:
            picks.append(_tag_forward({
                "symbol": sym,
                "direction": "SHORT",
                "asset_class": "CRYPTO",
                "strategy": "luxalgo_confluence",
                "source_system": "june2026_luxalgo_short_fallback",
                "confidence": 0.65,
                "score": 72,
                "trust": 5,
            }, "luxalgo_confluence_v2_short",
               ENHANCED_V2_BY_CLASS["CRYPTO"][0]["enhancement"]))
    return picks[:5]

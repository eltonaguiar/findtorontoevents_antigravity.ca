...
def equity_rsi_divergence_mr(data: Dict[str, pd.DataFrame]) -> List[Dict[str, Any]]:
    ...
    for symbol in symbols:
        ...
        # Look for divergence over last 15 bars (relaxed from 10)
        lookback = min(15, len(df) - 2)

        # Check for bullish divergence: RSI makes higher low while price makes lower low
        bullish_div = False
        if current_rsi < 45:  # Further relaxed for signal generation
            ...
            for i in range(-lookback, 0):
                ...
                if (
                    rsi_14.iloc[i] < rsi_14.iloc[i - 1]
                    and rsi_14.iloc[i] < rsi_14.iloc[i + 1]
                    and i - 1 >= -len(rsi_14)
                    and i + 1 < 0
                ):
            ...
            if len(price_lows) >= 2 and len(rsi_lows) >= 2:
                ...
                prev_price_low = max(
                    [p for p in price_lows if p[0] < recent_price_low[0]],
                    key=lambda x: x[0],
                    default=None,
                )
                ...
                prev_rsi_low = max(
                    [r for r in rsi_lows if r[0] < recent_rsi_low[0]],
                    key=lambda x: x[0],
                    default=None,
                )
                ...
                if (
                    prev_price_low
                    and prev_rsi_low
                    and recent_price_low[1] < prev_price_low[1]  # Price made lower low
                    and recent_rsi_low[1] > prev_rsi_low[1]  # RSI made higher low
                ):
                    # Relaxed divergence: allow partial divergences (RSI higher low without price lower low)
                    bullish_div = True
                elif (
                    prev_price_low
                    and prev_rsi_low
                    and recent_price_low[1]
                    <= prev_price_low[1] * 1.02  # Allow small price increases
                    and recent_rsi_low[1] > prev_rsi_low[1]  # RSI higher low
                ):
                    # Additional partial divergence condition
                    bullish_div = True

        # Check for bearish divergence: RSI makes lower high while price makes higher high
        bearish_div = False
        if current_rsi > 55:  # Further relaxed for signal generation
            price_highs = []
            rsi_highs = []
            ...
            if len(price_highs) >= 2 and len(rsi_highs) >= 2:
                ...
                prev_price_high = min(
                    [p for p in price_highs if p[0] < recent_price_high[0]],
                    key=lambda x: x[0],
                    default=None,
                )
                ...
                prev_rsi_high = min(
                    [r for r in rsi_highs if r[0] < recent_rsi_high[0]],
                    key=lambda x: x[0],
                    default=None,
                )
                ...
                if (
                    prev_price_high
                    and prev_rsi_high
                    and recent_price_high[1]
                    > prev_price_high[1]  # Price made higher high
                    and recent_rsi_high[1] < prev_rsi_high[1]  # RSI made lower high
                ):
                    bearish_div = True
                elif (
                    prev_price_high
                    and prev_rsi_high
                    and recent_price_high[1]
                    >= prev_price_high[1] * 0.98  # Allow small price decreases
                    and recent_rsi_high[1] < prev_rsi_high[1]  # RSI lower high
                ):
                    # Additional partial divergence condition
                    bearish_div = True
            ...
            signals.append(
                {
                    ...
                    "confidence": round(
                        min(
                            0.75,
                            0.50
                            + (30 - current_rsi) / 30 * 0.2
                            + (0.1 if vol_r > 1.5 else 0),
                        ),
                        2,
                    ),

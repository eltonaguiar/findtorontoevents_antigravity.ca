...
| 4 | pm_macro_overlay fetcher fix (Phase 1 was a silent no-op) | ✅ this PR | Live-verified 2026-06-12 20:00Z: `Kalshi KXFEDDECISION-26JUN: cut=2.0% hike=2.0% hold=98.0%` + `Polymarket meeting 2026-06: cut=0.5% hike=0.9% hold=98.6%` → consensus HOLD, 0 picks (correct). Pre-fix: both fetchers returned None every run since 2026-06-06 |
| 5 | First eligible lead/lag report (`status: OK`) | ⏳ ~2026-07-03 | Needs ≥20 daily snapshots per market; capture runs in every alpha-engine-live full-cycle (2h cron, dedupe = 1 row/market/day) |
| 6 | Wire lead/lag verdicts into production | ⏳ blocked on #5 | Target: `pm_macro_overlay.py::generate_macro_picks` (gate emission on LEADING verdict) and/or `audit_trail/quality_gates.py` source weight. Consumers must only use reports generated strictly BEFORE the pick timestamp (no leakage) |

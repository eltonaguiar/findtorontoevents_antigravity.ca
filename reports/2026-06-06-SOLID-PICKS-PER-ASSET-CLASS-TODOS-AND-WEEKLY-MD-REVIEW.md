...
- Explicit answer to "are the T1 items under ai-tournament legit?": **No, not for Goal #1 / real money / main /audit edge claims.** As of latest ai_tournament_leaderboard.json (2026-06-05 03:28Z): only 3 T1s (kimi_direct resolved n=52 wr67.3% pf2.78 but 48 excluded_untrust; grok3 n=98 wr55.1% pf2.22 but 322 excluded; llm7_qwen n=39 wr69.2% pf2.4 but 40 excluded). These use tournament-internal CI-adjusted scoring on *their own generated picks*, with tiny effective clean samples after filters, and are last in the trust hierarchy ("tournament_picks / ai_leaderboard (research only)"). They do not flow into policy-clean closed picks, pf_registry, or money_ready_verdict (which show 0/9 ready, CRYPTO ~36%/PF~1.0 on n~300 clean). The live ai-tournament.html already carries the full "UNPROVEN until 7d + n>=100 post intrabar+drift cleanup" banner + column tooltip exactly because of the 914 MISPRICED + missing replay issues. Tournament T1s are a meta-game ranking of LLMs/personas, not evidence of production edge. Only mega_mutation::crypto (on the real closed picks ledger, post all hygiene/OHLCV/scrutiny) is the current verified solid T1.

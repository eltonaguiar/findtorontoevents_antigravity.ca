...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($settingsInput as $s) {
        ...
        if ($conn->query($sql)) {
            $saved++;
            $sv_q = $conn->query("SELECT sync_version FROM accountability_reminder_settings WHERE discord_user_id = $d_id_sql AND app_user_id = $a_id_sql AND task_id = $task_id LIMIT 1");
            $sv = 1;
            if ($sv_q && $sv_q->num_rows > 0) {
                $sv_row = $sv_q->fetch_assoc();
                $sv = isset($sv_row['sync_version']) ? (int)$sv_row['sync_version'] + 1 : 1;
                $conn->query("UPDATE accountability_reminder_settings SET sync_version = $sv WHERE discord_user_id = $d_id_sql AND app_user_id = $a_id_sql AND task_id = $task_id");
            }
            $pk_vals = array('discord_user_id' => $discord_id, 'app_user_id' => $app_user_id, 'task_id' => $task_id);
            $row_data = array('discord_user_id' => $discord_id, 'app_user_id' => $app_user_id, 'task_id' => $task_id, 'discord_reminder' => $discord_rem, 'dashboard_reminder' => $dashboard_rem, 'reminder_time' => $reminder_time, 'timezone' => $tz, 'sync_version' => $sv);
            $email = '';
            if ($app_user_id > 0) {
                $email = sync_get_user_email_local($conn, $app_user_id);
            }
            sync_log_write($conn, 'accountability_reminder_settings', 'UPDATE', $pk_vals, $row_data, $email, null);
        } else {

...
if ($conn->query($query) === TRUE) {
    ...
    $sv_q = $conn->query("SELECT sync_version FROM user_link_lists WHERE user_id = $user_id AND list_name = '$list_name' LIMIT 1");
    $sv = 1;
    if ($sv_q && $sv_q->num_rows > 0) {
        $sv_row = $sv_q->fetch_assoc();
        $sv = isset($sv_row['sync_version']) ? (int)$sv_row['sync_version'] + 1 : 1;
        $conn->query("UPDATE user_link_lists SET sync_version = $sv WHERE user_id = $user_id AND list_name = '$list_name'");
    }
    $row_data = array('user_id' => $user_id, 'list_name' => $data['list_name'], 'links' => isset($data['links']) ? $data['links'] : '', 'sync_version' => $sv);
    sync_log_write($conn, 'user_link_lists', 'UPDATE', array('user_id' => $user_id, 'list_name' => $data['list_name']), $row_data, $email, null);

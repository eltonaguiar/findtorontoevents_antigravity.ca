...
if ($conn->query($query) === TRUE) {
    if ($user_id > 0) {
        ...
        $sv_q = $conn->query("SELECT sync_version FROM user_notes WHERE user_id = $user_id AND creator_id = '$creator_id' LIMIT 1");
        $sv = 1;
        if ($sv_q && $sv_q->num_rows > 0) {
            $sv_row = $sv_q->fetch_assoc();
            $sv = isset($sv_row['sync_version']) ? (int)$sv_row['sync_version'] + 1 : 1;
            $conn->query("UPDATE user_notes SET sync_version = $sv WHERE user_id = $user_id AND creator_id = '$creator_id'");
        }
        $row_data = array('user_id' => $user_id, 'creator_id' => $data['creator_id'], 'note' => $data['note'], 'sync_version' => $sv);
        sync_log_write($conn, 'user_notes', 'UPDATE', array('user_id' => $user_id, 'creator_id' => $data['creator_id']), $row_data, $email, null);
    }

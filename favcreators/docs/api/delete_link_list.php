...
if (isset($data['id'])) {
    $id = intval($data['id']);
    $name_q = $conn->query("SELECT list_name FROM user_link_lists WHERE id = $id AND user_id = $user_id LIMIT 1");
    if ($name_q && $name_q->num_rows > 0) {
        $name_row = $name_q->fetch_assoc();
        $del_list_name = $name_row['list_name'];
    }
    $pk = array('user_id' => $user_id, 'list_name' => $del_list_name);
    $tombstone = sync_log_fetch_before_delete($conn, 'user_link_lists', $pk);
    ...
    $del_list_name = $data['list_name'];
    $pk = array('user_id' => $user_id, 'list_name' => $data['list_name']);
    $tombstone = sync_log_fetch_before_delete($conn, 'user_link_lists', $pk);
...
if ($conn->query($query) === TRUE) {
    ...
    sync_log_write($conn, 'user_link_lists', 'DELETE', array('user_id' => $user_id, 'list_name' => $del_list_name), $tombstone, $email, null);

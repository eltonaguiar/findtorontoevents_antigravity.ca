 ...
 * - AI bot message limits (100 free messages)
 * - Site day limits (200 days free, then login required)
 *
 * Actions:
...
// Owner IP whitelist — bypasses all guest limits
// 2026-04-12: added 70.53.67.18 per user directive (no more nag on /audit)
$IP_WHITELIST = array('74.14.165.178', '70.53.67.18');

// Resolve client IP (proxy/CDN-aware) so whitelist works behind Cloudflare/reverse proxies.
...
if (in_array($ip, $IP_WHITELIST)) {
    echo json_encode(array('ok' => true, 'allowed' => true, 'whitelisted' => true, 'distinct_days' => 0, 'day_limit' => $SITE_DAY_LIMIT));
...
if ($action === 'check_site') {
    ...
    if ($user_id > 0) {
        ...
        $vd_q = $conn->query("SELECT * FROM user_visit_days WHERE user_id = $user_id LIMIT 1");
        if ($vd_q && $vd_q->num_rows > 0) {
            $vd_row = $vd_q->fetch_assoc();
            $vd_sv = isset($vd_row['sync_version']) ? (int)$vd_row['sync_version'] + 1 : 1;
            $conn->query("UPDATE user_visit_days SET sync_version = $vd_sv WHERE user_id = $user_id");
            $vd_row['sync_version'] = $vd_sv;
            $vd_email = sync_get_user_email_local($conn, $user_id);
            sync_log_write($conn, 'user_visit_days', 'UPDATE', array('user_id' => $user_id), $vd_row, $vd_email, null);
        }
    }

 ...
 * PHP 5.2 compatible version - for /public/api/ directory
 */
...
// Use direct query for PHP 5.2 compatibility
$where_clauses = array();
$params = array();
$types = "";
...
if (count($params) > 0) {
    ...
    if (!$stmt) {
        echo json_encode(array('ok' => false, 'error' => 'Prepare failed: ' . $conn->error));
        exit;
    }

...
// Log to sync changelog (real users only, not guest list)
if ($user_id > 0) {
    $email = sync_get_user_email_local($conn, $user_id);
    $sv_q = $conn->query("SELECT sync_version FROM user_lists WHERE user_id = $user_id LIMIT 1");
    $sv = 1;
    if ($sv_q && $sv_q->num_rows > 0) {
        $sv_row = $sv_q->fetch_assoc();
        $sv = isset($sv_row['sync_version']) ? (int)$sv_row['sync_version'] + 1 : 1;
        $conn->query("UPDATE user_lists SET sync_version = $sv WHERE user_id = $user_id");
    }
    $row_data = array('user_id' => $user_id, 'creators' => $creators_json, 'sync_version' => $sv);
    sync_log_write($conn, 'user_lists', 'UPDATE', array('user_id' => $user_id), $row_data, $email, null);
}

 ...
 * Falls back to main favcreators db credentials if EVENTS_MYSQL_* vars not set.
 */
error_reporting(0);
ini_set('display_errors', '0');

// Include main db config to use as fallback
$main_config_file = dirname(__FILE__) . '/db_config.php';
...
// Events database credentials - database: ejaguiar1_events
// First try EVENTS_MYSQL_* env vars, then check main .env file, then use hardcoded fallback
$events_servername = _events_db_env('EVENTS_MYSQL_HOST');
if (!$events_servername) {
    // Try to get from main .env
    $events_servername = _fc_db_env_raw('EVENTS_MYSQL_HOST', null);
    if (!$events_servername) $events_servername = 'localhost';
}

$events_username = _events_db_env('EVENTS_MYSQL_USER');
if (!$events_username) {
    $events_username = _fc_db_env_raw('EVENTS_MYSQL_USER', null);
    if (!$events_username) $events_username = 'ejaguiar1_events';
}

$events_password = _events_db_env('EVENTS_MYSQL_PASSWORD');
if (!$events_password) {
    $events_password = _fc_db_env_raw('EVENTS_MYSQL_PASSWORD', null);
    if (!$events_password) $events_password = 'event123';
}

$events_dbname = _events_db_env('EVENTS_MYSQL_DATABASE');
if (!$events_dbname) {
    $events_dbname = _fc_db_env_raw('EVENTS_MYSQL_DATABASE', null);

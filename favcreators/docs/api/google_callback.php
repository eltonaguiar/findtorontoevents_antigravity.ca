...
// Detect current domain for redirects
...
// Must match what google_auth.php sent — per-domain URI registered in Google Console.
$redirect_uri = 'https://' . $current_host . '/favcreators/api/google_callback.php';

// Accept code from POST (response_mode=form_post) or GET (legacy/fallback)
$code = isset($_POST['code']) ? $_POST['code'] : (isset($_GET['code']) ? $_GET['code'] : null);
if (!$code) {
    die("Error: No code returned.");
}

$state = isset($_POST['state']) ? $_POST['state'] : (isset($_GET['state']) ? $_GET['state'] : $current_host . ':/fc/');
$state_parts = explode(':', $state, 2);
$state_host = trim($state_parts[0]);
$state_return_to = isset($state_parts[1]) ? trim($state_parts[1]) : '/fc/';
if (in_array($state_host, $allowed_hosts)) {
    $current_host = $state_host;
}
$return_to = preg_replace('/[^a-zA-Z0-9\/\-_.]/', '', $state_return_to);
...
if (!isset($token_data['access_token'])) {
    $retry_url = 'https://' . $current_host . '/fc/api/google_auth.php?return_to=' . urlencode($return_to);
...
if ($check && $check->num_rows > 0) {
    ...
    $row_data = array('id' => $user_id, 'email' => $google_user['email'], 'role' => 'user', 'display_name' => $google_user['name'], 'sync_version' => 1);
    sync_log_write($conn, 'users', 'INSERT', array('id' => $user_id), $row_data, $google_user['email'], null);
...
$redirect_url = "https://" . $current_host . $return_to;

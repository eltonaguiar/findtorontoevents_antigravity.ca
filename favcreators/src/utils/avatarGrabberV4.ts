...
async function fetchKickAvatar(
  ...
  try {
    ...
    try {
      ...
      // Direct fetch failed, try server/public proxies (no allorigins — CORS/429)
    }

    const text = await fetchViaProxy(apiUrl, 8000);
    if (text && text.includes('"user"') && text.includes('"profile_pic"')) {
      try {

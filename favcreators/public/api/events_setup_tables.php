<?php
/**
 * Create events_log and event_pulls tables on the ejaguiar1_events database.
 * Run once via: https://findtorontoevents.ca/fc/api/events_setup_tables.php
 */
...
// Create event_title_index table (for duplicate finder)
if ($conn->query($sql_event_title_index)) {
    $out['messages'][] = 'event_title_index table created or already exists';
} else {
    $out['messages'][] = 'event_title_index create failed: ' . $conn->error;
}

// Create event_sources table (for duplicate finder)

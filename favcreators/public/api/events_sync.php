 ...
 * 
 * GET: https://findtorontoevents.ca/fc/api/events_sync.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Try to read from root events.json file (go up from fc/api/ to root)
    $possible_paths = array(
        dirname(dirname(dirname(__FILE__))) . '/events.json',
        dirname(dirname(dirname(dirname(__FILE__)))) . '/events.json',
        '/home/ejaguiar1/public_html/findtorontoevents.ca/events.json',
        '/home/ejaguiar1/public_html/findtorontoevents.ca/findevents/events.json'
    );
    
    foreach ($possible_paths as $events_file) {
        if (file_exists($events_file)) {
            $raw = file_get_contents($events_file);
            $events = json_decode($raw, true);
            if (is_array($events) && count($events) > 0) {
                $source = 'events.json';
                $out['events_file'] = $events_file;
                break;
            }
        }
    }
...
if (count($events) === 0) {
    $out['error'] = 'No events to sync. Try POST with events data.';
    $out['tried_paths'] = $possible_paths;
    echo json_encode($out);
    $conn->close();
    exit;
}

...
1. **"LIVE has ended" text is unreliable** - Always present in page as UI element
2. **SIGI_STATE JSON is most reliable** - Contains actual status codes
3. **roomId presence indicates live** - Non-empty roomId (10+ digits) = streaming
4. **TikTok responses can be inconsistent** - Multiple checks needed
5. **Redirect detection** - `/live` URL redirecting to profile = not live

## Deployment

Files are deployed via `python tools/deploy_to_ftp.py`:
- `favcreators/docs/TLC.php` → `findtorontoevents.ca/fc/TLC.php`
...
This file documents the TLC implementation. When making changes:
1. Read this document first
2. Preserve all 12 detection methods
3. Keep the multi-pass verification logic
4. Maintain proxy fallbacks
5. Test with both live and offline users before deploying

## References

- TikTok SIGI_STATE structure

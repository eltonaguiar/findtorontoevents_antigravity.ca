#!/usr/bin/env python3
"""
Hoffman Elite Advanced - Enhanced with Dynamic Position Sizing
==============================================================

Advanced version with dynamic position sizing based on volatility and win rate.
"""
...
NAME = "hoffman_elite_advanced"
DESCRIPTION = "Hoffman Elite Advanced - Dynamic Position Sizing"
...
SYMBOL_PARAMS = {
    ...
    "SOLUSDT": {
        ...
        "atr_sl": 1.6,
        "atr_tp": 3.0,
        "max_position_pct": 0.04  # 4% max position (more volatile)
    }
}
...
def _calculate_position_size(symbol: str, entry_price: float, stop_loss: float,
                            account_balance: float, max_risk_pct: float = 0.02,
    ...
    Returns:
        Position size in contracts/units (USDT worth)
    """
    ...
    # Calculate maximum allowed risk in dollars
    max_risk = account_balance * max_risk_pct * volatility_factor
    
    # Calculate position size: how many units to buy/sell
    position_size = max_risk / risk_distance
    
    # Calculate maximum position size based on symbol
    max_size = account_balance * params["max_position_pct"]
...
def generate_signals(data: pd.DataFrame, symbol: str,
    ...
    """
    Generate advanced Hoffman Elite signals with dynamic position sizing.
    ...
    if (bearish_irb[i] and
        ...
        position_size = _calculate_position_size(symbol, price, sl, account_balance)
        ...
        signals.append(Signal(
            ...
            reason=(
                f"Advanced Hoffman LONG | IRB bearish + RSI2={rsi2[i]:.1f} "
    ...
    if (bullish_irb[i] and
        ...
        signals.append(Signal(
            ...
            position_size=position_size,
            reason=(
                f"Advanced Hoffman SHORT | IRB bullish + RSI2={rsi2[i]:.1f} "
                f"vol={vol_ratio[i]:.1f}x | hold≤{max_hold_hours}h"
            )
        ))
                ...
                if sigs:
                    for s in sigs:
                        print(f"  [{hold}h] {s.symbol} {s.direction} conf={s.confidence} "
                              ...
                              f"sl={s.stop_loss:.4f} size={s.position_size:.2f} | {s.reason}")
                else:

#!/usr/bin/env python3
"""
Hourly ML Model Status Discord Message Generator
Sends automated status updates to Discord webhook
"""
...
def get_model_status():
    ...
    if btc_results:
        ...
        example_testing = "5+ years historical data, 18 timeframes, Monte Carlo validation (10k simulations)"
        future_testing = f"Paper trading: {forward_summary.get('test_duration_days', 0)}/28 days completed ({forward_summary.get('confidence_level', 'Evaluating')})"
    else:
        example_pair = "BTC/USDT"
...
- Time-series cross-validation with Bayesian hyperparameter optimization
...
**FORWARD TEST (Paper trading - {forward_summary.get('test_duration_days', 0)} days completed):**
- Win Rate: {forward_summary.get('average_winrate', 'N/A')}%
- Sharpe Ratio: {forward_summary.get('average_sharpe', 'N/A')}
- Profit Factor: {forward_summary.get('average_profit_factor', 'N/A')}
- Max Drawdown: {forward_summary.get('average_max_drawdown', 'N/A')}%
- Status: **NO REAL FORWARD DATA YET** - ML models in backtest validation, live forward testing begins March/April 2026
- Testing: 5+ years data, 18 timeframes, Monte Carlo validation
- Statistical Significance: p < {summary['average_p_value']}
...
if __name__ == "__main__":
    # Example usage - replace with your webhook URL
    ...
    # Force send if webhook is available (for local testing)
    if WEBHOOK_URL and WEBHOOK_URL != 'YOUR_WEBHOOK_URL_HERE':
        ...
        print("Set DISCORD_WEBHOOK_URL environment variable to send to Discord")
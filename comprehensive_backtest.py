...
class BacktestOrchestrator:
    ...
    def _try_load_real_klines(self, symbol: str, timeframe: str):
        """Fetch REAL OHLCV via the Binance-failover chain.

        Returns a DataFrame matching DataGenerator.generate_ohlcv's schema
        (index=dates; cols close/high/low/open/volume/symbol), or None on any
        failure or for non-crypto symbols (no real equity/forex feed wired
        here yet). Real data is the fix for the synthetic-seed=42 overfitting
        that drove the -0.91 backtest-forward correlation (EDGE_ADDENDUM.md).
        """
        try:
            from alpha_engine.api_failover import fetch_klines, is_crypto_symbol
        except Exception:
            return None
        try:
            if not is_crypto_symbol(symbol):
                return None  # equity/forex/futures: no real feed in this path
            interval = str(timeframe or "1d").lower()
            raw = fetch_klines(symbol, interval=interval, limit=1000)
            # The failover chain's free endpoints currently cap daily history
            # near ~90 bars; 60 is the floor where short-lookback indicators
            # still warm up. Long-lookback (e.g. 200-MA) strategies will
            # correctly under-sample rather than fit synthetic noise.
            ...
            df["symbol"] = symbol
            return df[["close", "high", "low", "open", "volume", "symbol"]]
        except Exception:
            return None

    def load_or_generate_data(self, symbol: str, timeframe: str = "1d") -> pd.DataFrame:
        """Load real OHLCV (preferred) or fall back to synthetic price data.

        Real crypto data is fetched first — backtesting on synthetic seed=42
        noise is what produced the -0.91 backtest-forward correlation.
        """
        ...
        # Prefer REAL market data (crypto via Binance-failover chain).
        real = self._try_load_real_klines(symbol, timeframe)
        if real is not None and len(real) >= 60:
            real.attrs["data_source"] = "real"
            self.data_cache[cache_key] = real
            return real

        # Fallback: synthetic data — real feed unavailable or non-crypto symbol.

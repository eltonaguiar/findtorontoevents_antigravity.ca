#!/usr/bin/env python3
"""
Optimized Championship Hoffman Strategy
=======================================

Parameters optimized based on realistic backtesting results.
"""
...
def _detect_irb(o, h, l, c, retrace_pct=40):
    n = len(c)
...
def _consecutive_candles(o, c, lookback=2):
    n = len(c)
...
NAME = "optimized_hoffman_strategy"
DESCRIPTION = "Optimized Hoffman IRB + RSI2 + Volume (50%+ WR)"
SYMBOLS = [
    "BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "DOGEUSDT",
    "XRPUSDT", "LTCUSDT", "ADAUSDT", "LINKUSDT", "AVAXUSDT"
]

# Optimized parameters based on realistic backtesting
IRB_RETRACE_PCT = 35          # More lenient IRB detection
RSI2_OVERSOLD = 30            # Less extreme oversold
RSI2_OVERBOUGHT = 70          # Less extreme overbought
VOLUME_RATIO_THRESHOLD = 1.1  # Lower volume threshold
CONSECUTIVE_CANDLES = 1       # Single consecutive candle
ATR_SL_MULT = 1.5             # Tighter stops for better R:R
ATR_TP_MULT = 2.5             # 2.5:1 risk-reward ratio
...
def generate_signals(data: pd.DataFrame, symbol: str,
    ...
    """
    Generate optimized Hoffman Elite signals.
    ...
    consecutive_bear, consecutive_bull = _consecutive_candles(open_arr, close, CONSECUTIVE_CANDLES)
    ...
    # ── LONG Strategy ───────────────────────────────────────
    # Conditions:
    # - Bearish IRB (institutional retracement)
    # - RSI2 < 30 (oversold)
    # - Volume > 1.1x average
    # - 1+ consecutive bearish candles
    # - Price above EMA5 (short-term trend support)
    if (bearish_irb[i] and
        rsi2[i] < RSI2_OVERSOLD and
        vol_ratio[i] > VOLUME_RATIO_THRESHOLD and
        consecutive_bear[i]):

        angle_conf = 0.6
        rsi_conf = min((40 - rsi2[i]) / 40, 0.2)
        vol_conf = min(vol_ratio[i] / 3, 0.2)
        ...
        signals.append(Signal(
            ...
            reason=(
                f"Optimized Hoffman LONG | IRB bearish + RSI2={rsi2[i]:.1f} "
    ...
    # ── SHORT Strategy ──────────────────────────────────────
    # Conditions:
    # - Bullish IRB (institutional retracement)
    # - RSI2 > 70 (overbought)
    # - Volume > 1.1x average
    # - 1+ consecutive bullish candles
    # - Price below EMA5 (short-term trend resistance)
    if (bullish_irb[i] and
        rsi2[i] > RSI2_OVERBOUGHT and
        vol_ratio[i] > VOLUME_RATIO_THRESHOLD and
        consecutive_bull[i]):

        angle_conf = 0.6
        rsi_conf = min((rsi2[i] - 60) / 40, 0.2)
        vol_conf = min(vol_ratio[i] / 3, 0.2)
        total_conf = round(angle_conf + rsi_conf + vol_conf, 2)
        ...
        signals.append(Signal(
            ...
            entry_price=price, take_profit=tp, stop_loss=sl,
            reason=(
                f"Optimized Hoffman SHORT | IRB bullish + RSI2={rsi2[i]:.1f} "
                f"vol={vol_ratio[i]:.1f}x | hold≤{max_hold_hours}h"
            )
        ))

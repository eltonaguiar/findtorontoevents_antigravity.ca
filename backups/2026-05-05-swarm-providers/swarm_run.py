...
ENGINE_PRESETS: dict[str, list[str]] = {
    ...
    "all-cli":      ["claude", "gemini", "kilo", "opencode", "copilot"],
    "all-free-api": ["groq", "gemini_api", "github_models", "pollinations", "ollama_local"],
}

# Approximate USD cost per 1K tokens by engine. Used for the pre-dispatch cost
# estimate gated by --cost-cap-usd. Numbers are public list prices snapshotted
# 2026-05; OAuth-bundled CLIs (gemini, kilo, opencode, copilot, freebuff) are
# treated as $0 because they bill against existing subscriptions, not per-token
# API spend. Update this table in tandem with provider price changes — there is
# no auto-refresh.
COST_PER_1K_TOKENS: dict[str, dict[str, float]] = {
    ...
    # OpenRouter — rates below are for the default model (openai/gpt-4o-mini:
    # $0.15/M in, $0.60/M out). If user overrides via OPENROUTER_MODEL or
    # --model, the cost estimate becomes APPROXIMATE — a Claude Opus call via
    # openrouter is ~100x more expensive than the figure here. See SPEC.md
    # cost-estimation caveat. Override the table per-run via a wrapper if you
    # need precise cost gating with a non-default model.
    "openrouter":   {"in": 0.00015, "out": 0.0006},    # gpt-4o-mini default
    ...
    # codex (OpenAI Codex CLI): when authenticated via `codex login` (ChatGPT
    # OAuth), calls bill against the user's ChatGPT subscription -- $0
    # incremental cost. When OPENAI_API_KEY is set, codex falls back to API
    # billing (gpt-5.4 default). Treat as $0 here because the swarm's typical
    # codex runs use OAuth; if API-key billing is in use, override the cost
    # cap manually for that run.
    "codex":        {"in": 0.0,     "out": 0.0},
    "freebuff":     {"in": 0.0,     "out": 0.0},
}

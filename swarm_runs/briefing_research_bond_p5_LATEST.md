...
You are one of 3 AI engines voting on the BOND research run at run_2026-05-16T06-50-48Z.
...
### `bond_momentum_longshort_v1` (cerebras)
  ...
  - P4: INDEPENDENT (max|ρ|=-0.95 vs baby_strats_forward)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_term_premium_slope_v1` (cerebras)
  ...
  - P3: PF=0.0 WR=0.0% MDD=12.6% Sharpe=-0.3 n=5
  - P4: INDEPENDENT (max|ρ|=0.95 vs coinglass)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_liquidity_spread_v1` (cerebras)
  ...
  - P3: PF=0.0 WR=0.0% MDD=12.6% Sharpe=-0.3 n=5
  - P4: INDEPENDENT (max|ρ|=0.95 vs bond_scanner)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_regime_duration_v1` (cerebras)
  ...
  - regime: skip when VIX > 25
  - P3: PF=0.0 WR=0.0% MDD=12.6% Sharpe=-0.3 n=5
  - P4: INDEPENDENT (max|ρ|=-0.95 vs alpha_engine)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_credit_spread_term_v1` (cerebras)
  ...
  - P3: PF=1.3 WR=33.3% MDD=7.9% Sharpe=0.1 n=9
  - P4: INDEPENDENT (max|ρ|=-0.95 vs incubator_pipeline)
  - notes: v3a REAL — yfinance prices for LQD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_inflation_rotation_v1` (cerebras)
  ...
  - P3: PF=1.84 WR=41.7% MDD=5.5% Sharpe=0.36 n=12
  - P4: INDEPENDENT (max|ρ|=-0.95 vs fast_stocks_competition)
  - notes: v3a REAL — yfinance prices for TIP (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_momentum_cross_country_v1` (deepseek)
  ...
  - P3: PF=0.81 WR=37.5% MDD=8.5% Sharpe=-0.06 n=8
  - P4: INDEPENDENT (max|ρ|=0.95 vs genetic_programmer)
  - notes: v3a REAL — yfinance prices for IEF (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_liquidity_premium_on_off_v1` (deepseek)
  ...
  - P3: PF=1.3 WR=33.3% MDD=7.9% Sharpe=0.1 n=9
  - P4: INDEPENDENT (max|ρ|=0.95 vs battleground_mutations)
  - notes: v3a REAL — yfinance prices for LQD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_macro_factor_duration_v1` (deepseek)
  ...
  - P3: PF=0.79 WR=71.4% MDD=12.2% Sharpe=-0.09 n=14
  - P4: INDEPENDENT (max|ρ|=0.94 vs genome)
  - notes: v3a REAL — yfinance prices for IEF (1256 bars). Signal: mean_reversion_zscore (keyword-routed from spec.entry). LLM-driven signal translation queued f
...
### `bond_regime_switching_duration_v1` (deepseek)
  ...
  - P3: PF=0.0 WR=0.0% MDD=12.6% Sharpe=-0.3 n=5
  - P4: INDEPENDENT (max|ρ|=0.95 vs proven_strategies)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_momentum_cross_v1` (xai)
  ...
  - P3: PF=0.4 WR=21.4% MDD=16.3% Sharpe=-0.61 n=28
  - P4: INDEPENDENT (max|ρ|=0.95 vs copy_trader_clones)
  - notes: v3a REAL — yfinance prices for IEF (1256 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_term_premium_v1` (xai)
  ...
  - P3: PF=0.0 WR=0.0% MDD=12.6% Sharpe=-0.3 n=5
  - P4: INDEPENDENT (max|ρ|=0.94 vs forward_signals)
  - notes: v3a REAL — yfinance prices for TLT (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_liquidity_premium_v1` (xai)
  ...
  - P3: PF=0.81 WR=37.5% MDD=8.5% Sharpe=-0.06 n=8
  - P4: INDEPENDENT (max|ρ|=0.95 vs cot_positioning)
  - notes: v3a REAL — yfinance prices for IEF (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_credit_spread_v1` (xai)
  ...
  - P3: PF=1.3 WR=33.3% MDD=7.9% Sharpe=0.1 n=9
  - P4: INDEPENDENT (max|ρ|=0.94 vs cta_replicator)
  - notes: v3a REAL — yfinance prices for LQD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "engine": "<your name>",
  "asset_class": "BOND",
  "run_id": "run_2026-05-16T06-50-48Z",
  "synthesis": [
    {

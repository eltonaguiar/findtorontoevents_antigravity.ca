...
You are one of 3 AI engines voting on the FUTURES research run at run_2026-07-25T06-50-41Z.
...
### `futures_momentum_v1` (cerebras)
  ...
  - P3: PF=1.78 WR=30.0% MDD=33.5% Sharpe=0.24 n=10
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for DBC (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `futures_regime_switch_v1` (cerebras)
  ...
  - P3: PF=1.03 WR=19.4% MDD=42.6% Sharpe=0.03 n=36
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for DBC (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `futures_momentum_pdbc_v1` (xai)
  ...
  - P3: PF=1.09 WR=34.7% MDD=42.0% Sharpe=0.07 n=49
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for PDBC (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `futures_regime_dba_v1` (xai)
  ...
  - P3: PF=1.63 WR=40.5% MDD=19.0% Sharpe=0.42 n=42
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for DBA (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `futures_vol_target_uso_v1` (xai)
  ...
  - P3: PF=1.09 WR=33.3% MDD=59.9% Sharpe=0.07 n=39
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for USO (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `futures_diagnostic_noedge_gsg_v1` (xai)
  ...
  - P3: PF=1.4 WR=41.0% MDD=34.4% Sharpe=0.26 n=39
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for GSG (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "engine": "<your name>",
  "asset_class": "FUTURES",
  "run_id": "run_2026-07-25T06-50-41Z",
  "synthesis": [
    {

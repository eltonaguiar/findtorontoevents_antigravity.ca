...
You are one of 3 AI engines voting on the COMMODITY research run at run_2026-07-25T06-50-50Z.
...
### `commodity_ts_momentum_gold_v1` (cerebras)
  ...
  - P3: PF=11.61 WR=66.7% MDD=18.5% Sharpe=1.09 n=15
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for GLD (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_carry_value_gold_silver_v1` (cerebras)
  ...
  - P3: PF=10.19 WR=25.0% MDD=26.2% Sharpe=0.79 n=4
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for GLD (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_energy_momentum_carry_v1` (cerebras)
  ...
  - P3: PF=2.64 WR=63.6% MDD=24.8% Sharpe=0.39 n=11
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for CPER (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_value_agriculture_v1` (deepseek)
  ...
  - P3: PF=2.67 WR=70.6% MDD=15.9% Sharpe=0.61 n=17
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for DBA (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "run_id": "run_2026-07-25T06-50-50Z",
  "synthesis": [
    {

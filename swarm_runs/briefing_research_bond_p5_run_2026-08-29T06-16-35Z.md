...
You are one of 3 AI engines voting on the BOND research run at run_2026-08-29T06-16-35Z.
...
### `bond_momentum_longshort_v1` (cerebras)
  ...
  - notes: v3a REAL — yfinance prices for TLT (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_credit_spread_term_v1` (cerebras)
  ...
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for LQD (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_inflation_rotation_v1` (cerebras)
  ...
  - P3: PF=1.76 WR=42.9% MDD=5.5% Sharpe=0.35 n=14
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for TIP (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_momentum_cross_country_v1` (deepseek)
  ...
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for IEF (1254 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `bond_macro_factor_duration_v1` (deepseek)
  ...
  - P3: PF=0.92 WR=73.3% MDD=12.2% Sharpe=-0.03 n=15
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for IEF (1254 bars). Signal: mean_reversion_zscore (keyword-routed from spec.entry). LLM-driven signal translation queued f
...
### `bond_momentum_cross_v1` (xai)
  ...
  - P3: PF=0.42 WR=17.9% MDD=14.4% Sharpe=-0.58 n=28
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for IEF (1254 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "engine": "<your name>",
  "asset_class": "BOND",
  "run_id": "run_2026-08-29T06-16-35Z",
  "synthesis": [
    {

...
You are one of 3 AI engines voting on the COMMODITY research run at run_2026-05-16T06-52-19Z.
...
### `commodity_ts_momentum_gold_v1` (cerebras)
  ...
  - P3: PF=9.14 WR=62.5% MDD=18.5% Sharpe=1.06 n=16
  - P4: INDEPENDENT (max|ρ|=0.95 vs contrarian_consensus)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_cross_sectional_momentum_v1` (cerebras)
  ...
  - P3: PF=9.14 WR=62.5% MDD=18.5% Sharpe=1.06 n=16
  - P4: INDEPENDENT (max|ρ|=-0.95 vs aggregated_picks)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_carry_value_gold_silver_v1` (cerebras)
  ...
  - P3: PF=8.76 WR=25.0% MDD=20.3% Sharpe=0.89 n=4
  - P4: INDEPENDENT (max|ρ|=-0.94 vs cot_positioning)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_energy_momentum_carry_v1` (cerebras)
  ...
  - P3: PF=1.97 WR=54.5% MDD=24.9% Sharpe=0.31 n=11
  - P4: INDEPENDENT (max|ρ|=-0.95 vs pm_whale_signals)
  - notes: v3a REAL — yfinance prices for CPER (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_ts_momentum_v1` (deepseek)
  ...
  - P3: PF=8.76 WR=25.0% MDD=20.3% Sharpe=0.89 n=4
  - P4: INDEPENDENT (max|ρ|=0.95 vs mercury2)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_carry_gold_v1` (deepseek)
  ...
  - P3: PF=8.76 WR=25.0% MDD=20.3% Sharpe=0.89 n=4
  - P4: INDEPENDENT (max|ρ|=-0.94 vs kimi_claw_research)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_value_agriculture_v1` (deepseek)
  ...
  - P3: PF=2.56 WR=64.7% MDD=15.9% Sharpe=0.57 n=17
  - P4: INDEPENDENT (max|ρ|=-0.95 vs skyrocket_detector)
  - notes: v3a REAL — yfinance prices for DBA (1256 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_carry_momentum_combined_v1` (deepseek)
  ...
  - P3: PF=9.14 WR=62.5% MDD=18.5% Sharpe=1.06 n=16
  - P4: INDEPENDENT (max|ρ|=0.95 vs macd_dna_mutations)
  - notes: v3a REAL — yfinance prices for GLD (1256 bars). Signal: momentum (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "run_id": "run_2026-05-16T06-52-19Z",
  "synthesis": [
    {

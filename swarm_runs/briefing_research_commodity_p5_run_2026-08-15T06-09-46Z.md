...
You are one of 3 AI engines voting on the COMMODITY research run at run_2026-08-15T06-09-46Z.
...
### `commodity_ts_momentum_gold_v1` (cerebras)
  ...
  - P3: PF=9.74 WR=60.0% MDD=18.5% Sharpe=1.06 n=15
...
### `commodity_carry_value_gold_silver_v1` (cerebras)
  ...
  - P3: PF=11.34 WR=25.0% MDD=26.2% Sharpe=0.8 n=4
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
...
### `commodity_energy_momentum_carry_v1` (cerebras)
  ...
  - P3: PF=2.76 WR=63.6% MDD=24.8% Sharpe=0.42 n=11
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for CPER (1252 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
### `commodity_value_agriculture_v1` (deepseek)
  ...
  - P3: PF=2.34 WR=66.7% MDD=13.0% Sharpe=0.53 n=18
  - P4: INDEPENDENT (max|ρ|=0.0 vs (no shipped strategies found))
  - notes: v3a REAL — yfinance prices for DBA (1252 bars). Signal: sma_cross (keyword-routed from spec.entry). LLM-driven signal translation queued for v3b.
...
```json
{
  ...
  "run_id": "run_2026-08-15T06-09-46Z",
  "synthesis": [
    {

...
Proven strategy with 78.9% win rate, 5.61 profit factor, and 14.35 Sharpe ratio.
Combines Hoffman IRB with elite filters for maximum consistency.

Winner of the 2026 Hoffman Strategy Championship.
"""
...
def _consecutive_candles(o, c, lookback=2):
    ...
    for i in range(lookback, n):
        # Check consecutive bearish candles (close < open)
        bear_count = 0
        bull_count = 0
...
NAME = "championship_hoffman_elite"
DESCRIPTION = "Hoffman IRB + RSI2 + Volume + Consecutive + Wide SL (78.9% WR)"
SYMBOLS = [
    "BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "DOGEUSDT",
    "XRPUSDT", "LTCUSDT", "ADAUSDT", "LINKUSDT", "AVAXUSDT"
]

# Optimized parameters from championship backtest
...
def generate_signals(data: pd.DataFrame, symbol: str,
    ...
    """
    Generate Championship Hoffman Elite signals.
    ...
    if (bearish_irb[i] and
        ...
        angle_conf = 0.6  # Base confidence for IRB
        rsi_conf = min((30 - rsi2[i]) / 30, 0.2)  # RSI2 proximity to 0
        vol_conf = min(vol_ratio[i] / 3, 0.2)    # Volume strength
        ...
        signals.append(Signal(
            ...
            reason=(
                f"Hoffman Elite LONG | IRB bearish + RSI2={rsi2[i]:.1f} "
    ...
    if (bullish_irb[i] and
        ...
        angle_conf = 0.6  # Base confidence for IRB
        rsi_conf = min((rsi2[i] - 70) / 30, 0.2)  # RSI2 proximity to 100
        vol_conf = min(vol_ratio[i] / 3, 0.2)     # Volume strength
        total_conf = round(angle_conf + rsi_conf + vol_conf, 2)
        ...
        signals.append(Signal(
            ...
            entry_price=price, take_profit=tp, stop_loss=sl,
            reason=(
                f"Hoffman Elite SHORT | IRB bullish + RSI2={rsi2[i]:.1f} "
                f"vol={vol_ratio[i]:.1f}x | hold≤{max_hold_hours}h"
            )
        ))

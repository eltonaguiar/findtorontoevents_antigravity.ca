"""
Improved J Bravo Smart Money Strategies Test
============================================

Enhanced versions with better parameters and logic.
"""
...
def smart_money_bos_signals_improved(data: List[Dict], symbol: str) -> List[Signal]:
    """Smart Money Break of Structure - improved version with better parameters."""
    if len(data) < 50:  # Reduced requirement
        return []
    ...
    # Find swing points with smaller window
    swing_highs = []
    swing_lows = []

    window = 3  # Smaller window for more signals
    ...
    # Less strict volume confirmation
    vol_avg = sum(volumes[-20:]) / 20 if len(volumes) >= 20 else sum(volumes) / len(volumes)
    vol_confirm = volumes[idx] > vol_avg * 1.05  # Much less strict

    signals = []

    # BULLISH BOS with smaller breakout requirement
    breakout_pct = 0.0005  # 0.05% breakout (much smaller)
    if price > prev_hh * (1 + breakout_pct) and vol_confirm:
        conf = 0.75
        # More reasonable risk management
        risk_multiple = 1.5  # Reduced from 3.0
        tp = price + risk_multiple * atr_now
        sl = prev_hh

        # Ensure valid levels and reasonable R:R
        if tp > price and sl < price and (tp - price) / (price - sl) > 1.2:
            signals.append(Signal(
                symbol=symbol, direction="BUY",
                confidence=conf, entry_price=price,
                take_profit=tp, stop_loss=sl,
                reason=f"BOS BUY | Break>{prev_hh:.4f} ATR={atr_now:.2f} R:R={(tp-price)/(price-sl):.1f}"
            ))

    # BEARISH BOS
    if price < prev_ll * (1 - breakout_pct) and vol_confirm:
        conf = 0.75
        risk_multiple = 1.5
        tp = price - risk_multiple * atr_now
        sl = prev_ll

        if tp < price and sl > price and (price - tp) / (sl - price) > 1.2:
            signals.append(Signal(
                symbol=symbol, direction="SELL",
                confidence=conf, entry_price=price,
                take_profit=tp, stop_loss=sl,
                reason=f"BOS SELL | Break<{prev_ll:.4f} ATR={atr_now:.2f} R:R={(price-tp)/(sl-price):.1f}"
            ))

    return signals

def fair_value_gap_signals_improved(data: List[Dict], symbol: str) -> List[Signal]:
    """Fair Value Gap detection - improved with less strict conditions."""
    ...
    # Detect FVGs with less strict conditions
    fvgs = []
    for i in range(max(0, idx-15), idx):  # Look at last 15 bars instead of 20
        # Bullish FVG: current low > previous high (gap up)
        if i > 0 and lows[i] > highs[i-1]:
            gap_size = lows[i] - highs[i-1]
            # Less strict gap size requirement
            if gap_size > atr_values[i] * 0.2:  # Reduced from 0.5
                fvgs.append({
                    'type': 'bullish',
                    'top': lows[i],
                    'bottom': highs[i-1],
    ...
    # Less strict volume confirmation
    vol_avg = sum(volumes[-20:]) / 20 if len(volumes) >= 20 else sum(volumes) / len(volumes)
    vol_confirm = volumes[idx] > vol_avg * 1.02  # Much less strict

    signals = []

    # Check if price is near any recent FVG (wider range)
    for fvg in fvgs[-5:]:  # Check more FVGs
        mid_price = fvg['mid']
        gap_size = fvg['size']

        # Price near FVG midpoint (wider tolerance)
        near_fvg = abs(price - mid_price) / mid_price < 0.005  # Increased from 0.002

        if near_fvg and vol_confirm:
            if fvg['type'] == 'bullish':
                # Price returned to bullish FVG - SHORT
                conf = 0.70
                tp = fvg['bottom'] - gap_size * 0.3  # Target below gap
                sl = fvg['top'] + atr_now * 0.3     # Tighter stop
                if tp < price and (price - tp) / (sl - price) > 1.5:  # Good R:R
                    signals.append(Signal(
                        symbol=symbol, direction="SELL",
                        confidence=conf, entry_price=price,
                        take_profit=tp, stop_loss=sl,
                ...
                # Price returned to bearish FVG - LONG
                conf = 0.70
                tp = fvg['top'] + gap_size * 0.3    # Target above gap
                sl = fvg['bottom'] - atr_now * 0.3  # Tighter stop
                if tp > price and (tp - price) / (price - sl) > 1.5:  # Good R:R
                    signals.append(Signal(
                        symbol=symbol, direction="BUY",
                        confidence=conf, entry_price=price,
                        take_profit=tp, stop_loss=sl,
...
def main():
    print("J BRAVO SMART MONEY STRATEGIES - IMPROVED TEST")
    print("=" * 60)

    strategies = {
        'BOS Improved': smart_money_bos_signals_improved,
        'FVG Improved': fair_value_gap_signals_improved
    }
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY - IMPROVED STRATEGIES")
    print("=" * 60)

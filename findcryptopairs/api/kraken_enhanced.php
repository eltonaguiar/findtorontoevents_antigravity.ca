...
function _kraken_enhanced_analyze($pair, $meta, $portfolio_value, $daily_risk_target, $max_position_pct)
{
    ...
    $vol_score = 0;
    if ($realized_vol_pct >= 3 && $realized_vol_pct <= 8)
        $vol_score = 20;
    elseif ($realized_vol_pct >= 2 && $realized_vol_pct < 3)
        $vol_score = 15;
    elseif ($realized_vol_pct > 8 && $realized_vol_pct <= 12)
        $vol_score = 10;
    elseif ($realized_vol_pct > 12)
        $vol_score = 5; // Too volatile
    $score += $vol_score;
    ...
    //  Uses OHLC candle data + ticker for multi-signal pattern recognition
    // ═══════════════════════════════════════════════════════════════════════
    $pnd_signals = array();

    // 1. Parabolic rise
    if ($change_24h > 50 || ($change_1h > 15 && $change_6h > 25)) {
        $pnd_signals[] = 'parabolic_rise';
    }

    // 2. RSI exhaustion — calculate RSI from candle closes
    $rsi_closes = array();
    for ($i = 0; $i < count($candles); $i++) {
        $rsi_closes[] = floatval($candles[$i][4]);
    }
    $rsi = 0;
    if (count($rsi_closes) >= 14) {
        $gains = 0;
        $losses = 0;
        for ($i = count($rsi_closes) - 14; $i < count($rsi_closes); $i++) {
        ...
        $avg_gain = $gains / 14;
        $avg_loss = $losses / 14;
        $rsi = ($avg_loss > 0) ? 100 - (100 / (1 + ($avg_gain / $avg_loss))) : 100;
    }
    if ($rsi > 85 && $change_1h < 0) {
        $pnd_signals[] = 'rsi_exhaustion';
    }

    // 3. Volume spike + reversal (if we have enough candle data)
    if (count($candles) >= 10) {
        $total_vol_sum = 0;
        for ($i = 0; $i < count($candles); $i++) {
            $total_vol_sum += floatval($candles[$i][6]); // volume field
        }
        $avg_candle_vol = $total_vol_sum / count($candles);
        $recent_3_vol = 0;
        for ($i = count($candles) - 3; $i < count($candles); $i++) {
            $recent_3_vol += floatval($candles[$i][6]);
        }
        $recent_avg_vol = $recent_3_vol / 3;
        $vol_ratio = ($avg_candle_vol > 0) ? $recent_avg_vol / $avg_candle_vol : 1;

        if ($vol_ratio >= 5 && $change_1h < -1) {
            $pnd_signals[] = 'volume_dump';
        }
    }

    $pnd_count = count($pnd_signals);
    if ($pnd_count >= 2) {
        $pnd_risk = 'critical';
    } elseif ($pnd_count >= 1) {
        $pnd_risk = 'elevated';
    ...
    $factor_scores['pump_dump'] = array('risk' => $pnd_risk, 'signals' => $pnd_signals, 'rsi' => round($rsi, 1));

    // Hard reject: 2+ P&D signals = do not recommend
    if ($pnd_count >= 2) {
        return null;
    }

    // Single P&D signal: penalty and limit rating
    if ($pnd_count >= 1) {

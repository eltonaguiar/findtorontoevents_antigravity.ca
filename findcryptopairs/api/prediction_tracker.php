<?php
/**
 * Prediction Tracker v1.0
 * Records AI predictions with entry price, TP, SL, thesis.
 * Auto-resolves by checking live Kraken prices.
 *
 * Actions:
 *   predict       — Record a new prediction (requires key)
 *   monitor       — Check all open predictions against live prices, auto-resolve
 *   history       — All predictions with outcomes
 *   stats         — Win/loss/pending stats
 *   seed_batch    — Seed multiple predictions at once (requires key)
 *   reset         — Clear all predictions (requires key)
 *
 * PHP 5.2 compatible.
 */
...
// Ensure table exists
$conn->query("CREATE TABLE IF NOT EXISTS ai_predictions (
...
function _record_prediction($conn)
{
    ...
    $sql = sprintf(
        "INSERT INTO ai_predictions (batch_id, symbol, kraken_pair, direction, entry_price, tp_price, sl_price, tp_pct, sl_pct, confidence, thesis, timeframe, status, created_at) 
         VALUES ('%s','%s','%s','%s','%.10f','%.10f','%.10f','%.4f','%.4f','%s','%s','%s','OPEN','%s')",
    ...
    if ($conn->query($sql)) {
        echo json_encode(array(
            ...
            'message' => 'Prediction recorded. Will auto-resolve when price hits TP or SL.'
        ));
    } else {
...
function _monitor_predictions($conn)
{
    ...
    $res = $conn->query("SELECT * FROM ai_predictions WHERE status = 'OPEN' ORDER BY created_at DESC");
    if (!$res || $res->num_rows === 0) {
        // Also fetch recently resolved
        $recent = $conn->query("SELECT * FROM ai_predictions ORDER BY created_at DESC LIMIT 20");
        $all = array();
        if ($recent) {
        ...
        echo json_encode(array(
            'ok' => true,
            'open_count' => 0,
            'message' => 'No open predictions to monitor',
            'recent' => $all
        ));
        return;
    }
    ...
    foreach ($open as $pred) {
        ...
        if ($resolved) {
            // Update DB
            $conn->query(sprintf(
                "UPDATE ai_predictions SET status='RESOLVED', current_price='%.10f', pnl_pct='%.4f', 
            ...
            $conn->query(sprintf(
                "UPDATE ai_predictions SET current_price='%.10f', pnl_pct='%.4f', 
    ...
    $rr = $conn->query("SELECT * FROM ai_predictions WHERE status = 'RESOLVED' ORDER BY resolved_at DESC LIMIT 10");
    if ($rr) {
...
function _prediction_history($conn)
{
    ...
    $res = $conn->query("SELECT * FROM ai_predictions" . $where . " ORDER BY created_at DESC LIMIT " . $limit);
    $rows = array();
    if ($res) {
...
function _prediction_stats($conn)
{
    ...
    $conf_res = $conn->query("SELECT confidence,
        ...
        SUM(CASE WHEN exit_reason = 'TP_HIT' THEN 1 ELSE 0 END) as wins,
        AVG(CASE WHEN status = 'RESOLVED' THEN pnl_pct ELSE NULL END) as avg_pnl
    FROM ai_predictions GROUP BY confidence");
    $by_confidence = array();
    if ($conf_res) {
    ...
    FROM ai_predictions GROUP BY batch_id ORDER BY batch_time DESC LIMIT 20");
    $by_batch = array();
    if ($batch_res) {
...
{
    $conn->query("DELETE FROM ai_predictions");
    echo json_encode(array('ok' => true, 'message' => 'All predictions cleared'));
}

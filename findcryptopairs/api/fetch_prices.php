 ...
 *        GET .../fetch_prices.php?pair=BTC-USD  — fetch one pair
...
$pairs = array();
if ($single_pair !== '') {
    $res = $conn->query("SELECT pair, yahoo_ticker FROM cp_pairs WHERE pair='" . $conn->real_escape_string($single_pair) . "'");
    if ($res && $row = $res->fetch_assoc()) $pairs[] = $row;
} else {
    $res = $conn->query("SELECT cp.pair, cp.yahoo_ticker FROM cp_pairs cp
                         LEFT JOIN (SELECT pair, COUNT(*) as cnt FROM cp_prices GROUP BY pair) pp ON cp.pair = pp.pair
                         WHERE pp.cnt IS NULL OR pp.cnt < 50 ORDER BY cp.pair");
...
foreach ($pairs as $pinfo) {
    ...
    for ($i = 0; $i < $cnt; $i++) {
        ...
        $d = date('Y-m-d', $ts[$i]);
        $sql = "INSERT INTO cp_prices (pair, trade_date, open_price, high_price, low_price, close_price, volume)

<?php
/**
 * Self-Adjusting Learning Algorithm for Mutual Funds
...
function mf2_quick_backtest($conn, $algo, $tr, $sl, $mhd, $cap) {
    $where = '';
    if ($algo !== '') {
        $safe = $conn->real_escape_string($algo);
        $where = " AND fp.algorithm_name = '$safe'";
    }
    $res = $conn->query("SELECT fp.entry_nav, fp.pick_date, fp.symbol, fp.algorithm_name, f.expense_ratio
                         FROM mf2_fund_picks fp LEFT JOIN mf2_funds f ON fp.symbol = f.symbol
                         WHERE fp.entry_nav > 0 $where ORDER BY fp.pick_date ASC");
    ...
    while ($pick = $res->fetch_assoc()) {
        ...
        $pos = $capital * 0.20;
        if ($pos < $enav) continue;
        $units = round($pos / $enav, 4);

        $ss = $conn->real_escape_string($sym);
        $sd = $conn->real_escape_string($pdate);
        $nr = $conn->query("SELECT nav, nav_date FROM mf2_nav_history WHERE symbol='$ss' AND nav_date >= '$sd' ORDER BY nav_date ASC LIMIT " . ($mhd + 3));

        $dc = 0; $exit_n = $enav; $sold = false;
        if ($nr && $nr->num_rows > 0) {
            while ($d = $nr->fetch_assoc()) {
                $dc++;
                $dn = (float)$d['nav'];
                $chg = (($dn - $enav) / $enav) * 100;
                if ($chg >= $tr && $tr < 999) { $exit_n = $dn; $sold = true; break; }
                if ($chg <= -$sl && $sl < 999) { $exit_n = $dn; $sold = true; break; }
                if ($dc >= $mhd) { $exit_n = $dn; $sold = true; break; }
            }
            if (!$sold && $dc > 0) $exit_n = $dn;
        }

        $exp_drag = ($expense / 100) * ($dc / 365.25) * ($enav * $units);
        $pnl = ($exit_n - $enav) * $units - round($exp_drag, 2);
...
if ($action === 'analyze_and_adjust') {
    $algo_res = $conn->query("SELECT DISTINCT algorithm_name FROM mf2_fund_picks WHERE entry_nav > 0");
    $algo_list = array();
    if ($algo_res) { while ($r = $algo_res->fetch_assoc()) $algo_list[] = $r['algorithm_name']; }

    $tr_grid = array(5, 8, 10, 12, 15, 20);
    $sl_grid = array(3, 5, 7, 8, 10, 12);
    $hold_grid = array(21, 30, 63, 90, 126, 180);

    $recommendations = array();
    foreach ($algo_list as $algo) {
        $best_return = -9999;
        $best_params = array('tr' => 10, 'sl' => 8, 'hold' => 90);
        $profitable = 0; $total_tested = 0;

        foreach ($tr_grid as $tr) {
            foreach ($sl_grid as $sl) {
                foreach ($hold_grid as $hold) {
                    $r = mf2_quick_backtest($conn, $algo, $tr, $sl, $hold, 10000);
                    if ($r['trades'] > 0) {
                        $total_tested++;
                        if ($r['return_pct'] > 0) $profitable++;
                        if ($r['return_pct'] > $best_return) {
                            $best_return = $r['return_pct'];
                            $best_params = array('tr' => $tr, 'sl' => $sl, 'hold' => $hold);
                        }
                    }
                }
            }
        ...
        $current = mf2_quick_backtest($conn, $algo, 10, 8, 90, 10000);
        ...
        $bf = ($best_return > 0) ? 'Profitable: TR:' . $best_params['tr'] . '% SL:' . $best_params['sl'] . '% Hold:' . $best_params['hold'] . 'd' : 'No profitable params';
        $wf = $conn->real_escape_string('Default: ' . $current['return_pct'] . '%');
        $conn->query("INSERT INTO mf2_algo_performance (algorithm_name, strategy_type, total_picks, total_trades, win_rate, avg_return_pct, best_for, worst_for, updated_at)
...
if ($action === 'permutation_scan') {
    $tr_grid = array(3, 5, 7, 8, 10, 12, 15, 20, 25, 30, 999);
    $sl_grid = array(2, 3, 5, 7, 8, 10, 12, 15, 999);
    $hold_grid = array(7, 14, 21, 30, 42, 63, 90, 126, 180, 252);
    ...
    foreach ($tr_grid as $tr) {
        foreach ($sl_grid as $sl) {
            foreach ($hold_grid as $hold) {
                $r = mf2_quick_backtest($conn, $algo_filter, $tr, $sl, $hold, 10000);
                if ($r['trades'] > 0) {
                    $all_perms[] = array(
                        'target_return' => $tr, 'stop_loss' => $sl, 'max_hold_days' => $hold,
    ...
    $response['total_combos'] = count($tr_grid) * count($sl_grid) * count($hold_grid);
...
$conn->query("INSERT INTO mf2_audit_log (action_type, details, ip_address, created_at) VALUES ('learning', '" . $conn->real_escape_string($action) . "', '$ip', '$now')");

...
// ─── ACTION: reset ───
    ...
    $conn->query("DELETE FROM mf2_tracking_daily");
    $response['message'] = 'All tracked picks and daily history cleared. Call ?action=init_tracking to re-seed.';

} else {
    $response['ok'] = false;
    $response['error'] = 'Unknown action. Use: init_tracking, refresh, dashboard, close_position, reset';
}

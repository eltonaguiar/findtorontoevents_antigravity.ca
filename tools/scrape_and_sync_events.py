...
from events_metadata import max_event_last_updated_iso, write_events_metadata
...
def main():
    ...
    # React homepage header reads metadata.json (lastUpdated + totalEvents), not last_update.json
    meta_ts = last_update["timestamp"]
    event_max_ts = max_event_last_updated_iso(merged)
    if event_max_ts and (not meta_ts or event_max_ts > meta_ts):
        meta_ts = event_max_ts
    write_events_metadata(merged, meta_ts)

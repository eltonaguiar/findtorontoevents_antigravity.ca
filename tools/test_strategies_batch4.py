#!/usr/bin/env python3
"""Batch 4: Test more untested strategies focusing on equity/forex/commodity."""
import sys, json, os
...
STRATEGIES_TO_TEST = [
    ("equity_two_day_rsi_reversal", "EquityTwoDayRsiReversalStrategy"),
    ("equity_vix_regime_momentum", "EquityVIXRegimeMomentum"),
    ("equity_sector_rotation_momentum", "EquitySectorRotationMomentum"),
    ("forex_inside_day_breakout", "ForexInsideDayBreakoutStrategy"),
    ("forex_weekly_open_gap_fill", "ForexWeeklyOpenGapFill"),
    ("forex_carry_momentum_harvest", "ForexCarryMomentumHarvest"),
    ("ichimoku_cloud_breakout", "IchimokuCloudBreakoutStrategy"),
    ("kama_mean_reversion", "KamaMeanReversionStrategy"),
    ("keltner_rsi_confluence", "KeltnerRSIConfluenceStrategy"),
    ("fibonacci_rsi_mean_reversion", "FibonacciRsiMeanReversionStrategy"),
    ("heikin_ashi_trend_rider", "HeikinAshiTrendRiderStrategy"),
    ("kalman_mean_reversion", "KalmanMeanReversionStrategy"),
    ("fomc_drift_calendar", "FOMCDriftCalendarStrategy"),
    ("funding_rate_mean_reversion_v1", "FundingRateMeanReversionStrategy"),
    ("carter_squeeze_breakout", "CarterSqueezeBreakoutStrategy"),
]

SYMBOLS = {
    "crypto": ["BTC-USD", "ETH-USD", "SOL-USD", "XRP-USD", "ADA-USD", "AVAX-USD", "LINK-USD", "DOT-USD", "DOGE-USD", "LTC-USD"],
    "etf": ["SPY", "QQQ", "DIA", "IWM", "XLF", "XLK", "XLV", "XLE"],
    "forex": ["EURUSD=X", "GBPUSD=X", "USDJPY=X", "AUDUSD=X"],
    "equity": ["AAPL", "MSFT", "NVDA", "GOOGL", "AMZN", "META", "TSLA", "JPM"],
    "commodity": ["GC=F", "SI=F", "CL=F", "NQ=F"],
}
...
def backtest_signals(strat_cls, df, symbol):
    try:
        ...
        for sig in signals:
            ...
            risk = abs(entry - sl) if direction == 1 else abs(sl - entry)
            reward = abs(tp - entry) if direction == 1 else abs(entry - tp)
            if risk == 0:
                continue
...
def main():
    ...
    for module_name, class_name in STRATEGIES_TO_TEST:
        ...
        if valid:
            ...
            all_results[class_name] = {
                "module": module_name, "n": total_n,
                "wr": round(agg_wr, 1), "pf": round(agg_pf, 2),
                "symbols_with_signals": len(valid),
                "per_class": {ac: {"n": d["n"],
                                   "wr": round(d["wins"]/d["n"]*100, 1) if d["n"] > 0 else 0,
    ...
    out_path = "baby_strategies/results/batch4_strategies.json"

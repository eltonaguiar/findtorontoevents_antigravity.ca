...
class _VenueScraperBase(BaseScraper):
    ...
    def _iso(self, date_str: str) -> Optional[str]:
        ...
        # Date only — noon UTC keeps Toronto calendar day correct
        m = re.match(r"(\d{4}-\d{2}-\d{2})$", date_str)
        if m:
            return m.group(1) + "T12:00:00Z"

        # Long month — noon UTC to keep Toronto calendar day correct
        for fmt in ["%B %d, %Y", "%b %d, %Y", "%B %d %Y", "%b %d %Y"]:
            try:
                dt = datetime.strptime(date_str, fmt).replace(hour=12, minute=0, second=0)
        ...
        for fmt in ["%B %d", "%b %d"]:
            try:
                dt = datetime.strptime(date_str, fmt).replace(year=datetime.now().year, hour=12, minute=0, second=0)
                return dt.isoformat() + "Z"
            except ValueError:
                continue

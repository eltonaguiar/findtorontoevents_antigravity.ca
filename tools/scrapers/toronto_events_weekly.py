#!/usr/bin/env python3
"""
Toronto Events Weekly - curated weekly listings / newsletter.
Source: https://torontoeventsweekly.ca
"""
import re
...
class TorontoEventsWeeklyScraper(BaseScraper):
    """Scraper for Toronto Events Weekly (torontoeventsweekly.ca)."""

    SOURCE_NAME = "Toronto Events Weekly"
    BASE_URL = "https://torontoeventsweekly.ca"
    SOURCE_URLS = [BASE_URL + "/", BASE_URL + "/events", BASE_URL + "/weekly"]
    ...
    def scrape(self) -> List[ScrapedEvent]:
        ...
        for url in self.SOURCE_URLS:
            ...
            if not soup:
                continue
            for item in soup.find_all(class_=re.compile(r"event|post|entry|listing", re.I)) or soup.find_all("article") or soup.find_all("li"):
                try:
                    title_el = item.find(["h1", "h2", "h3", "h4"]) or item.find(class_=re.compile(r"title|heading", re.I))
                    if not title_el:
                        continue
                    ...
                    if not start_dt:
                        continue
                    loc_el = item.find(class_=re.compile(r"location|venue", re.I))

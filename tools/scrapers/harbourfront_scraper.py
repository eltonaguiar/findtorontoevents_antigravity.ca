#!/usr/bin/env python3
"""
Harbourfront Centre Events Scraper
Scrapes harbourfrontcentre.com for events.

Harbourfront Centre hosts major festivals (KUUMBA, Family Day Weekend,
International Festival of Authors, etc.) and year-round arts/culture programming.
"""
import re
import json
import time
...
class HarbourfrontCentreScraper(BaseScraper):
    """Scraper for Harbourfront Centre events."""

    SOURCE_NAME = "Harbourfront Centre"
    BASE_URL = "https://harbourfrontcentre.com"
    VENUE = TORONTO_VENUES["harbourfront centre"]

    EVENTS_URLS = [
        f"{BASE_URL}/whats-on/",
        f"{BASE_URL}/series/family-day-weekend-2026/",
        f"{BASE_URL}/program/music/",
        f"{BASE_URL}/explore/",
    ]
    ...
    def _parse_hc_date(self, date_str: str) -> Optional[str]:
        """Parse Harbourfront Centre date formats."""
        if not date_str:
            return None
        ...
        if iso_d:
            return iso_d.group(1) + "T12:00:00Z"

        # "February 15, 2026" etc.
        for fmt in ["%B %d, %Y", "%b %d, %Y", "%B %d %Y"]:
            try:
                dt = datetime.strptime(date_str, fmt)
                # Noon UTC — same fix as date-only ISO above
        ...
        for fmt in ["%B %d", "%b %d"]:
            try:
                dt = datetime.strptime(date_str, fmt)
                dt = dt.replace(year=datetime.now().year, hour=12, minute=0, second=0)
                return dt.isoformat() + "Z"
            except ValueError:
                continue
    ...
    def _parse_event_card(self, card, page_url: str) -> Optional[ScrapedEvent]:
        """Parse an HTML event card from Harbourfront Centre."""
        try:
            title_el = (
            ...
            # Date
            date_el = (
                card.find(class_=re.compile(r"date|time|when|schedule", re.I))
                or card.find("time")
            )
            date_text = ""
            if date_el:
            ...
            parsed_date = self._parse_hc_date(date_text)
            if not parsed_date:
                text = card.get_text()
                date_match = re.search(r"(\w+ \d{1,2},?\s*\d{4})", text)
                if date_match:
                    parsed_date = self._parse_hc_date(date_match.group(1))
            ...
            # Price
            price_el = card.find(class_=re.compile(r"price|cost|ticket|admission", re.I))
    ...
    def scrape(self) -> List[ScrapedEvent]:
        """Scrape Harbourfront Centre events."""
        all_events = []
        ...
        for url in self.EVENTS_URLS:
            ...
            for ev_data in self._extract_jsonld(soup):
                ...
                parsed_date = self._parse_hc_date(start)
                parsed_end = self._parse_hc_date(end) if end else None
                if not parsed_date:
                    continue
            ...
            # HTML cards
            containers = (
                soup.find_all(class_=re.compile(r"event-card|event-item|program-card|listing", re.I))
                or soup.find_all("article", class_=re.compile(r"event|program", re.I))
                or soup.find_all("div", class_=re.compile(r"event-card|card", re.I))
            )
...
def scrape_harbourfront() -> List[dict]:
    """Convenience function for standalone use."""
    scraper = HarbourfrontCentreScraper()
    return scraper.scrape_to_json()


if __name__ == "__main__":
    events = scrape_harbourfront()
    if events:

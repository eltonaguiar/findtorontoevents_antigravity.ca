...
def normalize_multiday_fields(event):
    ...
    This is critical because:
      ...
      - Legacy / UI events may use camelCase (isMultiDay, endDate)
...
def enrich_images(events, max_fetches=100, delay=0.5):
    """Fetch og:image for events that have a URL but no image.

    Visits the event page, looks for og:image meta tag, JSON-LD image,
    or the first large <img> on the page.  Respects rate limiting.
    """
    import time
    import requests
    ...
    no_image = [e for e in events if not e.get("image") and e.get("url")]
    print(f"\n{'='*60}")
    print(f"IMAGE ENRICHMENT: {len(no_image)} events without images")
    print('='*60)
    ...
    for event in no_image:
        if fetched >= max_fetches:
            print(f"  Reached max fetches ({max_fetches}), stopping enrichment")
            break
        ...
        # Skip URLs that are unlikely to have useful og:image
        skip_patterns = ["toronto.ca", "google.com", "notion.so"]
        ...
        try:
            ...
            if fetched % 20 == 0:
                print(f"  Progress: {fetched}/{min(len(no_image), max_fetches)} fetched, {enriched} enriched")

        except Exception as e:
            continue

    print(f"  Enrichment complete: fetched {fetched} pages, enriched {enriched} events")

    # Image stats
    total_with = sum(1 for e in events if e.get("image"))
    total = len(events)
    print(f"  Image coverage: {total_with}/{total} ({total_with/total*100:.1f}%)")
...
def main():
    ...
    parser.add_argument("--enrich-images", action="store_true", default=True,
                        help="Fetch og:image for events missing thumbnails (default: true)")
    parser.add_argument("--max-image-fetches", type=int, default=100,
                        help="Max pages to fetch for image enrichment (default: 100)")
    ...
    # Image enrichment: fetch og:image for events without thumbnails
    if args.enrich_images and not args.dry_run:
        merged = enrich_images(merged, max_fetches=args.max_image_fetches)

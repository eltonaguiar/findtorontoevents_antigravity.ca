...
Supported engines:
    CLI:  claude, gemini, opencode, kilo, copilot, agent (Cursor), kimi (Moonshot), openclaude
    API:  deepseek, cerebras, xai, inception, ollama_cloud, ollama_local, openrouter (driven via tools/swarm/api_consult.py)

Excluded: codex (OpenAI Codex CLI — disabled 2026-05-05 per user request), freebuff (PTY engine — REMOVED 2026-05-04), codebuff (TUI-only + out of credits).
...
CLI_ENGINES = {"claude", "gemini", "opencode", "kilo", "copilot", "agent", "kimi", "openclaude"}
# PTY_ENGINES set removed 2026-05-04 with freebuff. Restore if a new PTY/TUI
...
def _strip_code_fences(s: str) -> str:
    s = s.strip()
    # Primary: whole string is a code fence (no preamble).
    m = re.match(r"^```(?:json)?\s*\n?(.*?)\n?```\s*$", s, re.DOTALL)
    if m:
        return m.group(1).strip()
    # Secondary: reasoning preamble before a code fence (kilo, groq, deepseek-r1
    # often emit "Here's the JSON:\n```json\n{...}\n```"). Extract just the
    # fenced block so _extract_json_object can parse it directly.
...
# Module-level constant: body-level HTML error signals. Defined once
# at module load so _extract_json_object doesn't recreate it per call.
_HTML_SIGNALS = (
    "cloudflare", "nginx", "502 bad gateway", "403 forbidden",
    "access denied", "rate limit", "captcha", "attention required",
    "connection refused", "service unavailable", "504 gateway",
)
...
def _extract_json_object(s: str) -> dict | None:
    ...
    # 2026-05-05 fix: detect HTML-200 silent failure. When a provider
    # (ollama gateway, Cloudflare, misconfigured API) returns HTTP 200
    # with HTML body, all three parsing strategies return None and the
    # swarm records ok_count=1 with garbage output. Detect early and
    # return None with a clear stderr warning so the output is flagged
    # rather than silently recorded as success.
    stripped = s.lstrip()
    # Primary: stripped string starts with an HTML document marker.
    if (stripped.startswith("<!doctype") or stripped.startswith("<!DOCTYPE") or
            stripped.startswith("<html") or stripped.startswith("<head>")):
        sys.stderr.write(
            f"[_extract_json_object] HTML-200 SILENT FAILURE: "
            f"provider returned HTML, not JSON. Head: {stripped[:100]!r}\n"
        )
        return None
    # Secondary: body-level HTML signals in first 500 chars of a string
    # that starts with '<' but not a doc marker. Catches error pages
    # (Cloudflare challenge, Nginx 502, 403) embedded in JSON contexts.
    if stripped.startswith("<"):
        chunk = stripped[:500].lower()
        if any(sig in chunk for sig in _HTML_SIGNALS):
            sys.stderr.write(
                f"[_extract_json_object] HTML-200 SILENT FAILURE: "
                f"HTML body signals found in first 500 chars. "
                f"Head: {stripped[:100]!r}\n"
            )
            return None
    try:
        return json.loads(s)
...
def call_kimi(prompt: str, args: argparse.Namespace) -> tuple[str, str, dict]:
    ...
    Large-prompt handling (2026-05-05 fix): Windows CreateProcessW caps the
    command line at ~32 KB. Swarm briefs can hit 5-8 KB easily. Threshold
    is set conservatively at 6 KB: below it, `-p <prompt>` argv is used
    (kimi accepts this for quick runs); above it, prompt is fed via stdin
    and `-p ""` (empty string trigger) is passed as the positional arg.
    The stdin path avoids the Windows argv ceiling entirely. Tested on
    2026-05-05 with `kimi --help` smoke-check confirming `-p ""` is accepted.
    """
...
def main() -> int:
    ...
    # Clean up temporary session context files created for --from-session.
    if args.from_session:
        for suffix in ("_ctx.md",):
            ...
            if tmp_ctx.exists():
                try:
                    ...
                    pass  # permission denied — not fatal, session still complete

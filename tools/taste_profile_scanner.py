...
def scan_youtube_channel(channel_url):
    ...
    for i, pl in enumerate(playlists):
        ...
        for v in videos:
            # 2026-04-19: yt-dlp returns title=None for some private/age-restricted
            # videos (was crashing on .startswith). Coerce to empty string and skip.
            title = v.get('title') or ''
            channel = v.get('channel') or v.get('uploader') or ''
            vid_id = v.get('id', '')
            duration = v.get('duration')

            # Skip deleted/private (and the None case that triggers this anyway)
            if not title or title.startswith('[Deleted') or title.startswith('[Private'):
                continue

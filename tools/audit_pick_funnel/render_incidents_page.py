...
def fetch_all():
    ...
    with conn.cursor() as cur:
        ...
        # FINDINGS is a single cross-cutting table; defensive — may not exist until
        # tools/ai_tournament/migrations/202605291400_findings.sql is applied.
        findings = []
        try:
            cur.execute("SELECT * FROM FINDINGS ORDER BY noteworthy DESC, "
                        "FIELD(severity,'P0','P1','P2','P3','INFO'), created_at DESC")
            findings = list(cur.fetchall())
        except Exception as e:
            print(f"[render] FINDINGS table not available yet ({type(e).__name__}); skipping section")
    conn.close()
...
def finding_link(r):
    """Render the optional link to a parent incident/enhancement."""
    bits = []
    if r.get("linked_incident_id"):
        bits.append(f'<a href="/audit/incidents.html#incidents">INC-{esc(r["linked_incident_id"])}</a>')
    if r.get("linked_enhancement_id"):
        bits.append(f'<a href="/audit/incidents.html#enhancements">ENH-{esc(r["linked_enhancement_id"])}</a>')
    return " · ".join(bits) if bits else '<span style="color:#4b5563">—</span>'


def render_findings_section(findings):
    """Dated agent-logged findings (single table). Noteworthy pinned first, then
    by severity, then newest. Reuses created_est/sev_badge/status_badge."""
    out = ['<h2 id="findings">Findings (dated agent observations)</h2>']
    if not findings:
        out.append('<div class="small">No findings logged yet. Agents add them via '
                   '<code>tools/audit_pick_funnel/cli_track.py finding --title ...</code>.</div>')
        return "\n".join(out)
    n_note = sum(1 for r in findings if r.get("noteworthy"))
    out.append(f'<div class="small">Total: <strong>{len(findings)}</strong> · '
               f'<strong>{n_note}</strong> noteworthy · times shown in EST</div>')
    out.append('<table class="lb"><thead><tr>'
               '<th>★</th><th>Sev</th><th>Status</th><th>Created</th><th>Class</th><th>Type</th>'
               '<th>Title</th><th>Detail</th><th>Source</th><th>Linked</th><th>Links</th>'
               '</tr></thead><tbody>')
    for r in findings:
        star = '⭐' if r.get("noteworthy") else ''
        out.append(f'<tr><td>{star}</td><td>{sev_badge(r["severity"])}</td>'
                   f'<td>{status_badge(r["status"])}</td>'
                   f'<td>{created_est(r)}</td>'
                   f'<td class="small">{esc(r.get("asset_class") or "OVERALL")}</td>'
                   f'<td class="small">{esc(r.get("finding_type") or "")}</td>'
                   f'<td><strong>{esc(r["title"])}</strong></td>'
                   f'<td class="small" style="color:#9ca3af">{esc((r.get("detail") or "")[:300])}{"…" if r.get("detail") and len(r["detail"])>300 else ""}</td>'
                   f'<td class="small">{esc(r.get("source_agent") or "")}</td>'
                   f'<td class="small">{finding_link(r)}</td>'
                   f'<td class="small">{render_links(r)}</td></tr>')
    out.append('</tbody></table>')
    return "\n".join(out)


def render_html(incidents, enhancements, generated_at, findings=None):
...
{render_findings_section(findings or [])}
...
def main():
    ...
    html = render_html(incidents, enhancements, generated_at, findings)
    ...
    feed = {
        ...
        "findings": [{kk:(str(vv) if hasattr(vv,'isoformat') else vv) for kk,vv in r.items()} for r in findings],
        "totals": {"incidents": sum(len(v) for v in incidents.values()), "enhancements": sum(len(v) for v in enhancements.values()), "findings": len(findings)},
    }

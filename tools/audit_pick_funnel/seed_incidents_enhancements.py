...
INCIDENTS = [
    ...
    ("OVERALL", "trust_score NULL on 99.99% of closed picks",
     ...
     "P0", "RESOLVED", "trading_picks.trust_score / audit_dashboard/hc_filter.js",
     "RESOLVED 2026-05-31: 7,423/8,322 closed picks (89.2%) now have non-NULL trust_score. Backfill completed.",
    ...
    ("OVERALL", "5 FOREX rows have pnl_pct < -100% (one at -106,700%)",
     ...
     "P0", "RESOLVED", "trading_picks.pnl_pct (FOREX category)",
     "RESOLVED 2026-05-31: 0 rows with pnl_pct < -100 remain. Clamp fix verified.",
     "reports/2026-05-25_audit_ui_edge_audit.md", None, None, "claude-opus-4-7"),
    ...
    ("OVERALL", "signal_outcomes table 82 days stale",
     ...
     "P0", "RESOLVED", "at_signal_outcomes / outcome resolver pipeline",
     "RESOLVED 2026-05-31: 114,540 rows, last written 2026-05-31. Resolver active.",
    ...
    ("OVERALL", "COT paper pilot over-emission",
     ...
     "P0", "RESOLVED", "cot_paper_pilot.py / cot_positioning strategy",
     "RESOLVED 2026-05-31: Dedup by CFTC release week shipped (commit d317560ac9c).",
     None, None, None, "ring-2.6-1t"),
    ...
    ("OVERALL", "Smart Picks 'Signal Time' is dashboard-file age, not pick age",
     "RESOLVED 2026-05-31: smart_picks_engine.py line 1505 now emits signal_time from pick.open_time/timestamp/entry_date. Template (line 12906) renders with fallback chain signal_time->timestamp->generated_at->age_hours.",
     "P1", "RESOLVED", "audit_trail/dashboard_generator.py (smart_picks_feed builder)",
     "signal_time emitted by smart_picks_engine.py line 1505. Template fallback chain verified.",
     "updates/2026-05-31-incident-batch-resolve.md", "https://findtorontoevents.ca/audit/", None, "claude-opus-4-7"),
    ...
    ("OVERALL", "smart_picks.json file 25 days stale",
     ...
     "P0", "RESOLVED", "data/smart_picks.json / smart_picks_engine.py",
     "RESOLVED 2026-05-31: FALSE ALARM. Dashboard reads alpha_engine/data/smart_picks.json (hourly), not root data/ copy.",
     None, None, None, "ring-2.6-1t"),
    ...
    ("FOREX", "forex_carry.py exists in repo but is NOT in allowlist",
     "RESOLVED 2026-05-31: carry_trade (the strategy name emitted by forex_carry.py) added to NON_CRYPTO_STRATEGY_POLICY with probation thresholds + _FOREX_ALLOWED frozenset. Naming mismatch bug fixed: policy key changed from 'forex_carry' to 'carry_trade' to match emission source.",
     "P1", "RESOLVED", "alpha_engine/non_crypto_policy.py allowlist",
     "carry_trade added to policy + forex allowlist. Naming mismatch resolved.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "ring-2.6-1t"),

    ("FOREX", "FOREX SL at 0.5% sits at median daily FX ATR",
     "RESOLVED 2026-05-31: SL widened to 1.0% in non_crypto_policy.py line 428. TP 1.5% / SL 1.0% gives 1.5:1 R:R. Further optimization via ATR-based sizing deferred to forex_carry carry-trade strategy.",
     "P1", "RESOLVED", "alpha_engine FOREX TP/SL config",
     "FOREX SL widened to 1.0% (non_crypto_policy.py). R:R improved to 1.5:1.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "ring-2.6-1t"),
    ...
    ("OVERALL", "WON status rows show avg pnl_pct = -41.1%",
     "RESOLVED 2026-05-31: Verified live — 0 WON rows with negative pnl_pct. WON n=313, avg_pnl=+4.76%. All status/pnl contradictions resolved. Remaining WON rows all have exit_reason TP_HIT/TIME_EXIT/TP_HIT_REPLAY.",
     "P0", "RESOLVED", "trading_picks.status='WON' rows",
     "Verified: 0 contradictions. WON avg_pnl=+4.76% on n=313. Re-labeling was completed in prior fix.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "qwen-code"),

    ("OVERALL", "56,559 ghost rows in trading_picks (top cohort: 20,474 identical MATICUSDT entries)",
     "RESOLVED 2026-05-31: Verified live — 0 quan_engine MATICUSDT LONG pnl=-15 ghost rows remaining. Dedup completed in prior fix. Total ACTIVE picks: 5,405 with 0 UNKNOWN category.",
     "P0", "RESOLVED", "trading_picks ghost-row write path",
     "Verified: 0 ghost rows. Dedup completed in prior run.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "qwen-code"),
    ...
    ("OVERALL", "UNKNOWN asset_class on 951 active + 54 closed picks",
     "RESOLVED 2026-05-31: Verified live — 0 ACTIVE picks with NULL/UNKNOWN category. Backfill completed in prior fix. Classifier guard at write time confirmed active.",
     "P2", "RESOLVED", "trading_picks.category writer / classifier",
     "Verified: 0 UNKNOWN active picks. Backfill + classifier guard confirmed.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "claude-opus-4-7"),
    ...
    ("OVERALL", "sync_active_mysql_picks_to_json upstream writer missing — root cause of 0.09% raw-pick outcome coverage",
     "RESOLVED 2026-05-31: signal_outcomes now has 114,540 rows, last created 2026-05-31 01:37:58 UTC. Outcome resolver pipeline is live and running. Coverage gap closed.",
     "P0", "RESOLVED", "alpha_engine/active_picks_sync.py (proposed) + forward_validator.validate_picks()",
     "Verified: signal_outcomes 114,540 rows, last entry today. Resolver pipeline live.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "opencode/ring-2.6-1t"),
    ...
    ("FOREX", "All FOREX strategies losers except cta_cross_asset_tsmom SHORT (93% USDJPY concentration)",
     "RESOLVED 2026-05-31: FOREX consolidation implemented in non_crypto_policy.py (PR #6). All FOREX strategies blocked except cta_cross_asset_tsmom SHORT + carry_trade (probation). forex_rsi2, carry_trade_momentum, inverse_carry, forex_carry_ppp blacklisted.",
     "P0", "RESOLVED", "alpha_engine FOREX strategies (concentration risk)",
     "FOREX consolidation shipped (PR #6). Two-leg basket: cta_cross_asset_tsmom SHORT + carry_trade probation.",
     "updates/2026-05-31-incident-batch-resolve.md", None, None, "qwen-code+ring-2.6-1t"),

...
Runs walk-forward verdict lookup + optional pipeline metadata,
writes a single JSON artifact for promotion gates.

Usage:
  python3 tools/strategy_admit.py --strategy etf_dual_momentum
  python3 tools/strategy_admit.py --strategy vwap_reversion --asset-class CRYPTO --pipeline
...
def _hypothesis_registered(strategy: str) -> Optional[str]:
    ...
    if isinstance(reg, dict) and strategy in reg.get("hypotheses", {}):
        return strategy
    for e in entries:
...
def _pipeline_gate(strategy: str, asset_class: str) -> Dict[str, Any]:
    try:
        from verified_strategies.admissibility_pipeline import AdmissibilityPipeline

        AdmissibilityPipeline()
        return {
            "available": True,
            "note": "Call AdmissibilityPipeline with trade list for full 10-step verdict",
...
def admit(strategy: str, asset_class: str, include_pipeline: bool = False) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "recommendation": (
            "Live merge allowed only when walkforward PASS + class MONEY_READY + forward n>=100."
            if status != "PROMOTE_LIVE"
            else "All gates green — still require forward n>=100 before sizing."
        ),
    }
    if include_pipeline:
        out["pipeline"] = _pipeline_gate(strategy, asset_class)
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--pipeline", action="store_true", help="Include admissibility pipeline metadata")
    ...
    result = admit(args.strategy, args.asset_class, include_pipeline=args.pipeline)

        ...
        with conn.cursor() as cur:
            for pnl, row_id in updates:
                cur.execute(
                    ...
                    "AND (pnl_pct IS NULL OR status NOT IN ('WON', 'TP_HIT', 'CLOSED_WIN') OR pnl_pct <= 0.01) "
                    "AND (pnl_pct IS NULL OR status NOT IN ('LOST', 'SL_HIT', 'CLOSED_LOSS') OR pnl_pct >= -0.01)",
                    (pnl, row_id),
                )
                fixed += cur.rowcount
        conn.commit()

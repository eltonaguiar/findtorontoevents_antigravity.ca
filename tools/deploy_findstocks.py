#!/usr/bin/env python3
"""Quick deploy for FindStocks portfolio analysis system."""
import os
import ftplib
...
def main():
    ...
    print(f"Deploying FindStocks Portfolio to FTP: {host}")
    print(f"Remote path: {remote_path}/findstocks/\n")
    ...
    try:
        with ftplib.FTP(host) as ftp:
            ...
            # Upload findstocks/api/
            local_api = WORKSPACE / "findstocks" / "api"
            if local_api.is_dir():
                print("Uploading findstocks/api/ ...")
                n = upload_tree(ftp, local_api, f"{remote_path}/findstocks/api")
                print(f"  -> {n} files\n")

            # Upload findstocks/portfolio/
            local_portfolio = WORKSPACE / "findstocks" / "portfolio"
            if local_portfolio.is_dir():
                print("Uploading findstocks/portfolio/ ...")
                n = upload_tree(ftp, local_portfolio, f"{remote_path}/findstocks/portfolio")
                print(f"  -> {n} files\n")

        print("FindStocks deploy complete.")
        print()
        print("Post-deploy steps:")
        print("  1. Setup schema:  https://findtorontoevents.ca/findstocks/api/setup_schema.php")
        print("  2. Import picks:  https://findtorontoevents.ca/findstocks/api/import_picks.php")
        print("  3. Fetch prices:  https://findtorontoevents.ca/findstocks/api/fetch_prices.php")
        print("  4. View page:     https://findtorontoevents.ca/findstocks/portfolio/")
    except Exception as e:

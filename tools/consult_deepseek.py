#!/usr/bin/env python3
"""Consult DeepSeek for a second opinion. Mirrors tools/consult_cerebras.py.

Reads prompt from stdin or a single positional path arg, POSTs to
https://api.deepseek.com/chat/completions with model=deepseek-chat (V3),
writes reports/consult_deepseek_<UTC>_<slug>.md.

Requires env DEEPSEEK_API (or DEEPSEEK_API_KEY).
"""
...
def main() -> int:
    ...
    if not key:
        print("DEEPSEEK_API not set in env", file=sys.stderr)
        return 1
    ...
    out_path = out_dir / f"consult_deepseek_{ts}_{slug}.md"
    md = [
        f"# DeepSeek consult — {slug} ({ts})\n",

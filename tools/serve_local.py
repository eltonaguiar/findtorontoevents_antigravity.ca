...
AUDIT_DOCS_ROOT = _WORKSPACE_ROOT / "audit_dashboard"
...
class CORSRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Audit dashboard: mirror the live /audit/ route from local audit_dashboard/.
        if raw_path == "/audit" or raw_path == "/audit/":
            audit_index = AUDIT_DOCS_ROOT / "index.html"
            if audit_index.is_file():
                self.send_response(200)
                self.send_header("Content-Type", "text/html; charset=utf-8")
                self.end_headers()
                self.wfile.write(audit_index.read_bytes())
                return
        if raw_path.startswith("/audit/"):
            sub = raw_path[len("/audit/"):].lstrip("/")
            if sub and ".." not in sub:
                candidate_roots = [AUDIT_DOCS_ROOT]
                if sub.startswith("data/"):
                    candidate_roots.extend([
                        _WORKSPACE_ROOT / "audit_trail",
                        _WORKSPACE_ROOT / "audit_dashboard",
                    ])
                for root in candidate_roots:
                    try:
                        local = (root / sub.replace("/", os.sep)).resolve()
                    ...
                    if local and local.is_file():
                        ...
                        return
                    if local and local.is_dir() and (local / "index.html").is_file():
                        self.send_response(200)
                        self.send_header("Content-Type", "text/html; charset=utf-8")
                        self.end_headers()
...
def main():
    ...
    print(f"  Main page (index.html):  http://127.0.0.1:{PORT}/")
    print(f"  Audit dashboard:         http://127.0.0.1:{PORT}/audit/")
    print(f"  Hyro challenge tracker:  http://127.0.0.1:{PORT}/audit/hyrotrader/")
    print(f"  FavCreators:             http://127.0.0.1:{PORT}/fc/  or  .../fc/#/guest")
    print(f"  Serving from:            {workspace_root}")
    print(f"  Tip: Use 127.0.0.1 (not localhost) if the browser hangs — Windows often resolves localhost to IPv6 first while this server listens on IPv4.")
    if not API_PROXY:
        print(f"  To use REAL MySQL: set FAVCREATORS_API_PROXY=https://findtorontoevents.ca then restart.")
    print(f"  Verify API: open http://127.0.0.1:{PORT}/fc/api/get_notes.php?user_id=0 — you should see JSON.")
    ...
    try:
        ...
        if e.errno == 98 or "Address already in use" in str(e) or "WinError 10048" in str(e):
            ...
            print("    PowerShell:  $env:PORT=5174; python tools/serve_local.py")
            print("    bash:        PORT=5174 python tools/serve_local.py")

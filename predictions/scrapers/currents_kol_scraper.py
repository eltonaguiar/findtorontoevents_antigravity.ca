"""
Currents API KOL Scraper
========================
Finds KOL mentions in news articles via Currents API (currentsapi.services)
and extracts directional signals (price targets, sentiment, direction).

Key advantage: Currents API returns NLP-detected article categories,
enabling better relevance filtering for financial content.

Free-tier friendly: 600 requests/day — caps total requests at 10 per run.
"""
...
# Config
# ---------------------------------------------------------------------------
CURRENTS_KEY = os.getenv("CURRENTS_NEWS_API", "")
BASE = "https://api.currentsapi.services/v1/search"

# ---------------------------------------------------------------------------
# Relevant article categories (NLP-detected by Currents API)
# ---------------------------------------------------------------------------
RELEVANT_CATEGORIES = {
    "finance", "business", "technology", "crypto", "cryptocurrency",
    "markets", "economy",
}
...
def _fetch_news(query: str, page_size: int = 10) -> dict | None:
    """Fetch articles from Currents API /search endpoint.

    Uses the same SSL context pattern as newsapi_kol_scraper.
    """
...
def _has_relevant_category(article: dict) -> bool:
    """Check if an article has at least one relevant financial category.

    The Currents API returns a 'category' field as a list of NLP-detected
    categories. We filter to only process financially relevant articles.
    """
    categories = article.get("category", [])
    if not categories:
        # No categories means we can't filter — allow it through
        return True
    article_cats = {c.lower().strip() for c in categories}
    return bool(article_cats & RELEVANT_CATEGORIES)
...
def _build_kol_queries(kols: list[dict], batch_size: int = 5) -> list[str]:
    """Group KOL display names into batched queries for Currents API.
...
def extract_predictions_from_articles(
    ...
    """Extract directional predictions from a list of Currents API articles.

    Parameters
    ----------
    articles : list[dict]
        Article objects from the Currents API response.
    source_query : str
    ...
    for article in articles:
        # Filter by NLP category relevance
        if not _has_relevant_category(article):
            continue
        ...
        # Source info — Currents uses 'author' instead of nested source object
        source_name = article.get("author") or "unknown"
        ...
        for sym_info in symbol_hits:
            ...
            predictions.append({
                "predictor_id": f"currents:{source_name}",
                "platform": "currents",
...
def main():
    if not CURRENTS_KEY:
        print("[currents_kol_scraper] No CURRENTS_NEWS_API set - skipping.")
        return

    print("=" * 60)
    print("Currents API KOL Scraper")
    print("=" * 60)
    ...
    # Free tier: 600 requests/day — cap total requests at 10 per run
    MAX_REQUESTS = 10
    request_count = 0
    ...
    for query in kol_queries[:kol_query_limit]:
        ...
        data = _fetch_news(query, page_size=10)
        request_count += 1

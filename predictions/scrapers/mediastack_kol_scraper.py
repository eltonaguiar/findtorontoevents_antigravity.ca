"""
Mediastack KOL Scraper
======================
Finds KOL mentions in global news articles via Mediastack.com API and
extracts directional signals (price targets, sentiment, direction).

Especially useful for macro/forex coverage that other news APIs miss.

Free-tier friendly: 500 requests/month (~16/day) — caps at 6 per run.
...
# ---------------------------------------------------------------------------
# Topic queries for prediction-related news (fewer to conserve quota)
# ---------------------------------------------------------------------------
TOPIC_QUERIES = [
...
def _fetch_news(keywords: str, limit: int = 25) -> dict | None:
    """Fetch articles from Mediastack /news endpoint.

    Uses plain HTTP (no SSL) because Mediastack free tier does not
    support HTTPS.
    """
    ...
    try:
        ...
        print(f"  API error for \"{keywords}\": {e}")
        return None
...
def _build_kol_queries(kols: list[dict], batch_size: int = 3) -> list[str]:
    """Group KOL display names into batched comma-separated keyword queries.

    Mediastack uses comma-separated keywords (AND logic), so we batch
    smaller groups to increase hit rate.

    Returns queries like: 'Arthur Hayes,prediction'
    Each KOL name becomes its own query to maximize relevance.
    """
    names = [k["display_name"] for k in kols if k.get("display_name")]
    queries: list[str] = []
    for i in range(0, len(names), batch_size):
        batch = names[i : i + batch_size]
        # Comma-separated names as keywords
        query = ",".join(batch)
...
def extract_predictions_from_articles(
    ...
    """Extract directional predictions from a list of Mediastack articles.

    Parameters
    ----------
    articles : list[dict]
        Article objects from the Mediastack response ``data`` array.
    source_query : str
    ...
    for article in articles:
        ...
        # Source info
        source_name = article.get("source", "unknown") or "unknown"
        ...
        for sym_info in symbol_hits:
            ...
            predictions.append({
                "predictor_id": f"mediastack:{source_name}",
                "platform": "mediastack",
...
def main():
    if not MEDIASTACK_KEY:
        print("[mediastack_kol_scraper] No MEDIASTACK_API_KEY set - skipping.")
        return

    print("=" * 60)
    print("Mediastack KOL Scraper")
    print("=" * 60)
    ...
    # Free tier: 500 requests/month (~16/day) — cap at 6 per run
    MAX_REQUESTS = 6
    request_count = 0

    all_predictions: list[dict] = []

    # --- KOL name queries (max 3 to stay within budget) ---
    ...
    for query in kol_queries[:kol_query_limit]:
        ...
        data = _fetch_news(query, limit=25)
        request_count += 1
        ...
        time.sleep(2)  # Rate limit between calls (conservative for 500/month)
        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    request_count * 25,  # approximate articles fetched
                    inserted,
                    "",
                ),
            )

...
GNews KOL Scraper
=================
Finds KOL mentions in news articles via GNews.io API and extracts
directional signals (price targets, sentiment, direction).

Free-tier friendly: 100 requests/day — caps at 8 requests per run.
"""
...
TOPIC_QUERIES = [
    ...
    "stock market outlook OR SPY forecast",
    "ethereum target OR solana prediction",
    "forex forecast OR gold prediction",
]
...
def _fetch_news(query: str, max_results: int = 10) -> dict | None:
    """Fetch articles from GNews.io /search endpoint.

    Uses relaxed SSL context matching the newsapi scraper pattern.
    """
...
def _build_kol_queries(kols: list[dict], batch_size: int = 3) -> list[str]:
    """Group KOL display names into batched OR queries for GNews.

    Smaller batches (3) than NewsAPI since GNews query length is more limited.
...
def extract_predictions_from_articles(
    ...
    """Extract directional predictions from a list of GNews articles.

    Parameters
    ----------
    articles : list[dict]
        Article objects from the GNews response.
    source_query : str
    ...
    for article in articles:
        ...
        # Source info — GNews nests source under {"name": "..."}
        ...
        # 30% reduction: multiply by 0.7 for KOL mentions, 0.5 for generic news
        ...
        for sym_info in symbol_hits:
            ...
            predictions.append({
                "predictor_id": f"gnews:{source_name}",
                "platform": "gnews",
...
def main():
    if not GNEWS_API_KEY:
        print("[gnews_kol_scraper] No GNEWS_API_KEY set - skipping.")
        return

    print("=" * 60)
    print("GNews KOL Scraper")
    print("=" * 60)
    ...
    for query in kol_queries[:kol_query_limit]:
        ...
        data = _fetch_news(query, max_results=10)
        request_count += 1

...
NewsAPI KOL Scraper
===================
Finds KOL mentions in news articles via NewsAPI.org and extracts
directional signals (price targets, sentiment, direction).

Free-tier friendly: caps total requests at 10 per run.
"""
...
# Config
# ---------------------------------------------------------------------------
NEWSAPI_KEY = os.getenv("NEWSAPIDOTORG") or os.getenv("NEWSAPI_KEY", "")
...
# ---------------------------------------------------------------------------
TOPIC_QUERIES = [
    "bitcoin prediction OR bitcoin forecast OR BTC price target",
    "crypto prediction OR altcoin forecast OR ethereum target",
    "stock market prediction OR SPY forecast OR market outlook",
]
...
def _fetch_news(query: str, page_size: int = 20) -> dict | None:
    """Fetch articles from NewsAPI /everything endpoint.

    Reuses the SSL context + urllib pattern from news_portfolio_builder.py.
    """
...
def _build_kol_queries(kols: list[dict], batch_size: int = 5) -> list[str]:
    """Group KOL display names into batched OR queries for NewsAPI.
...
def extract_predictions_from_articles(
    ...
    """Extract directional predictions from a list of NewsAPI articles.

    Parameters
    ----------
    articles : list[dict]
        Article objects from the NewsAPI response.
    source_query : str
    ...
    for article in articles:
        ...
        for sym_info in symbol_hits:
            ...
            predictions.append({
                "predictor_id": f"newsapi:{source_name}",
                "platform": "newsapi",
...
def main():
    if not NEWSAPI_KEY:
        print("[newsapi_kol_scraper] No NEWSAPI_KEY set - skipping.")
        return

    print("=" * 60)
    print("NewsAPI KOL Scraper")
    print("=" * 60)
    ...
    # Free tier: 100 requests/day — cap total requests at 10
    MAX_REQUESTS = 10
    request_count = 0
    ...
    for query in kol_queries[:kol_query_limit]:
        ...
        data = _fetch_news(query, page_size=20)
        request_count += 1
        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    request_count * 20,  # approximate articles fetched
                    inserted,
                    "",
                ),
            )

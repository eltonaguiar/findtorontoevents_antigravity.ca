...
$pass = 'wannabet';
$db = 'ejaguiar1_sportsbet';

$conn = mysql_connect($host, $user, $pass);
if (!$conn) {
    die(json_encode(['ok' => false, 'error' => 'Connection failed: ' . mysql_error()]));
}

if (!mysql_select_db($db)) {
    die(json_encode(['ok' => false, 'error' => 'DB select failed: ' . mysql_error()]));
}

// Schema creation queries (from sports_schema.php with ML enhancements)
$queries = [
    "CREATE TABLE IF NOT EXISTS lm_sports_odds (
    ...
    "CREATE TABLE IF NOT EXISTS lm_sports_credit_usage (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    "CREATE TABLE IF NOT EXISTS lm_sports_clv (
        id INT AUTO_INCREMENT PRIMARY KEY,
        event_id VARCHAR(100) NOT NULL,
    ...
    "CREATE TABLE IF NOT EXISTS lm_sports_value_bets (
        id INT AUTO_INCREMENT PRIMARY KEY,
        event_id VARCHAR(100) NOT NULL,
        sport VARCHAR(50) NOT NULL,
        home_team VARCHAR(100) NOT NULL,
    ...
    "CREATE TABLE IF NOT EXISTS lm_sports_daily_picks (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    "CREATE TABLE IF NOT EXISTS lm_sports_bets (
        id INT AUTO_INCREMENT PRIMARY KEY,
        event_id VARCHAR(100) NOT NULL,
        sport VARCHAR(50) NOT NULL,
        home_team VARCHAR(100) NOT NULL,
        ...
        ml_model_type VARCHAR(50) NOT NULL DEFAULT 'baseline',
        ml_predicted_at DATETIME DEFAULT NULL,
        ...
        KEY idx_ml_prediction (ml_prediction),
        KEY idx_ml_win_prob (ml_win_prob),
        KEY idx_ml_should_bet (ml_should_bet)
    ) ENGINE=MyISAM DEFAULT CHARSET=utf8",

    "CREATE TABLE IF NOT EXISTS lm_sports_bankroll (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
foreach ($queries as $q) {
    if (mysql_query($q)) {
        ...
        echo json_encode(['ok' => false, 'error' => mysql_error()]);
        mysql_close($conn);
        exit;
    }
}
...
echo json_encode(['ok' => true, 'tables_created' => $success, 'tables' => $tables]);

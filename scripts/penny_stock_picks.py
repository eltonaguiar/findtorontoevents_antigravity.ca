...
def safe_api_json(resp, ctx):
    """Parse a requests Response as JSON, degrading gracefully.

    The findtorontoevents.ca penny-stock PHP endpoint can return an empty
    body or an HTML error page during a transient outage. A bare
    ``resp.json()`` then raises ``requests.exceptions.JSONDecodeError`` and
    crashes the workflow. This guard checks the HTTP status and body before
    parsing and returns ``None`` on any non-JSON response so callers can
    fall through to their fallback path instead of aborting.
    """
    if resp.status_code != 200:
        logger.warning(f"{ctx}: HTTP {resp.status_code} - treating as no data")
        return None
    if not (resp.text or '').strip():
        logger.warning(f"{ctx}: empty response body - treating as no data")
        return None
    try:
        return resp.json()
    except ValueError:
        logger.warning(f"{ctx}: non-JSON response ({len(resp.text)} bytes) - treating as no data")
        return None


def api_get_json(url, ctx, params=None, retries=2, timeout=30):
    """GET ``url`` and return parsed JSON, with a short retry on transient
    failures. Returns ``None`` (never raises) so a single bad API response
    cannot fail the whole workflow — callers degrade to their fallback.
    """
    for attempt in range(retries + 1):
        try:
            resp = requests.get(url, params=params, headers=API_HEADERS, timeout=timeout)
            data = safe_api_json(resp, ctx)
            if data is not None:
                return data
            # 5xx is worth a retry; 4xx / empty body is not
            if resp.status_code < 500:
                return None
        except Exception as e:
            logger.warning(f"{ctx}: request error ({e}) attempt {attempt + 1}/{retries + 1}")
        if attempt < retries:
...
# Path to the yfinance seed-list fallback. Loaded only when the primary
# API_BASE source returns no results (e.g. when the 50webs PHP endpoint
# is 404 because the file was never deployed to main — root-cause
# documented in reports/penny_picks_brokie_suitability_2026-05-13.md).
PENNY_UNIVERSE_SEED_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    '..', 'data', 'penny_universe_seed.json'
)


def _load_universe_seed():
    """Load static fallback seed list. Returns [] if absent."""
    try:
        with open(PENNY_UNIVERSE_SEED_PATH, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # Accept either {"symbols": [...]} or a bare list
        if isinstance(data, dict):
            return data.get('symbols', []) or []
        if isinstance(data, list):
            return data
    except (OSError, json.JSONDecodeError) as e:
        logger.warning(f"penny seed load failed: {e}")
    return []


def _yf_universe_fallback():
    """Fetch penny-band metadata for each seed symbol via yfinance.

    Used when the primary API_BASE endpoint returns empty/404. Returns
    the same candidate schema as the API path so downstream filtering is
    identical. Honors MIN_PRICE / MAX_PRICE / MIN_VOLUME constants.
    """
    seed = _load_universe_seed()
    if not seed:
        logger.warning("penny universe seed list missing or empty — fallback disabled")
        return []

    logger.info(f"yfinance fallback: probing {len(seed)} seed symbols")
    candidates = []
    for symbol in seed:
        try:
            t = yf.Ticker(symbol)
            # 5d history is enough to compute current price + recent volume
            hist = t.history(period='90d', interval='1d', auto_adjust=False)
            if hist.empty:
                continue
            price = float(hist['Close'].iloc[-1])
            vol_today = int(hist['Volume'].iloc[-1])
            avg_vol_3m = int(hist['Volume'].tail(63).mean()) if len(hist) >= 21 else int(hist['Volume'].mean())
            if not (MIN_PRICE <= price <= MAX_PRICE):
                continue
            if avg_vol_3m < MIN_VOLUME:
                continue
            # Pull exchange metadata; skip blocked
            ...
            candidates.append({
                ...
                'rrsp_eligible': False,  # US universe only in fallback
                'change_pct': 0,  # not computed in fallback
            })
            time.sleep(RATE_LIMIT_DELAY)
        except Exception as e:
            logger.debug(f"yf-fallback skip {symbol}: {e}")
            continue

    logger.info(f"yfinance fallback: {len(candidates)} candidates pass filters")
    return candidates


def get_penny_universe():
    """Fetch penny stock universe from screener API; fall back to yfinance
    seed list if the API returns empty / 404 / non-JSON.
    """
    candidates = []
    api_ok = False
    ...
    for region in ['us', 'ca']:
        for offset in range(0, 200, 100):
            ...
            data = api_get_json(url, f"universe[{region}@{offset}]", params=params, timeout=30)
            if data is None:
                # transient/empty/non-JSON — skip this page, fallback handles
                # the no-candidate case below.
                logger.warning(f"Failed to fetch {region} offset={offset}: no JSON")
                break
            try:
    ...
    if not api_ok and not candidates:
        logger.warning("primary API returned no candidates — engaging yfinance seed fallback")
        candidates = _yf_universe_fallback()
...
def post_picks_to_api(picks):
    ...
    try:
        ...
        result = safe_api_json(resp, 'store_picks')
        if result is None:
            # Empty / non-JSON / non-200 — local report was already saved,
            # so this is a non-fatal degraded state, not a crash.
            logger.error("store_picks: API returned no usable JSON — local report retained")
            return {'ok': False, 'error': 'non_json_api_response'}
        if result.get('ok'):

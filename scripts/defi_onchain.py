#!/usr/bin/env python3
"""
DeFi On-Chain Analytics — Crypto leading indicators from blockchain data.
...
Tracks metrics that lead price by hours to days:
  ...
  - Top DeFi yields (smart money flow)
...
Pipeline:
  ...
  3. Post to world_class_intelligence.php + quant_bridge.php
...
logger = logging.getLogger('defi_onchain')
...
def generate_onchain_signals(btc_network, defi_tvl, stablecoin, chain_tvls):
    ...
    if tvl_7d > 5:
        signals.append({
            ...
            'description': f"DeFi TVL up {tvl_7d:.1f}% in 7 days",
        })
    elif tvl_7d < -5:
        signals.append({
            ...
            'description': f"DeFi TVL down {abs(tvl_7d):.1f}% in 7 days",
        })
    ...
    if unconfirmed > 200000:
        signals.append({
            ...
            'direction': 'VOLATILE',
            'strength': 60,
            'description': f"BTC mempool congested ({unconfirmed:,} unconfirmed)",
        })
...
def main():
    """Run DeFi on-chain analytics pipeline."""
    logger.info("=" * 60)
    logger.info("DEFI ON-CHAIN ANALYTICS — Starting")
    logger.info("=" * 60)
    ...
    post_to_bridge('onchain_analytics', payload,
                   "BTC=$%s, TVL 7d=%+.1f%%" % (
                       "{:,.0f}".format(btc_network.get('market_price', 0)),
                       defi_tvl.get('tvl_7d_change_pct', 0)))

    # Summary
    logger.info("")
    logger.info("=" * 60)
    logger.info("DEFI ON-CHAIN SUMMARY")

...
def engineer_features(signals_df):
    ...
    # Signal features
    df['strength'] = pd.to_numeric(df['signal_strength'], errors='coerce').fillna(50) if 'signal_strength' in df.columns else 50
    _tp = pd.to_numeric(df['target_tp_pct'], errors='coerce').fillna(5) if 'target_tp_pct' in df.columns else 5
    _sl = pd.to_numeric(df['target_sl_pct'], errors='coerce').fillna(3).clip(lower=0.1) if 'target_sl_pct' in df.columns else 3
    df['tp_sl_ratio'] = _tp / _sl
    df['hold_hours'] = pd.to_numeric(df['max_hold_hours'], errors='coerce').fillna(24) if 'max_hold_hours' in df.columns else 24
    ...
    # 2026-04-19: fixed AttributeError: 'float' object has no attribute 'fillna'.
    # df.get(col, scalar) returns the scalar (not a Series) if the column is
    # absent, so pd.to_numeric(0.5).fillna(...) crashes. Use a Series default
    # matching the pattern used for hmm_regime / asset_class above.
    df['bundle_recent_wr'] = pd.to_numeric(
        df.get('bundle_recent_wr', pd.Series([0.5] * len(df))), errors='coerce'
    ).fillna(0.5)
    df['algo_recent_wr'] = pd.to_numeric(
        df.get('algo_recent_wr', pd.Series([0.5] * len(df))), errors='coerce'
    ).fillna(0.5)

...
class HMMRegimeDetector:
    ...
    def fit_predict(self, returns, volatility):
        ...
        # Try progressively safer configs when covariance becomes non-PD.
        # We keep a deterministic fallback so the workflow degrades gracefully.
        candidate_cfgs = [
            {"covariance_type": "full", "tol": 0.01, "min_covar": 1e-3, "random_state": 42},
            {"covariance_type": "diag", "tol": 0.01, "min_covar": 1e-3, "random_state": 43},
            {"covariance_type": "diag", "tol": 0.02, "min_covar": 1e-2, "random_state": 44},
        ]
        ...
        for idx, cfg in enumerate(candidate_cfgs, start=1):
            ...
            try:
                ...
                self.model = model
                logger.info(
                    "HMM regime: %s (confidence=%.2f, persistence=%.2f) [cfg=%s try=%d]",
                    regime_label,
                    current_prob,
                    persistence,
                    cfg["covariance_type"],
                ...
                logger.info(
                    "HMM state means: %s",
                    {self.regime_map[i]: round(float(means[i]) * 100, 3) for i in range(self.n_states)},
                )
                return regime_label, current_prob, persistence
            except Exception as e:
                last_err = e
                logger.warning(
                    "HMM attempt %d failed (covariance=%s, min_covar=%s): %s",
                    idx,
                    cfg["covariance_type"],
                    cfg["min_covar"],
                    e,
        ...
        logger.error("All HMM attempts failed, falling back to sideways regime: %s", last_err)
        return 'sideways', 0.35, 0.50

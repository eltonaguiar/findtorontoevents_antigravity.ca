...
class EnsembleStacker:
    ...
    def prepare_data(self, X, y):
        """Prepare data for stacking using temporal split (no shuffling)"""
        # Time-series safe split: use the last test_size fraction as validation
    ...
    def cross_validate(self, X, y, cv=5):
        """Perform time-series cross-validation on the stacking ensemble"""
        ...
        for train_idx, val_idx in tscv.split(X):
            ...
            # Further split training data for meta-model training
            inner_split = int(len(X_train) * (1 - self.test_size))
            X_tr = X_train.iloc[:inner_split] if hasattr(X_train, 'iloc') else X_train[:inner_split]
            X_inner_val = X_train.iloc[inner_split:] if hasattr(X_train, 'iloc') else X_train[inner_split:]
            y_tr = y_train.iloc[:inner_split] if hasattr(y_train, 'iloc') else y_train[:inner_split]
            y_inner_val = y_train.iloc[inner_split:] if hasattr(y_train, 'iloc') else y_train[inner_split:]

            # Train base models on inner training set
            base_models_trained = self.train_base_models(X_tr, y_tr)

            # Get predictions on inner validation set for meta-model training
            base_preds_inner = self.get_base_predictions(base_models_trained, X_inner_val)
            meta_features_inner = self.create_meta_features(base_preds_inner)
            meta_features_inner_scaled = self.scaler.fit_transform(meta_features_inner)
            meta_model = self.meta_model.fit(meta_features_inner_scaled, y_inner_val)

            # Evaluate on outer validation fold
            base_preds_val = self.get_base_predictions(base_models_trained, X_val)
            meta_features_val = self.create_meta_features(base_preds_val)
            meta_features_val_scaled = self.scaler.transform(meta_features_val)
            predictions = meta_model.predict(meta_features_val_scaled)

            cv_scores.append(mean_squared_error(y_val, predictions))

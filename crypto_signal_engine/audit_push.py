"""Crypto Signal Engine — Audit Trail Push (SQLite + MySQL dual-write)."""
import json, logging, os, sys

log = logging.getLogger("signal_engine.audit_push")
...
def normalize(p):
    return {
        ...
        "strategy": p.get("strategy", "signal_engine"),
...
def main():
    ...
    picks = load_picks()
    if not picks:
        log.info("No Signal Engine picks to push")
        return
    ...
    record_event("SIGNAL_ENGINE_AUDIT_PUSH", run_id=run_id,
    ...
    log.info("SignalEngine: %d/%d picks pushed to audit trail", ok, len(picks))

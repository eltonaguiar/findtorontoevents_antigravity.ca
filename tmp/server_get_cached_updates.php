 ...
 * Includes UTF-8 sanitizer for database content with Windows-1252 characters.
 */

header('Content-Type: application/json; charset=utf-8');

/**
 * Recursively sanitize strings in an array/value to valid UTF-8.
 * Replaces Windows-1252 smart quotes/special chars with UTF-8 equivalents,
 * then strips any remaining invalid bytes.
 * Works on PHP 5.2+.
 */
function _sanitize_utf8($data) {
    if (is_array($data)) {
        $out = array();
        foreach ($data as $k => $v) {
            $out[$k] = _sanitize_utf8($v);
        }
        return $out;
    }
    if (!is_string($data)) return $data;
    // Map common Windows-1252 bytes (0x80-0x9F) to UTF-8 equivalents
    $win1252_map = array(
    ...
    // Final fallback: strip non-UTF-8 byte sequences
...
// Sanitize all data to ensure valid UTF-8 (fixes Windows-1252 chars from scraped content)
$response = _sanitize_utf8(array(
    'ok' => true,
    'user_id' => $user_id,
    'is_guest' => $user_id === 0,
...
// Use JSON_INVALID_UTF8_SUBSTITUTE on PHP 7.2+ as a safety net

...
(function () {
  ...
  function speakText(text) {
    ...
    utter.pitch = 1;
    utter.lang = 'en-US';
    utter.onend = function () { state.speaking = false; updateBtnState(); };
    utter.onerror = function () { state.speaking = false; updateBtnState(); };
  ...
  async function routeCommandAsync(lower, raw) {
    try {
      ...
      if (/what'?s new/i.test(lower) || /what'?s trending/i.test(lower) || /what'?s popular/i.test(lower) ||
        ...
        setPrompts(['Events this weekend', 'Who is live?', 'New content this week', 'Top stock picks', 'Trending movies']);
        setStatus('Ready', '#64748b');
        return;
      }
  ...
  async function handleStreamers(lower) {
    ...
    try {
      ...
      if (isRefreshQuery || isWhoIsLive) {
        ...
        if (liveCreatorsList.length > 0) {
          html2 += '<br>Want me to open <b>' + escapeHtml(liveCreatorsList[0].name) + '</b>\'s stream?';
        }
      ...
      if (allLive.length > 0) {
        speakText(allLive.length + ' of your creators are live right now. ' + (allLive[0].name || allLive[0].display_name) + ' is streaming. Say refresh my creators for a full live check.');
      } else {
  ...
  function updatePrompts(section) {
    ...
    switch (section) {
      ...
      case 'streamers':
        prompts = [
          'Refresh my creators', 'Who is live?', 'Anyone streaming right now?',
  ...
  function init() {
    ...
    function updateLiveCheckMsg(current, total, currentCreator, percent) {
      ...
      if (_liveCheckLiveFound.length > 0) {
        liveHtml = '<div style="margin-top:6px;padding:6px 8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);border-radius:6px;font-size:0.8rem;">' +
          ...
          _liveCheckLiveFound.map(function(l) { return '<b>' + l.name + '</b> <span style="color:var(--text-muted);">(' + l.platform + ')</span>'; }).join(', ') +
          '</div>';
      }
      var viewBtnHtml = '';
    ...
    window.addEventListener('fc-live-found', function(e) {
      if (detectSection() !== 'streamers') return;
      var d = e.detail || {};
      _liveCheckLiveFound.push({ name: d.name, platform: d.platform });
      ...
      if (el) {
        ...
        if (d.liveCreators && d.liveCreators.length > 0) {
          ...
          for (var i = 0; i < d.liveCreators.length; i++) {
            ...
            liveHtml += '<div style="margin-top:4px;">\u2022 <b>' + lc.name + '</b> <span style="color:var(--text-muted);">(' + lc.platforms.join(', ') + ')</span></div>';
          }
          liveHtml += '</div>';
        } else {
          liveHtml = '<div style="margin-top:6px;color:var(--text-muted);font-size:0.85rem;">No creators are live right now.</div>';
        }
        el.innerHTML = '<div style="font-size:0.85rem;color:#86efac;font-weight:600;">\u2705 Finished! Checked ' + d.total + ' creators.</div>' + liveHtml;
      }

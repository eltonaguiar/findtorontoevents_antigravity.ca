...
def monte_carlo_test(df: pd.DataFrame, config: ForexMeanReversionConfig = None,
    ...
    """
    Monte Carlo permutation test to verify edge is not due to luck.
    Shuffle trade order 1000x, compute Sharpe distribution, check if
    actual Sharpe is in the top (1-confidence) percentile.
    """
    if config is None:
        config = ForexMeanReversionConfig()
    ...
    signals = generate_signals(df, config)
    # Collect raw trade pnls for permutation
    pnls = actual.get('equity_curve', [])
    if not pnls:
        return {'status': 'NO_EQUITY'}

    # Convert equity curve to per-trade pnls
    ...
    return {
        ...
        'simulated_mean_sharpe': round(np.mean(simulated_sharpes), 3),
        'simulated_std_sharpe': round(np.std(simulated_sharpes), 3),

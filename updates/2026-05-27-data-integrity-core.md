...
**Date:** 2026-05-27
**Status:** Execution Complete — Dedup Applied With Backup; Awaiting Peer Review
...
- `trading_picks`: 49,622 total rows before dedup; 45,283 after dedup
- Duplicate rows removed: 4,339 rows (8.7% of original table) across 1,304 groups
- Backup table created: `trading_picks_duplicates_backup` with 4,339 rows
- Backup verification: 0 overlap with live table after delete; 4,339/4,339 backup rows have a keeper in `trading_picks`; 0 orphaned backup rows
...
**Operations (in order):**
1. Create backup table `trading_picks_duplicates_backup` using the same duplicate-selection predicate used for deletion
2. Insert all rows that will be deleted into the backup table
3. Delete newer duplicate rows — keeps oldest (smallest `id`) per (symbol, direction, strategy, entry_price)
4. Fix confidence scale (if needed)
5. Add UNIQUE INDEX (if not exists)

**Scope:** `trading_picks` table
**Measured:** 4,339 duplicate rows, 0 confidence fixes needed, UNIQUE INDEX already exists
**Status:** **EXECUTED** — 4,339 rows backed up and removed; live table reduced from 49,622 to 45,283 rows

**Legitimate-purpose review:** The duplicate rows were generated by aggregator re-runs that minted fresh UUIDs for identical picks. Verification showed every backup row has a surviving keeper row in `trading_picks`; no orphaned backup rows were found. Because the existing unique index prevents future duplicates at the DB layer, a nightly dedup job is not required for this exact duplicate class. If future work needs historical emission-frequency analysis, use the backup table as an audit archive rather than reintroducing duplicate live-ledger rows.
...
- Direct-script execution verified without `PYTHONPATH=.` after adding project-root `sys.path` bootstrapping
- Dedup apply verified:
  - `trading_picks`: 45,283 rows after deletion
  - `trading_picks_duplicates_backup`: 4,339 rows
  - Live/backup ID overlap: 0 rows
  - Backup rows with surviving keeper: 4,339 rows
  - Backup rows without surviving keeper: 0 rows
...
|--------|--------|---------|
| Duplicate rows | **BACKED UP + DELETED** ✓ | True per-strategy n, accurate WR/PF |
| Backup table | **CREATED** ✓ | Recoverable audit trail for all removed rows |
...
2. **Dedup & confidence:** Dedup was committed after successful backup insertion and deletion. Rollback is possible from `trading_picks_duplicates_backup`.
...
- Dedup rollback SQL:
  ...
  SELECT b.*
  FROM trading_picks_duplicates_backup b
  LEFT JOIN trading_picks tp ON tp.id = b.id
  WHERE tp.id IS NULL;
  ```
...
| [`tools/mysql_dedup_fix.py`](tools/mysql_dedup_fix.py) | Credential + backup fix | Migrated from `AUDIT_DB_PASS` env var to `tools.db_env.get_stocks_creds()` with fallback; added direct-script import bootstrap; added `trading_picks_duplicates_backup` creation before delete |
...
- [x] Execute Phase 2 (dedup — `mysql_dedup_fix.py --apply`; backup created first)
...
- [x] Verify post-execution metrics for dedup + FOREX clamp
- [ ] Commit to main
...
1. **Data Loss Risk:** Dedup rows were backed up before deletion into `trading_picks_duplicates_backup`; all 4,339 backed-up rows have a surviving keeper and 0 orphaned backup rows were found. Oldest row is always kept (preserves original entry time).
...
5. **Credential Fix:** `mysql_dedup_fix.py` now uses `tools.db_env.get_stocks_creds()` which resolves credentials from `DB_PASSWORDS_JSON` (canonical), `MYSQL_PASSWORD`, `DB_STOCKS_PASSWORD`, `DB_PASS_STOCKS`, or `AUDIT_DB_PASS` in priority order. Direct `python3 tools/mysql_dedup_fix.py` execution was verified without requiring `PYTHONPATH=.`.

6. **Nightly Dedup Decision:** Not needed for the exact duplicate class because `uq_trading_picks_dedup` already exists and blocks future identical duplicate rows. Recommended nightly action is an audit-only metric that alerts if duplicate count rises above zero, not a recurring delete job.

7. **KNOWN_GHOST_COHORTS Empty:** The `cleanup_ghost_rows.py` tool's `KNOWN_GHOST_COHORTS` list is empty. It needs to be populated from `tools/ghost_sweep_2026_05_08.py` before Phase 1 and Phase 5 can execute. This is a prerequisite task.

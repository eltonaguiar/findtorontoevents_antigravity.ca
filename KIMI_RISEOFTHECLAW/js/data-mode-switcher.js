...
function getBasePath() {
    ...
    // GitHub Pages: /findtorontoevents_antigravity.ca/riseoftheclaw.html
    var ghMatch = path.match(/^(\/[^/]+\/)riseoftheclaw/);
    if (ghMatch) return ghMatch[1] + 'riseoftheclaw/';
    // Direct host: /riseoftheclaw/... or /riseoftheclaw.html
    if (path.indexOf('riseoftheclaw') !== -1) return '/riseoftheclaw/';
    return '';
}
...
async function fetchBacktestedData() {
    try {
        var cacheBust = '?_=' + Date.now();
        var basePath = getBasePath();
        var resp = await fetch(basePath + 'data/dashboard_data.json' + cacheBust);
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        var data = await resp.json();

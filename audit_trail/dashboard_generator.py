...
def generate():
    ...
    # -- Honest Kill Switch metrics (auto-kill blacklist impact) --
    # Reads honest_kill_switch.json and blacklist_impact_simulation.json
    # generated by alpha_engine/honest_kill_switch.py and
    # tools/blacklist_impact_simulation.py (via honest-kill-switch.yml cron).
    try:
        _hks_path = ROOT / "alpha_engine" / "data" / "honest_kill_switch.json"
        _bis_path = ROOT / "reports" / "blacklist_impact_simulation.json"
        _kill_data = {}
        _sim_data = {}
        if _hks_path.exists():
            _kill_data = json.loads(_hks_path.read_text(encoding="utf-8", errors="replace"))
        if _bis_path.exists():
            _sim_data = json.loads(_bis_path.read_text(encoding="utf-8", errors="replace"))
        if _kill_data or _sim_data:
            _ks = _kill_data.get("summary", {})
            _delta = _sim_data.get("delta", {})
            payload["kill_switch"] = {
                "generated_at": _kill_data.get("generated_at"),
                "total_strategies": _ks.get("total_strategies", 0),
                "evaluated": _ks.get("evaluated", 0),
                "survivors": _ks.get("survivors", 0),
                "killed": _ks.get("killed", 0),
                "kill_rate_pct": _ks.get("kill_rate", 0),
                "criteria": _ks.get("criteria", {}),
                "impact": {
                    "wr_lift_pp": _delta.get("wr_lift_pp", 0),
                    "pf_lift": _delta.get("pf_lift", 0),
                    "pnl_damage_removed": _delta.get("pnl_damage_removed", 0),
                    "trades_removed_pct": _delta.get("trades_removed_pct", 0),
                    "verdict": _sim_data.get("verdict", "N/A"),
                    "baseline_wr": _sim_data.get("baseline", {}).get("wr", 0),
                    "post_wr": _sim_data.get("post_blacklist", {}).get("wr", 0),
                    "baseline_pf": _sim_data.get("baseline", {}).get("pf", 0),
                    "post_pf": _sim_data.get("post_blacklist", {}).get("pf", 0),
                },
                "killed_strategies": [
                    k.get("strategy", "") for k in _kill_data.get("killed", [])
                ][:30],
            }
            _ks_killed = _ks.get("killed", 0)
            _ks_wr = _delta.get("wr_lift_pp", 0)
            print("  [KILL SWITCH] Injected metrics: %d killed, WR lift +%.2fpp" % (_ks_killed, _ks_wr))
    except Exception as _ks_err:
        print("  [KILL SWITCH] Metrics injection skipped (non-fatal): %s" % _ks_err)

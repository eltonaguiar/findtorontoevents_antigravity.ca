...
**ML LAYER = HALTED + fabricated WRs (now fixed).** elite_scorer `ML_PROVEN_STRATEGIES` emptied; `ml_strategy_reviver` gated off; both were re-injecting/force-scoring 28%-WR losers. OPEN: gate ml_crypto_predictor emission; wire/retire ml_gatekeeper (shadow + synthetic A/B log); restart ml_health monitor.

...
## VETTING NOTES (Claude Code — 2026-05-16)

### Code Reference Verification: ALL PASS ✅

Every file path and line number in this document was verified against the actual repo:

| Reference | File | Lines | Verdict |
|-----------|------|-------|---------|
| `win_probability >= 0.65` | `KIMI_FEB172026/live_scanner.py:306` | 300-315 | ✅ EXACT MATCH |
| `recommend_position_size` tiers | `KIMI_FEB172026/ml_signal_ranker.py:482-497` | 482-500 | ✅ EXACT MATCH |
| `feature_names` 24-feature list | `KIMI_FEB172026/ml_signal_ranker.py:99-108` | 99-115 | ✅ EXACT MATCH |
| `ELIMINATION_RULES` dict | `KIMI_FEB172026/elimination_engine.py:20-28` | 20-32 | ✅ EXACT MATCH |
| Regime weights | `KIMI_RISEOFTHECLAW/alpha_engine_v2.py:559-566` | 559-575 | ✅ EXACT MATCH |
| `MIN_CONFIDENCE=65`, `MIN_RR=2.0` | `KIMI_RISEOFTHECLAW/alpha_engine_v2.py:76-78` | 74-80 | ✅ EXACT MATCH |
| Pump detector criteria | `KIMI_FEB172026/crypto_acceleration_engine.py:95-98` | 95-110 | ✅ EXACT MATCH |

**This is MiniMax's most credible document.** They read actual production code, not just dashboard data.

### Important Caveats

1. **Wire-up status unknown:** The KIMI directories contain research/experimental code. Check whether these systems are wired into the live production pick path (`alpha_engine/production_scanner.py`, `audit_trail/quality_gates.py`). Code existing ≠ code running live.

2. **COMMODITY filter (Section 7.1):** "cot_positioning_CT_locked LONG → 89.8% WR, PF 13.1" — this comes from the dashboard, not from code review. Our OOS dataset has n=0 COT picks. The filter parameters are plausible but the performance claim is unverified.

3. **ml_score_min=0.65 in UI vs live pipeline:** The `win_probability >= 0.65` gate in `live_scanner.py` is the right threshold — but our OOS data shows the picks passing this gate are under `source_system = kimi_signal_tracking` and `aggregated_picks`, NOT under "ml_enhanced" labels.

### Recommended Implementation Priority

Based on code verification + OOS results, the highest-ROI filters to expose in the UI are:

1. **`source_system` filter** — Allow user to select only T1 systems: `kimi_signal_tracking`, `aggregated_picks` (OOS PF 7.0-15.9)
2. **`win_probability >= 0.65`** — Already implemented in `live_scanner.py:306`; expose as UI slider
3. **`MIN_CONFIDENCE = 65, MIN_RR = 2.0`** — Already in `alpha_engine_v2.py:76-78`; expose as defaults
4. **Tournament league filter** — `CHAMPIONS_THRESHOLD=75` from `elimination_engine.py:28`; surface in dashboard
5. **FOREX exclude toggle** — Default ON based on OOS PF=0.27 confirmed sub-floor

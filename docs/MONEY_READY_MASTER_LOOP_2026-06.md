...
## CYCLE-2 STATE UPDATE — 2026-06-13 (supersedes the 2026-06-11 §current-state snapshot)
- **0/9 still; class numbers flat 24h** (CRYPTO 1155/32.4/0.73, COMMODITY 115/34.8/1.05, FOREX 95/41.1/1.10, EQUITY 119/34.5/0.46). The 90d intrabar backlog is EXHAUSTED — new resolutions now arrive at LIVE emission rate, so accrual (time) is the lever, not re-replay.
- **FIRST QUANTIFIED LEAD:** `luxalgo_confluence × CRYPTO × SHORT` (H-20260612-luxalgo_confluence_v2_short) — honest intrabar n=47, WR 66%, PF 1.888, 16 real losses; **cluster-bootstrap PF CI-LB = 1.09 (n_eff 44.6, ρ 0.45).** A statistically real positive edge (first sleeve to clear CI-LB>1.0) but BELOW the sizing bar (needs CI-LB>1.15 AND n_eff≥80). Front-runner of the forward calendar; do NOT size — re-run CI-LB at n_eff≈80. Fed by P0C (#570: block CRYPTO LONG, keep SHORT emitting).
- **Measurement hardening landed since the edition snapshot:** emission dup-fix at the sync chokepoint (628aaa5c31; 6h post-fix window 8.7% vs trailing-7d 73.6%); #132 walk-forward now reads 1,607 honest intrabar rows (was a 512-row JSON); #134 A/B dual-write WIRED (ffe555ad34, was defined-but-never-called) — verify engagement next hourly; 22k UNKNOWN-class rows reclassified; 141k kimi pollution purged. H1 GREEN both focus classes.
- **Open measurement risks (peers fixing in parallel):** INCIDENT#138 KIMI JSON-silo dashboard bypass (opencode); INCIDENT_STOCKS#15 regime-guard reads intraday-rebounding market_regime not authoritative risk_off_explanation (corrects the earlier P0-2 "VERIFIED"); non_crypto_consensus PF 9.68 has 0 honest-intrabar rows (NOT a FOREX winner — caution in DAILY_IDEAS). alpha_engine/config.py is a 3-way merge contention.
- **Do-not-relitigate additions (this cycle):** snipe_spike_30m (both arms closed, H-113); non_crypto_consensus as a "FOREX lead" without intrabar adjudication.

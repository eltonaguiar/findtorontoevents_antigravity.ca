...
import sys
sys.path.insert(0, '.')
from alpha_engine.grid_trading_incubator import GridTrader
...
def backtest_strategy(df, interval):
    ...
    grid_sizes = {'1h': 0.005, '4h': 0.01, '1d': 0.02}
    grid_size = grid_sizes[interval]
    trader = GridTrader(initial_cash=initial_capital, grid_size=grid_size, num_levels=10)
    ...
    num_trades = trader.num_fills_buy + trader.num_fills_sell
    open_pos = len(trader.open_longs) + len(trader.open_shorts)
...
if __name__ == '__main__':
    ...
    # Generate HTML table
    html = '<!DOCTYPE html><html><head><title>Grid Trading Incubator Backtest</title><style>table {{border-collapse: collapse;}} th, td {{border: 1px solid black; padding: 8px; text-align: center;}}</style></head><body><h1>Grid Trading Incubator Backtest Results</h1><table><tr><th>Symbol/TF</th><th>Total Return</th><th>Sharpe</th><th>Sortino</th><th>Max DD</th><th>Num Trades</th><th>Open Pos</th><th>Real PnL%</th><th>Calmar</th><th>Bars</th></tr>'
    ...
    with open('grid_results.html', 'w') as f:
        f.write(html)
    print("Full grid results saved to grid_results.json and grid_results.html")
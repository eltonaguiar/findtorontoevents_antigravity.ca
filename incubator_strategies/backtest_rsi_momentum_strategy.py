    ...
    for i in range(len(df)):
        ...
        if position_open:
            ...
            if current_price <= sl_price:
                exit_price = current_price * (1 - slippage)  # sell lower adverse
                exit_reason = 'SL'
                exit_triggered = True
            # Signal exit
            ...
            if signals['entry_long'].iloc[i]:
                ...
                entry_price = current_price * (1 + slippage)  # buy higher adverse
...
if __name__ == '__main__':
    ...
    for symbol in symbols:
        for interval in intervals:
            ...
            results[key] = res
            print(f"RSI Momentum {key}: Sharpe {res['sharpe']:.2f}, Winrate {res['winrate']:.1%}, Trades {res['num_trades']}, Return {res['total_return']:.2%} vs BuyHold {res['buy_hold_return']:.2%}")

    # Save JSON
    with open('incubator_strategies/rsi_momentum_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    # Generate HTML table
    html = '''<!DOCTYPE html><html><head><title>RSI Momentum Strategy Backtest</title></head><body><h1>RSI Momentum Backtest Results</h1><table border="1" style="border-collapse:collapse;"><tr><th>Symbol/TF</th><th>Return</th><th>Sharpe</th><th>Sortino</th><th>Winrate</th><th>PF</th><th>Max DD</th><th>Trades</th><th>Avg Trade</th><th>Calmar</th><th>Buy&Hold</th></tr>'''
    for key, r in results.items():
        html += f'<tr><td>{key}</td><td>{r["total_return"]:.2%}</td><td>{r["sharpe"]:.2f}</td><td>{r["sortino"]:.2f}</td><td>{r["winrate"]:.1%}</td><td>{r["profit_factor"]:.2f}</td><td>{r["max_dd"]:.2%}</td><td>{r["num_trades"]}</td><td>{r["avg_trade"]:.2%}</td><td>{r["calmar"]:.2f}</td><td>{r["buy_hold_return"]:.2%}</td></tr>'
    html += '</table></body></html>'
    with open('incubator_strategies/rsi_momentum_results.html', 'w') as f:
        f.write(html)

    print("Results saved to incubator_strategies/rsi_momentum_results.json and .html")
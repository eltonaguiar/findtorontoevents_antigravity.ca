"""Mercury Conservative - Tight RSI thresholds with wider stops for high-conviction entries."""
...
class MercuryConservative(BaseStrategy):
    name = "mercury_conservative"
    display_name = "Mercury Conservative"
    source = "Multi-Source"
    category = "crypto"
    ...
    def generate_picks(self, data: dict) -> List[NormalizedPick]:
        ...
        for symbol, klines in data.items():
            ...
            # Conservative: RSI < 20 for LONG, > 80 for SHORT (extreme only, 3-bar lookback)
            direction = None
            if cross_up and price > ema200[-1] and rsi_recent_low < 20:
                direction = "LONG"
            elif cross_down and price < ema200[-1] and rsi_recent_high > 80:
                direction = "SHORT"

            if direction is None:
                continue

            # Conservative: wider stops (3x ATR), generous TP (4.5x ATR) = 1.5:1 RR
            if direction == "LONG":
                tp = round(price + 4.5 * atr_now, 6)
                sl = round(price - 3 * atr_now, 6)
            else:
                tp = round(price - 4.5 * atr_now, 6)
                sl = round(price + 3 * atr_now, 6)
            ...
            confidence = min(0.9, 0.55 + ema_gap * 10 + rsi_dist * 0.2)
            ...
            picks.append(NormalizedPick(
                ...
                reason=(
                    f"EMA(9)xEMA(21) {direction} [conservative] | "
                    f"price {'>' if direction == 'LONG' else '<'} EMA(200) | "
                    f"RSI={rsi_now:.1f} (extreme) | ATR={atr_now:.4f}"
                ),
                raw_signal={
                    "ema9": round(ema9[-1], 4),

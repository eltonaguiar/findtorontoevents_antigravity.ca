"""Mercury Vol-Adjusted Crossover - EMA(9)/EMA(21) cross with RSI extremes and ATR targets."""
...
class MercuryVolCrossover(BaseStrategy):
    name = "mercury_vol_crossover"
    display_name = "Mercury Vol-Adjusted Crossover"
    source = "Multi-Source"
    category = "crypto"
    ...
    def generate_picks(self, data: dict) -> List[NormalizedPick]:
        ...
        for symbol, klines in data.items():
            ...
            picks.append(NormalizedPick(
                ...
                reason=(
                    f"EMA(9)xEMA(21) {direction} | "
                    f"price {'>' if direction == 'LONG' else '<'} EMA(200) | "
                    f"RSI={rsi_now:.1f} | ATR={atr_now:.4f}"
                ),
                raw_signal={
                    "ema9": round(ema9[-1], 4),

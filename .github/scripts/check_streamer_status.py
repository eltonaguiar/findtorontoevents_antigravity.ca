...
def main():
    ...
    # Check each streamer (collect results for batch DB update)
    batch_updates = []
    for i, streamer in enumerate(streamers):
        ...
        if status["success"]:
            ...
            # Queue for batch update instead of individual DB call
    ...
    # Batch update all streamer statuses in ONE DB call (was 50 individual calls)
    if batch_updates:
        log_message(f"\nSending batch update for {len(batch_updates)} streamers (single DB connection)...", log_file)
        batch_url = f"{API_BASE}/api/batch_update_streamer_last_seen.php"
        batch_payload = {
        ...
        for attempt in range(MAX_RETRIES):
            try:
                batch_result = _http_post_json(batch_url, batch_payload, timeout=60)
                if batch_result["status"] == 200 and batch_result["data"].get("ok"):
                    processed = batch_result["data"].get("processed", 0)
                    results["summary"]["updated"] = processed
                    log_message(f"  Batch update successful: {processed} streamers updated in one call", log_file)
                    batch_ok = True
                    break
                else:
                    log_message(f"  Batch update error: {batch_result['data'].get('error', 'unknown')}", log_file)
            except Exception as e:
                log_message(f"  Batch update attempt {attempt + 1} failed: {e}", log_file)
                if attempt < MAX_RETRIES - 1:
                    time.sleep(RETRY_DELAY)

        if not batch_ok:
            log_message("  Batch update failed after all retries, falling back to individual updates", log_file)
            for update in batch_updates:
                try:
                    updated = update_last_seen(
                        {"creator_id": update["creator_id"], "creator_name": update["creator_name"],
                         "platform": update["platform"], "username": update["username"], "url": update["account_url"]},
                        {"is_live": update["is_live"], "stream_title": update["stream_title"], "viewer_count": update["viewer_count"]}
                    )
                    if updated:

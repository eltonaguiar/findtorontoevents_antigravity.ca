"""
Crypto Bundle Range Expansion Alpha - Baby Strat

Design reference:
- Toby Crabel NR expansion
- Trend-following continuation framing
"""
import numpy as np
import pandas as pd
...
class CryptoBundleRangeExpansionAlphaStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        if px > mother_high and is_bull and float(close.iloc[-1]) > float(open_.iloc[-1]):
            ...
            signals.append(self._mk(symbol, "BUY", px, atr_now, conf, "Bundle alpha upside range thrust"))
        elif px < mother_low and is_bear and float(close.iloc[-1]) < float(open_.iloc[-1]):
            conf = min(0.95, 0.55 + min(0.28, (vol_ratio - self.vol_mult) / 1.8))
            signals.append(self._mk(symbol, "SELL", px, atr_now, conf, "Bundle alpha downside range thrust"))
        return signals
...
if __name__ == "__main__":
    ...
    prices = 14000 * np.exp(np.cumsum(returns))
    df = pd.DataFrame(
        {
    ...
    s = CryptoBundleRangeExpansionAlphaStrategy()

"""
Crypto Short-Term NR+ER OBV Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNREROBVIgnitionStrategy:
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.obv_ema_period = self.params.get("obv_ema_period", 21)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (float(obv.iloc[-1]) > float(obv_ema.iloc[-1])):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (float(obv.iloc[-1]) < float(obv_ema.iloc[-1])):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNREROBVIgnitionStrategy()

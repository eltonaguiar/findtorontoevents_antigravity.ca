"""
Crypto Short-Term NR+ER Donchian Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERDonchianIgnitionStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        d_up = high.rolling(self.donchian_period, min_periods=1).max().shift(1)
        d_dn = low.rolling(self.donchian_period, min_periods=1).min().shift(1)
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (price > float(d_up.iloc[-1])):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (price < float(d_dn.iloc[-1])):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERDonchianIgnitionStrategy()

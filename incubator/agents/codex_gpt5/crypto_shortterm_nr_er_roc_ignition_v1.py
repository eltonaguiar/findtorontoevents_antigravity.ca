"""
Crypto Short-Term NR+ER ROC Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERRocIgnitionStrategy:
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.roc_period = self.params.get("roc_period", 8)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        roc_ma = roc.rolling(self.roc_period, min_periods=1).mean()
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (roc_now > float(roc_ma.iloc[-1]) and roc_now > 0):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (roc_now < float(roc_ma.iloc[-1]) and roc_now < 0):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERRocIgnitionStrategy()

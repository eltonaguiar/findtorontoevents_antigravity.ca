"""
Crypto Range Expansion Follow-through - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoRangeExpansionFollowthroughStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        vol_ok = float(volume.iloc[-1]) > float(vol_ma.iloc[-1]) * 1.3
        bull_trend = float(ema_f.iloc[-1]) > float(ema_s.iloc[-1])
        bear_trend = float(ema_f.iloc[-1]) < float(ema_s.iloc[-1])

        if not nr_prev or not inside_prev or not vol_ok:
            return []

        signals: List[Signal] = []
        if px > mother_high and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]):
            conf = min(0.95, 0.56 + min(0.28, (float(volume.iloc[-1]) / (float(vol_ma.iloc[-1]) + 1e-12) - 1.0) / 2.0))
            signals.append(self._mk(symbol, "BUY", px, atr_now, conf, "NR/inside-bar range expansion upward"))
        elif px < mother_low and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]):
            conf = min(0.95, 0.56 + min(0.28, (float(volume.iloc[-1]) / (float(vol_ma.iloc[-1]) + 1e-12) - 1.0) / 2.0))
            signals.append(self._mk(symbol, "SELL", px, atr_now, conf, "NR/inside-bar range expansion downward"))
        return signals
...
if __name__ == "__main__":
    ...
    s = CryptoRangeExpansionFollowthroughStrategy()

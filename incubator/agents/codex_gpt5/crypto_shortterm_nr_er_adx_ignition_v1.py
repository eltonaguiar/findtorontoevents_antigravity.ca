"""
Crypto Short-Term NR+ER ADX Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERADXIgnitionStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        adx, plus_di, minus_di = self._adx_bundle(data, self.adx_period)
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (adx_now >= self.adx_min and float(plus_di.iloc[-1]) > float(minus_di.iloc[-1])):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (adx_now >= self.adx_min and float(plus_di.iloc[-1]) < float(minus_di.iloc[-1])):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
    ...
    def _adx_bundle(self, data: pd.DataFrame, period: int):
        high = data["high"]
        low = data["low"]
        close = data["close"]
        up_move = high.diff()
        ...
        plus_di = 100.0 * plus_dm.rolling(period, min_periods=1).mean() / (atr + 1e-12)
        minus_di = 100.0 * minus_dm.rolling(period, min_periods=1).mean() / (atr + 1e-12)
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERADXIgnitionStrategy()

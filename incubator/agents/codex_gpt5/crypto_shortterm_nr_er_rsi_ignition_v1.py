"""
Crypto Short-Term NR+ER RSI Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERRSIIgnitionStrategy:
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.vol_multiplier = self.params.get("vol_multiplier", 0.0)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.998 and bull_trend and (rsi_now >= 52 and rsi_now > (rsi_prev - 0.5)):
        ...
        elif price <= range_low * 1.002 and bear_trend and (rsi_now <= 48 and rsi_now < (rsi_prev + 0.5)):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERRSIIgnitionStrategy()

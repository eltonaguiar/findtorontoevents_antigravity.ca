"""
Crypto Short-Term NR+ER VWMA Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERVWMAIgnitionStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        vwma = (close * volume).rolling(self.vwma_period, min_periods=1).sum() / (volume.rolling(self.vwma_period, min_periods=1).sum() + 1e-12)
        vwma_slope = float(vwma.iloc[-1] - vwma.iloc[-2])
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (price > float(vwma.iloc[-1]) and vwma_slope > 0):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (price < float(vwma.iloc[-1]) and vwma_slope < 0):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERVWMAIgnitionStrategy()

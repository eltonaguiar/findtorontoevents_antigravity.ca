"""
Crypto Short-Term NR+ER Choppiness Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERChopIgnitionStrategy:
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.chop_max = self.params.get("chop_max", 55.0)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        tr_sum = tr.rolling(self.chop_period, min_periods=1).sum()
        range_h = high.rolling(self.chop_period, min_periods=1).max()
        range_l = low.rolling(self.chop_period, min_periods=1).min()
        chop = 100.0 * np.log10((tr_sum + 1e-12) / (range_h - range_l + 1e-12)) / np.log10(float(self.chop_period))
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (float(chop.iloc[-1]) <= self.chop_max):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (float(chop.iloc[-1]) <= self.chop_max):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERChopIgnitionStrategy()

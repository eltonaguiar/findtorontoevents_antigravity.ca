"""
Crypto Short-Term NR+ER BBands Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERBBandsIgnitionStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        bb_sigma = close.rolling(self.bb_period, min_periods=1).std().fillna(0.0)
        bb_up = bb_mid + self.bb_std * bb_sigma
        bb_dn = bb_mid - self.bb_std * bb_sigma
        bb_width = (bb_up - bb_dn) / (bb_mid.abs() + 1e-12)
        bb_w_rising = float(bb_width.iloc[-1]) > float(bb_width.iloc[-2])
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (price > float(bb_up.iloc[-1]) and bb_w_rising):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (price < float(bb_dn.iloc[-1]) and bb_w_rising):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERBBandsIgnitionStrategy()

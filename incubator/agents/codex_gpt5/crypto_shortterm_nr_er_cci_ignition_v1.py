"""
Crypto Short-Term NR+ER CCI Ignition - Baby Strat
"""
import numpy as np
import pandas as pd
...
class CryptoShortTermNRERCCIIgnitionStrategy:
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        tp_ma = tp_price.rolling(self.cci_period, min_periods=1).mean()
        md = (tp_price - tp_ma).abs().rolling(self.cci_period, min_periods=1).mean()
        cci = (tp_price - tp_ma) / (0.015 * (md + 1e-12))
        ...
        signals: List[Signal] = []
        if price >= range_high * 0.999 and bull_trend and float(close.iloc[-1]) > float(open_.iloc[-1]) and (cci_now > 0 and cci_now > cci_prev):
        ...
        elif price <= range_low * 1.001 and bear_trend and float(close.iloc[-1]) < float(open_.iloc[-1]) and (cci_now < 0 and cci_now < cci_prev):
            conf = min(0.95, 0.57 + min(0.18, (vol_ratio - self.vol_multiplier) * 0.11) + min(0.12, (er_now - self.er_threshold) * 0.25))
...
if __name__ == "__main__":
    ...
    strat = CryptoShortTermNRERCCIIgnitionStrategy()

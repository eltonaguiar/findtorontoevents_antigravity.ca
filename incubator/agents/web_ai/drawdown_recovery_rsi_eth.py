"""
DrawdownRecoveryRSI ETH — Baby Strat
=================================
...
Strategy Logic:
- Entry when: Drawdown from 50-period high > 8.0% AND RSI < 33
- Exit when: TP = 2.2 x ATR, SL = 1.3 x ATR
- Symbol: ETH only
- ETH — higher vol than BTC, slightly wider DD gate

Unique Value Proposition:
Drawdown-gated RSI recovery tuned for ETH volatility profile.
Only trades oversold AFTER real pain/drawdown — no other strategy gates on % drawdown for ETH.
"""
...
class DrawdownRecoveryRSIETHStrategy:
    """Drawdown-gated RSI recovery buys for ETH."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.dd_threshold = self.params.get('dd_threshold', 8.0)
        self.atr_period = self.params.get('atr_period', 14)
        # R:R widened 2026-03-16: was 2.2, now 3.0 (target R:R >= 2.0; was 1.69, now 2.31)
        self.tp_atr = self.params.get('tp_atr', 3.0)
        self.sl_atr = self.params.get('sl_atr', 1.3)

    def generate_signals(self, data: pd.DataFrame, symbol: str = "ETHUSDT") -> List[Signal]:

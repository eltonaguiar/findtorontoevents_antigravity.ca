"""
crypto_soc_intraday_time_slices_a10_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: intraday_time_slices
...
class CryptoSoc095IntradayTimeSlicesStrategy:
    """Composite social-theme strategy: intraday_time_slices."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.trend_w = self.p.get('trend_w', 0.2751)
        self.meanrev_w = self.p.get('meanrev_w', 0.0985)
        self.breakout_w = self.p.get('breakout_w', 0.1458)
        self.volstate_w = self.p.get('volstate_w', 0.1289)
        self.flow_w = self.p.get('flow_w', 0.1245)
        self.session_w = self.p.get('session_w', 0.2273)

        self.long_threshold = self.p.get('long_threshold', 0.490)
        self.short_threshold = self.p.get('short_threshold', 0.490)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'intraday_time_slices comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'intraday_time_slices comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc095IntradayTimeSlicesStrategy()
    total = 0

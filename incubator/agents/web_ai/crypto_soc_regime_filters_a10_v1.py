"""
crypto_soc_regime_filters_a10_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: regime_filters
...
class CryptoSoc091RegimeFiltersStrategy:
    """Composite social-theme strategy: regime_filters."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.trend_w = self.p.get('trend_w', 0.3716)
        self.meanrev_w = self.p.get('meanrev_w', 0.0985)
        self.breakout_w = self.p.get('breakout_w', 0.1264)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'regime_filters comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'regime_filters comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc091RegimeFiltersStrategy()
    total = 0

"""
crypto_soc_trend_filtered_meanrev_a08_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================
...
class CryptoSoc077TrendFilteredMeanrevStrategy:
    """Composite social-theme strategy: trend_filtered_meanrev."""

    def __init__(self, params: Optional[Dict] = None):
        self.p = params or {}
        self.fast_ema = self.p.get('fast_ema', 14)
        self.slow_ema = self.p.get('slow_ema', 40)
        self.z_window = self.p.get('z_window', 30)
        self.breakout_window = self.p.get('breakout_window', 21)
        self.vol_window = self.p.get('vol_window', 33)
        self.atr_short = self.p.get('atr_short', 8)
        self.atr_long = self.p.get('atr_long', 38)
        self.atr_gate = self.p.get('atr_gate', 1.080)
        self.vol_ratio_gate = self.p.get('vol_ratio_gate', 1.150)
        self.tp_atr = self.p.get('tp_atr', 2.65)
        self.sl_atr = self.p.get('sl_atr', 1.10)
        self.cooldown = self.p.get('cooldown', 3)

        self.trend_w = self.p.get('trend_w', 0.2794)
        self.meanrev_w = self.p.get('meanrev_w', 0.3235)
        self.breakout_w = self.p.get('breakout_w', 0.0735)
        self.volstate_w = self.p.get('volstate_w', 0.1446)
        self.flow_w = self.p.get('flow_w', 0.1029)
        self.session_w = self.p.get('session_w', 0.0760)

        self.long_threshold = self.p.get('long_threshold', 0.440)
        self.short_threshold = self.p.get('short_threshold', 0.440)
...
if __name__ == '__main__':
    ...
    s = CryptoSoc077TrendFilteredMeanrevStrategy()
    total = 0

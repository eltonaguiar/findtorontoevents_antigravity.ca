"""
crypto_soc_proxy_decoupling_a07_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: proxy_decoupling
...
class CryptoSoc069ProxyDecouplingStrategy:
    """Composite social-theme strategy: proxy_decoupling."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.trend_w = self.p.get('trend_w', 0.3113)
        self.meanrev_w = self.p.get('meanrev_w', 0.0731)
        self.breakout_w = self.p.get('breakout_w', 0.1156)
        self.volstate_w = self.p.get('volstate_w', 0.2218)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'proxy_decoupling comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'proxy_decoupling comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc069ProxyDecouplingStrategy()
    total = 0

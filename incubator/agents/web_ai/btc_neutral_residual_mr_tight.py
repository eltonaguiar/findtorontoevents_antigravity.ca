"""
BTC-Neutral Residual Mean Reversion — TIGHT MUTATION
======================================================
...
class BtcNeutralResidualMRTightStrategy:
    """Tight mutation: TP=1.4x ATR, SL=1.2x ATR."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.tp_atr = self.p.get("tp_atr", 1.4)   # MUTATED: 2.0 -> 1.4
        self.sl_atr = self.p.get("sl_atr", 1.2)    # MUTATED: 1.5 -> 1.2
    ...
    def generate_signals(self, data, symbol="ETHUSDT", btc_data=None):
        ...
        if current_z < -self.z_entry:
            ...
            signals.append(Signal(
                ...
                reason=f"BTC-Neutral ResidZ={current_z:.2f} LONG_TIGHT",
            ))
        ...
        if current_z > self.z_entry:
            ...
            signals.append(Signal(
                ...
                take_profit=round(current_price - current_atr * self.tp_atr, 6),
                stop_loss=round(current_price + current_atr * self.sl_atr, 6),
                reason=f"BTC-Neutral ResidZ={current_z:.2f} SHORT_TIGHT",
            ))

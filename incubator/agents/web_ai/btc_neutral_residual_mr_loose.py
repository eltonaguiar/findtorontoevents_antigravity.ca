"""
BTC-Neutral Residual Mean Reversion — LOOSE MUTATION
======================================================
...
class BtcNeutralResidualMRLooseStrategy:
    """Loose mutation: z_entry 2.0 -> 1.7."""

    def __init__(self, params: Optional[Dict] = None):
        self.p = params or {}
        self.reg_window = self.p.get("regression_window", 30)
        self.z_window = self.p.get("zscore_window", 20)
        self.z_entry = self.p.get("z_entry", 1.7)   # MUTATED: 2.0 -> 1.7
    ...
    def generate_signals(self, data, symbol="ETHUSDT", btc_data=None):
        ...
        if current_z < -self.z_entry:
            ...
            signals.append(Signal(
                ...
                reason=f"BTC-Neutral ResidZ={current_z:.2f} LONG_LOOSE",
            ))
        ...
        if current_z > self.z_entry:
            ...
            signals.append(Signal(
                ...
                take_profit=round(current_price - current_atr * self.tp_atr, 6),
                stop_loss=round(current_price + current_atr * self.sl_atr, 6),
                reason=f"BTC-Neutral ResidZ={current_z:.2f} SHORT_LOOSE",
            ))

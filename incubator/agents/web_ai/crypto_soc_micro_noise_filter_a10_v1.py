"""
crypto_soc_micro_noise_filter_a10_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: micro_noise_filter
...
class CryptoSoc098MicroNoiseFilterStrategy:
    """Composite social-theme strategy: micro_noise_filter."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.breakout_w = self.p.get('breakout_w', 0.1071)
        self.volstate_w = self.p.get('volstate_w', 0.2833)
        self.flow_w = self.p.get('flow_w', 0.1824)
        self.session_w = self.p.get('session_w', 0.0922)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'micro_noise_filter comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'micro_noise_filter comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc098MicroNoiseFilterStrategy()
    total = 0

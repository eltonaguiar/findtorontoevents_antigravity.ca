"""
ADX-Bollinger Regime Switch — LOOSE MUTATION
==============================================
DNA Mutation B: Looser entry (more signals)
- RSI oversold: 35 -> 40 (+14%)
- RSI overbought: 65 -> 60 (-8%)
- ADX idle: 15 -> 12 (-20%)
...
class ADXBollingerRegimeSwitchLooseStrategy:
    """Loose mutation: relaxed RSI and ADX thresholds."""

    def __init__(self, params: Optional[Dict] = None):
        self.params = params or {}
        self.adx_period = self.params.get('adx_period', 14)
        self.adx_idle = self.params.get('adx_idle', 12)       # MUTATED: 15 -> 12
        self.adx_breakout = self.params.get('adx_breakout', 25)
        self.bb_period = self.params.get('bb_period', 20)
        self.bb_std = self.params.get('bb_std', 2.2)
        self.rsi_period = self.params.get('rsi_period', 14)
        self.rsi_os = self.params.get('rsi_oversold', 40)      # MUTATED: 35 -> 40
        self.rsi_ob = self.params.get('rsi_overbought', 60)    # MUTATED: 65 -> 60
            ...
            if cur_adx <= self.adx_breakout:
                ...
                if cur_close <= cur_bb_lower and cur_rsi < self.rsi_os:
                    signals.append(Signal(
                        ...
                        reason=f"MR_BUY_LOOSE ADX={cur_adx:.1f} RSI={cur_rsi:.1f}",
                        mode="MEAN_REV"
                    ))
                    last_signal_bar = i
                    ...
                    signals.append(Signal(
                        ...
                        take_profit=round(cur_bb_mid, 2),
                        stop_loss=round(cur_close + cur_atr * self.mr_sl_atr, 2),
                        reason=f"MR_SELL_LOOSE ADX={cur_adx:.1f} RSI={cur_rsi:.1f}",
                        mode="MEAN_REV"
                    ))
                    last_signal_bar = i
            else:
                ...
                if cur_close > cur_bb_upper:
                    signals.append(Signal(
                        ...
                        reason=f"BO_LONG_LOOSE ADX={cur_adx:.1f}",
                        mode="BREAKOUT"
                    ))
                    last_signal_bar = i
                    ...
                    signals.append(Signal(
                        ...
                        take_profit=round(cur_close - cur_atr * self.bo_tp_atr, 2),
                        stop_loss=round(cur_close + cur_atr * self.bo_sl_atr, 2),
                        reason=f"BO_SHORT_LOOSE ADX={cur_adx:.1f}",
                        mode="BREAKOUT"
                    ))
                    last_signal_bar = i

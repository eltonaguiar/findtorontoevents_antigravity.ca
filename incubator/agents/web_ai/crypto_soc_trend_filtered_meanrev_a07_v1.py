"""
crypto_soc_trend_filtered_meanrev_a07_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: trend_filtered_meanrev
...
class CryptoSoc067TrendFilteredMeanrevStrategy:
    """Composite social-theme strategy: trend_filtered_meanrev."""

    def __init__(self, params: Optional[Dict] = None):
        self.p = params or {}
        self.fast_ema = self.p.get('fast_ema', 12)
        self.slow_ema = self.p.get('slow_ema', 34)
        self.z_window = self.p.get('z_window', 26)
        self.breakout_window = self.p.get('breakout_window', 18)
        self.vol_window = self.p.get('vol_window', 28)
        self.atr_short = self.p.get('atr_short', 11)
        self.atr_long = self.p.get('atr_long', 34)
        self.atr_gate = self.p.get('atr_gate', 1.040)
        self.vol_ratio_gate = self.p.get('vol_ratio_gate', 1.100)
        self.tp_atr = self.p.get('tp_atr', 2.40)
        self.sl_atr = self.p.get('sl_atr', 1.70)
        self.cooldown = self.p.get('cooldown', 2)

        self.trend_w = self.p.get('trend_w', 0.2717)
        self.meanrev_w = self.p.get('meanrev_w', 0.3300)
        self.breakout_w = self.p.get('breakout_w', 0.0761)
        self.volstate_w = self.p.get('volstate_w', 0.1428)
        self.flow_w = self.p.get('flow_w', 0.1018)
        self.session_w = self.p.get('session_w', 0.0776)

        self.long_threshold = self.p.get('long_threshold', 0.430)
        self.short_threshold = self.p.get('short_threshold', 0.430)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'trend_filtered_meanrev comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'trend_filtered_meanrev comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc067TrendFilteredMeanrevStrategy()
    total = 0

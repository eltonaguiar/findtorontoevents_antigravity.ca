"""
crypto_soc_mtf_orb_pivots_a07_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: mtf_orb_pivots
...
class CryptoSoc066MtfOrbPivotsStrategy:
    """Composite social-theme strategy: mtf_orb_pivots."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.trend_w = self.p.get('trend_w', 0.2915)
        self.meanrev_w = self.p.get('meanrev_w', 0.0929)
        self.breakout_w = self.p.get('breakout_w', 0.2737)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'mtf_orb_pivots comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'mtf_orb_pivots comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc066MtfOrbPivotsStrategy()
    total = 0

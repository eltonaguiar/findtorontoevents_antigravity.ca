"""
crypto_soc_trend_filtered_meanrev_a06_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================
...
class CryptoSoc057TrendFilteredMeanrevStrategy:
    """Composite social-theme strategy: trend_filtered_meanrev."""

    def __init__(self, params: Optional[Dict] = None):
        self.p = params or {}
        self.fast_ema = self.p.get('fast_ema', 10)
        self.slow_ema = self.p.get('slow_ema', 70)
        self.z_window = self.p.get('z_window', 22)
        self.breakout_window = self.p.get('breakout_window', 15)
        self.vol_window = self.p.get('vol_window', 23)
        self.atr_short = self.p.get('atr_short', 10)
        self.atr_long = self.p.get('atr_long', 30)
        self.atr_gate = self.p.get('atr_gate', 1.000)
        self.vol_ratio_gate = self.p.get('vol_ratio_gate', 1.050)
        self.tp_atr = self.p.get('tp_atr', 2.15)
        self.sl_atr = self.p.get('sl_atr', 1.50)
        self.cooldown = self.p.get('cooldown', 1)

        self.trend_w = self.p.get('trend_w', 0.2639)
        self.meanrev_w = self.p.get('meanrev_w', 0.3367)
        self.breakout_w = self.p.get('breakout_w', 0.0787)
        self.volstate_w = self.p.get('volstate_w', 0.1409)
        self.flow_w = self.p.get('flow_w', 0.1006)
        self.session_w = self.p.get('session_w', 0.0792)

        self.long_threshold = self.p.get('long_threshold', 0.420)
        self.short_threshold = self.p.get('short_threshold', 0.420)
...
if __name__ == '__main__':
    ...
    s = CryptoSoc057TrendFilteredMeanrevStrategy()
    total = 0

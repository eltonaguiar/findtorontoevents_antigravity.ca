"""
Volume Climax Reversal — INVERSE MUTATION
==========================================
DNA Mutation C: Flip direction
- Original SHORT on bearish climax -> now BUY (momentum continuation)
- Original LONG on bullish climax -> now SELL (momentum continuation)
...
class VolumeClimaxReversalInverseStrategy:
    """Inverse mutation: trade WITH climax direction instead of against it."""
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        for i in range(min_len, len(data)):
            ...
            # INVERSE: bearish candle + RSI > 50 -> BUY (momentum continuation down then bounce)
            if cur_close < cur_open and cur_rsi > 50:
                ...
                signals.append(Signal(
                    ...
                    reason=f"VOL_CLIMAX_INV_LONG vol={vol_ratio:.1f}x RSI={cur_rsi:.1f}"
                ))
                last_signal_bar = i
            # INVERSE: bullish candle + RSI < 50 -> SELL
                ...
                signals.append(Signal(
                    ...
                    take_profit=round(cur_close - cur_atr * self.tp_atr, 2),  # INVERSE
                    stop_loss=round(cur_close + cur_atr * self.sl_atr, 2),    # INVERSE
                    reason=f"VOL_CLIMAX_INV_SHORT vol={vol_ratio:.1f}x RSI={cur_rsi:.1f}"
                ))
                last_signal_bar = i

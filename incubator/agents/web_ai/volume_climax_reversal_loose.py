"""
Volume Climax Reversal — LOOSE MUTATION
========================================
DNA Mutation B: Looser entry (more signals)
- Volume multiplier: 3.0 -> 2.5 (-17%)
...
class VolumeClimaxReversalLooseStrategy:
    """Loose mutation: vol_multiplier 3.0 -> 2.5."""

    def __init__(self, params: Optional[Dict] = None):
        self.params = params or {}
        self.vol_sma_period = self.params.get('vol_sma_period', 20)
        self.vol_multiplier = self.params.get('vol_multiplier', 2.5)  # MUTATED: 3.0 -> 2.5
        ...
        for i in range(min_len, len(data)):
            ...
            if cur_close < cur_open and cur_rsi > 50:
                ...
                signals.append(Signal(
                    ...
                    reason=f"VOL_CLIMAX_SHORT_LOOSE vol={vol_ratio:.1f}x RSI={cur_rsi:.1f}"
                ))
                last_signal_bar = i
                ...
                signals.append(Signal(
                    ...
                    take_profit=round(cur_close + cur_atr * self.tp_atr, 2),
                    stop_loss=round(cur_close - cur_atr * self.sl_atr, 2),
                    reason=f"VOL_CLIMAX_LONG_LOOSE vol={vol_ratio:.1f}x RSI={cur_rsi:.1f}"
                ))
                last_signal_bar = i

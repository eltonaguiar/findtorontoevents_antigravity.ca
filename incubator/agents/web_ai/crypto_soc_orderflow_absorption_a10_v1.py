"""
crypto_soc_orderflow_absorption_a10_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: orderflow_absorption
...
class CryptoSoc093OrderflowAbsorptionStrategy:
    """Composite social-theme strategy: orderflow_absorption."""

    def __init__(self, params: Optional[Dict] = None):
        self.p = params or {}
        self.fast_ema = self.p.get('fast_ema', 18)
        self.slow_ema = self.p.get('slow_ema', 52)
        self.z_window = self.p.get('z_window', 18)
        self.breakout_window = self.p.get('breakout_window', 27)
        self.vol_window = self.p.get('vol_window', 18)
        self.atr_short = self.p.get('atr_short', 10)
        self.atr_long = self.p.get('atr_long', 46)
        self.atr_gate = self.p.get('atr_gate', 1.160)
        self.vol_ratio_gate = self.p.get('vol_ratio_gate', 1.000)
        self.tp_atr = self.p.get('tp_atr', 1.90)
        self.sl_atr = self.p.get('sl_atr', 1.50)
        self.cooldown = self.p.get('cooldown', 2)

        self.trend_w = self.p.get('trend_w', 0.2365)
        self.meanrev_w = self.p.get('meanrev_w', 0.0792)
        self.breakout_w = self.p.get('breakout_w', 0.0878)
        self.volstate_w = self.p.get('volstate_w', 0.1482)
        self.flow_w = self.p.get('flow_w', 0.3755)
        self.session_w = self.p.get('session_w', 0.0729)

        self.long_threshold = self.p.get('long_threshold', 0.480)
        self.short_threshold = self.p.get('short_threshold', 0.480)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'orderflow_absorption comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'orderflow_absorption comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc093OrderflowAbsorptionStrategy()
    total = 0

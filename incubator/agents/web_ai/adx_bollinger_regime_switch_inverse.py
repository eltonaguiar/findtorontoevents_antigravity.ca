"""
ADX-Bollinger Regime Switch — INVERSE MUTATION
================================================
DNA Mutation C: Flip all directions
- MR BUY at lower BB -> MR SELL at lower BB (breakdown continuation)
- MR SELL at upper BB -> MR BUY at upper BB (breakout continuation)
- BO LONG above upper BB -> BO SHORT above upper BB (fade breakout)
- BO SHORT below lower BB -> BO LONG below lower BB (fade breakdown)
...
class ADXBollingerRegimeSwitchInverseStrategy:
    """Inverse mutation: flip all trade directions."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.mr_sl_atr = self.params.get('mr_sl_atr', 1.5)
        self.bo_tp_atr = self.params.get('bo_tp_atr', 2.5)
        self.bo_sl_atr = self.params.get('bo_sl_atr', 1.2)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = "BTCUSDT") -> List[Signal]:
        ...
        for i in range(min_len, len(data)):
            ...
            if cur_adx <= self.adx_breakout:
                ...
                # INVERSE: lower BB + oversold -> SELL (continuation breakdown)
                if cur_close <= cur_bb_lower and cur_rsi < self.rsi_os:
                    signals.append(Signal(
                        symbol=symbol, direction="SELL",  # INVERSE: was BUY
                        confidence=round(conf, 2),
                        entry_price=cur_close,
                        take_profit=round(cur_close - cur_atr * self.bo_tp_atr, 2),  # use BO TP
                        stop_loss=round(cur_close + cur_atr * self.mr_sl_atr, 2),
                        reason=f"MR_SELL_INV ADX={cur_adx:.1f} RSI={cur_rsi:.1f}",
                        mode="MEAN_REV"
                    ))
                    last_signal_bar = i
                # INVERSE: upper BB + overbought -> BUY (continuation breakout)
                    ...
                    signals.append(Signal(
                        ...
                        take_profit=round(cur_close + cur_atr * self.bo_tp_atr, 2),  # use BO TP
                        stop_loss=round(cur_close - cur_atr * self.mr_sl_atr, 2),
                        reason=f"MR_BUY_INV ADX={cur_adx:.1f} RSI={cur_rsi:.1f}",
                        mode="MEAN_REV"
                    ))
                    last_signal_bar = i
            else:
                ...
                # INVERSE: above upper BB -> SHORT (fade breakout)
                if cur_close > cur_bb_upper:
                    signals.append(Signal(
                        ...
                        reason=f"BO_SHORT_INV ADX={cur_adx:.1f}",
                        mode="BREAKOUT"
                    ))
                    last_signal_bar = i
                # INVERSE: below lower BB -> LONG (fade breakdown)
                    ...
                    signals.append(Signal(
                        ...
                        take_profit=round(cur_close + cur_atr * self.bo_tp_atr, 2),
                        stop_loss=round(cur_close - cur_atr * self.bo_sl_atr, 2),
                        reason=f"BO_LONG_INV ADX={cur_adx:.1f}",
                        mode="BREAKOUT"
                    ))
                    last_signal_bar = i

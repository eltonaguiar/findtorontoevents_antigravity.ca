"""
crypto_soc_vol_expansion_index_a10_v1 - Social-Inspired High-Sharpe Baby Strat
====================================================

Created by: web_ai
Date: 2026-02-26
Archetype: vol_expansion_index
...
class CryptoSoc092VolExpansionIndexStrategy:
    """Composite social-theme strategy: vol_expansion_index."""
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        self.trend_w = self.p.get('trend_w', 0.2944)
        self.meanrev_w = self.p.get('meanrev_w', 0.0792)
        self.breakout_w = self.p.get('breakout_w', 0.1651)
        self.volstate_w = self.p.get('volstate_w', 0.3026)
        self.flow_w = self.p.get('flow_w', 0.1052)
        self.session_w = self.p.get('session_w', 0.0536)

        self.long_threshold = self.p.get('long_threshold', 0.500)
        self.short_threshold = self.p.get('short_threshold', 0.500)
    ...
    def generate_signals(self, data: pd.DataFrame, symbol: str = 'BTCUSDT') -> List[Signal]:
        ...
        if composite > self.long_threshold:
            ...
            signals.append(self._mk(symbol, 'BUY', px, a, conf, f'vol_expansion_index comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
        elif composite < -self.short_threshold:
            conf = min(0.95, max(0.1, 0.52 + ((-composite) - self.short_threshold) * 0.55))
            signals.append(self._mk(symbol, 'SELL', px, a, conf, f'vol_expansion_index comp={composite:.2f} vei={vei.iloc[-1]:.2f}'))
...
if __name__ == '__main__':
    ...
    s = CryptoSoc092VolExpansionIndexStrategy()
    total = 0
